...
static pj_pool_t *cpool_create_pool(pj_pool_factory *pf, const char *name, pj_size_t initial_size,
                                    pj_size_t increment_sz, pj_pool_callback *callback);
...
static pj_size_t pool_sizes[PJ_CACHING_POOL_ARRAY_SIZE] = {256,   512,   1024,  2048,  4096,  8192,  12288, 16384,
...
PJ_DEF(void) pj_caching_pool_init(pj_caching_pool *cp, const pj_pool_factory_policy *policy, pj_size_t max_capacity)
{
    int i;
    pj_pool_t *pool;
...
PJ_DEF(void) pj_caching_pool_destroy(pj_caching_pool *cp)
{
    ...
    while (pool != (pj_pool_t *)&cp->used_list) {
        ...
        pj_list_erase(pool);
        PJ_LOG(4, (pool->obj_name, "Pool is not released by application, releasing now"));
        pj_pool_destroy_int(pool);
        pool = next;
    }
                                    ...
                                    pj_size_t increment_sz, pj_pool_callback *callback)
{
    ...
    if (initial_size <= pool_sizes[START_SIZE]) {
        ...
        for (idx = START_SIZE + 1; idx < PJ_CACHING_POOL_ARRAY_SIZE && pool_sizes[idx] < initial_size; ++idx)
            ;
    }
    ...
    if (idx == PJ_CACHING_POOL_ARRAY_SIZE || pj_list_empty(&cp->free_list[idx])) {
        ...
        /* Create new pool */
        pool = pj_pool_create_int(&cp->factory, name, initial_size, increment_sz, callback);
        if (!pool) {
...
static void cpool_release_pool(pj_pool_factory *pf, pj_pool_t *pool)
{
     ...
     * maximum capacity.
   . */
    if (pool_capacity > pool_sizes[PJ_CACHING_POOL_ARRAY_SIZE - 1] || cp->capacity + pool_capacity > cp->max_capacity) {
        pj_pool_destroy_int(pool);
        pj_lock_release(cp->lock);
        return;
    }
    ...
    /* Reset pool. */
    PJ_LOG(6, (pool->obj_name, "recycle(): cap=%d, used=%d(%d%%)", pool_capacity, pj_pool_get_used_size(pool),
...
static void cpool_dump_status(pj_pool_factory *factory, pj_bool_t detail)
{
    ...
    PJ_LOG(3, ("cachpool", "   Capacity=%u, max_capacity=%u, used_cnt=%u", cp->capacity, cp->max_capacity,
               cp->used_count));
    if (detail) {
        ...
        while (pool != (void *)&cp->used_list) {
            ...
            PJ_LOG(3, ("cachpool", "   %16s: %8d of %8d (%d%%) used", pj_pool_getobjname(pool),
        ...
        if (total_capacity) {
            PJ_LOG(3, ("cachpool", "  Total %9d of %9d (%d %%) used!", total_used, total_capacity,
