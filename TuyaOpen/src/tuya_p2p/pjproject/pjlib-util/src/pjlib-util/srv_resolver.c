...
static void dns_callback(void *user_data, pj_status_t status, pj_dns_parsed_packet *pkt);

/*
 * The public API to invoke DNS SRV resolution.
 */
PJ_DEF(pj_status_t)
pj_dns_srv_resolve(const pj_str_t *domain_name, const pj_str_t *res_name, unsigned def_port, pj_pool_t *pool,
                   pj_dns_resolver *resolver, unsigned option, void *token, pj_dns_srv_resolver_cb *cb,
                   pj_dns_srv_async_query **p_query)
{
    ...
    PJ_ASSERT_RETURN(domain_name && domain_name->slen && res_name && res_name->slen && pool && resolver && cb,
                     PJ_EINVAL);
    ...
    PJ_LOG(5, (query_job->objname, "Starting async DNS %s query_job: target=%.*s:%d",
               pj_dns_get_type_name(query_job->dns_state), (int)target_name.slen, target_name.ptr, def_port));

    status = pj_dns_resolver_start_query(resolver, &target_name, query_job->dns_state, 0, &dns_callback, query_job,
                                         &query_job->q_srv);
 ...
 */
PJ_DEF(pj_status_t) pj_dns_srv_cancel_query(pj_dns_srv_async_query *query, pj_bool_t notify)
{
...
static void build_server_entries(pj_dns_srv_async_query *query_job, pj_dns_parsed_packet *response)
{
    unsigned i;

    /* Save the Resource Records in DNS answer into SRV targets. */
    query_job->srv_cnt = 0;
    for (i = 0; i < response->hdr.anscount && query_job->srv_cnt < PJ_DNS_SRV_MAX_ADDR; ++i) {
        pj_dns_parsed_rr *rr = &response->ans[i];
        struct srv_target *srv = &query_job->srv[query_job->srv_cnt];

        if (rr->type != PJ_DNS_TYPE_SRV) {
            PJ_LOG(4, (query_job->objname, "Received non SRV answer for SRV query_job!"));
            continue;
        }
        ...
        pj_strncpy(&srv->target_name, &rr->rdata.srv.target, sizeof(srv->target_buf));
    ...
    if (query_job->srv_cnt == 0) {
        PJ_LOG(4, (query_job->objname, "Could not find SRV record in DNS answer!"));
        return;
    }
    ...
    for (i = 0; i < query_job->srv_cnt; ++i) {
        ...
        for (j = i + 1; j < query_job->srv_cnt && query_job->srv[j].priority == query_job->srv[i].priority; ++j) {
    ...
    for (i = 0; i < response->hdr.arcount; ++i) {
         ...
         */
        if ((rr->type == PJ_DNS_TYPE_A && (query_job->option & PJ_DNS_SRV_RESOLVE_AAAA_ONLY) != 0) ||
            (rr->type == PJ_DNS_TYPE_AAAA && (query_job->option & PJ_DNS_SRV_RESOLVE_AAAA) == 0)) {
            continue;
        }
        ...
        for (j = 0; j < query_job->srv_cnt; ++j) {
            if (pj_stricmp(&rr->name, &query_job->srv[j].target_name) == 0 &&
                ...
                if (rr->type == PJ_DNS_TYPE_A) {
                    pj_sockaddr_init(pj_AF_INET(), &query_job->srv[j].addr[cnt], NULL, query_job->srv[j].port);
                    query_job->srv[j].addr[cnt].ipv4.sin_addr = rr->rdata.a.ip_addr;
                } else {
                    pj_sockaddr_init(pj_AF_INET6(), &query_job->srv[j].addr[cnt], NULL, query_job->srv[j].port);
                    query_job->srv[j].addr[cnt].ipv6.sin6_addr = rr->rdata.aaaa.ip_addr;
                }
    ...
    for (i = 0; i < query_job->srv_cnt; ++i) {
        ...
        if ((query_job->option & PJ_DNS_SRV_RESOLVE_AAAA_ONLY) == 0 &&
            pj_inet_pton(pj_AF_INET(), &query_job->srv[i].target_name, &addr) == PJ_SUCCESS) {
            pj_sockaddr_init(pj_AF_INET(), &query_job->srv[i].addr[cnt], NULL, query_job->srv[i].port);
            query_job->srv[i].addr[cnt].ipv4.sin_addr = addr;
            ++query_job->srv[i].addr_cnt;
            ++query_job->host_resolved;
        } else if ((query_job->option & PJ_DNS_SRV_RESOLVE_AAAA) != 0 &&
                   pj_inet_pton(pj_AF_INET6(), &query_job->srv[i].target_name, &addr6) == PJ_SUCCESS) {
            pj_sockaddr_init(pj_AF_INET6(), &query_job->srv[i].addr[cnt], NULL, query_job->srv[i].port);
    ...
    PJ_LOG(5, (query_job->objname,
               ...
               (int)query_job->full_name.slen, query_job->full_name.ptr, query_job->srv_cnt, response->hdr.anscount,
               (query_job->srv_cnt ? ':' : ' ')));
    ...
    for (i = 0; i < query_job->srv_cnt; ++i) {
        ...
        if (query_job->srv[i].addr_cnt != 0) {
            pj_sockaddr_print(&query_job->srv[i].addr[0], addr, sizeof(addr), 2);
        } else
            pj_ansi_strcpy(addr, "-");

        PJ_LOG(5, (query_job->objname, " %d: SRV %d %d %d %.*s (%s)", i, query_job->srv[i].priority,
                   query_job->srv[i].weight, query_job->srv[i].port, (int)query_job->srv[i].target_name.slen,
...
static pj_status_t resolve_hostnames(pj_dns_srv_async_query *query_job)
{
    ...
    for (i = 0; i < query_job->srv_cnt; ++i) {
        ...
        PJ_LOG(5, (query_job->objname, "Starting async DNS A query_job for %.*s", (int)srv->target_name.slen,
                   srv->target_name.ptr));
        ...
        if ((query_job->option & PJ_DNS_SRV_RESOLVE_AAAA_ONLY) == 0) {
            ...
            status = pj_dns_resolver_start_query(query_job->resolver, &srv->target_name, PJ_DNS_TYPE_A, 0,
                                                 &dns_callback, &srv->common, &srv->q_a);
        }

        /* Start DNS AAAA record query */
        if (status == PJ_SUCCESS && (query_job->option & PJ_DNS_SRV_RESOLVE_AAAA) != 0) {
            status = pj_dns_resolver_start_query(query_job->resolver, &srv->target_name, PJ_DNS_TYPE_AAAA, 0,
...
static void dns_callback(void *user_data, pj_status_t status, pj_dns_parsed_packet *pkt)
{
    ...
    if (query_job->dns_state == PJ_DNS_TYPE_SRV) {
        ...
        if (status == PJ_SUCCESS) {
            if (PJ_DNS_GET_TC(pkt->hdr.flags)) {
                 ...
                 * just return error.
                 */
                PJ_LOG(4, (query_job->objname, "Discard truncated DNS SRV response for %.*s",
            ...
            PJ_LOG(4, (query_job->objname, "DNS SRV resolution failed for %.*s: %s", (int)query_job->full_name.slen,
                       query_job->full_name.ptr, errmsg));

            /* Trigger error when fallback is disabled */
            if ((query_job->option & (PJ_DNS_SRV_FALLBACK_A | PJ_DNS_SRV_FALLBACK_AAAA)) == 0) {
                goto on_error;
            }
        }
        ...
        if (query_job->srv_cnt == 0 && query_job->domain_part.slen > 0) {
            ...
            PJ_LOG(4, (query_job->objname,
                       ...
                       (int)query_job->full_name.slen, query_job->full_name.ptr, (int)query_job->domain_part.slen,
            ...
            if (query_job->option & PJ_DNS_SRV_FALLBACK_AAAA)
                new_option |= (PJ_DNS_SRV_RESOLVE_AAAA | PJ_DNS_SRV_RESOLVE_AAAA_ONLY);
        ...
        if (status == PJ_SUCCESS && pkt->hdr.anscount != 0) {
            ...
            if (status != PJ_SUCCESS) {
                PJ_PERROR(4,
                          (query_job->objname, status, "DNS %s record parse error for '%.*s'.",
                           (is_type_a ? "A" : "AAAA"), (int)query_job->domain_part.slen, query_job->domain_part.ptr));
            }
        }
        ...
        if (status == PJ_SUCCESS && pkt->hdr.anscount != 0) {
            ...
            for (i = 0; i < rec.addr_count && srv->addr_cnt < ADDR_MAX_COUNT; ++i) {
                ...
                if (is_type_a && rec.addr[i].af == pj_AF_INET()) {
                    pj_sockaddr_init(pj_AF_INET(), &srv->addr[srv->addr_cnt], NULL, srv->port);
                    srv->addr[srv->addr_cnt].ipv4.sin_addr = rec.addr[i].ip.v4;
                    added = PJ_TRUE;
                } else if (!is_type_a && rec.addr[i].af == pj_AF_INET6()) {
                    pj_sockaddr_init(pj_AF_INET6(), &srv->addr[srv->addr_cnt], NULL, srv->port);
                     ...
                     */
                    PJ_LOG(4, (query_job->objname, "Bad address family in DNS %s query for %.*s",
                               (is_type_a ? "A" : "AAAA"), (int)srv->target_name.slen, srv->target_name.ptr));
                }

                if (added) {
                    PJ_LOG(5, (query_job->objname, "DNS %s for %.*s: %s", (is_type_a ? "A" : "AAAA"),
                               (int)srv->target_name.slen, srv->target_name.ptr,
                               pj_sockaddr_print(&srv->addr[srv->addr_cnt], addr, sizeof(addr), 2)));
            ...
            /* Log error */
            PJ_PERROR(4, (query_job->objname, status, "DNS %s record resolution failed", (is_type_a ? "A" : "AAAA")));
        }
    ...
    if (query_job->host_resolved == query_job->srv_cnt) {
        ...
        PJ_LOG(5, (query_job->objname, "Server resolution complete, %d server entry(s) found", srv_rec.count));
...
on_error:
    /* Check for failure */
    if (status != PJ_SUCCESS) {
        PJ_PERROR(4, (query_job->objname, status, "DNS %s record resolution error for '%.*s'.",
                      pj_dns_get_type_name(query_job->dns_state), (int)query_job->domain_part.slen,
                      query_job->domain_part.ptr));
