...
PJ_DEF(void) pj_sha1_update(pj_sha1_context *context, const pj_uint8_t *data, const pj_size_t len)
{
    pj_size_t i, j;
...
PJ_DEF(void) pj_sha1_final(pj_sha1_context *context, pj_uint8_t digest[PJ_SHA1_DIGEST_SIZE])
{
    pj_uint32_t i;
    ...
    for (i = 0; i < PJ_SHA1_DIGEST_SIZE; i++) {
        digest[i] = (pj_uint8_t)((context->state[i >> 2] >> ((3 - (i & 3)) * 8)) & 255);
    }
