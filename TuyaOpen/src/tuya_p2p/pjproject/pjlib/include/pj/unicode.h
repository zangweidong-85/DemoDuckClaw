 ...
 */
PJ_DECL(wchar_t *) pj_ansi_to_unicode(const char *str, int len, wchar_t *wbuf, int wbuf_count);
 ...
 */
PJ_DECL(char *) pj_unicode_to_ansi(const wchar_t *wstr, pj_ssize_t len, char *buf, int buf_size);
 ...
 */
#define PJ_STRING_TO_NATIVE(s, buf, max) pj_ansi_to_unicode(s, strlen(s), buf, max)
 ...
 * native string is Unicode (PJ_NATIVE_STRING_IS_UNICODE is non-zero).
 */
#define PJ_NATIVE_TO_STRING(cs, buf, max) pj_unicode_to_ansi(cs, wcslen(cs), buf, max)
