...
PJ_DEF(pj_str_t) pj_str_unescape(pj_pool_t *pool, const pj_str_t *src_str)
{
    ...
    while (src != end) {
        if (*src == '%' && src < end - 2 && pj_isxdigit(*(src + 1)) && pj_isxdigit(*(src + 2))) {
            *dst = (pj_uint8_t)((pj_hex_digit_to_val(*(src + 1)) << 4) + pj_hex_digit_to_val(*(src + 2)));
            ++dst;
            src += 3;
        } else {
            *dst++ = *src++;
...
PJ_DEF(pj_ssize_t) pj_strncpy2_escape(char *dst_str, const pj_str_t *src_str, pj_ssize_t max, const pj_cis_t *unres)
{
    const char *src = src_str->ptr;
...
PJ_DEF(pj_str_t *) pj_strncpy_escape(pj_str_t *dst_str, const pj_str_t *src_str, pj_ssize_t max, const pj_cis_t *unres)
{
