...
static pj_uint32_t parse_version(char *str)
{
    ...
    maxtok = 4;
    delim = pj_str(".-");
    for (found_idx = pj_strtok(&in_str, &delim, &token, 0), i = 0; found_idx != in_str.slen && i < maxtok;
         ++i, found_idx = pj_strtok(&in_str, &delim, &token, found_idx + token.slen)) {
        int n;
...
#if (defined(PJ_WIN32_WINCE) && PJ_WIN32_WINCE) || (defined(PJ_WIN32_WINPHONE8) && PJ_WIN32_WINPHONE8)
 ...
 */
#if _WIN32_WCE <= 500
    si.sdk_ver = ((_WIN32_WCE / 100) << 24) | (((_WIN32_WCE % 100) / 10) << 16) | ((_WIN32_WCE % 10) << 8);
#else
    si.sdk_ver = (((_WIN32_WCE & 0xFF00) >> 8) << 24) | (((_WIN32_WCE & 0x00F0) >> 4) << 16) |
     ...
     */
    si.sdk_ver = ((_MSC_VER / 100) << 24) | (((_MSC_VER % 100) / 10) << 16) | ((_MSC_VER % 10) << 8);
     ...
     */
    {
        ...
        cnt = pj_ansi_snprintf(tmp, sizeof(tmp), "%s%s%s%s%s%s%s", si.os_name.ptr, ver_info(si.os_ver, os_ver),
                               (si.machine.slen ? "/" : ""), si.machine.ptr, (si.sdk_name.slen ? "/" : ""),
                               si.sdk_name.ptr, ver_info(si.sdk_ver, sdk_ver));
