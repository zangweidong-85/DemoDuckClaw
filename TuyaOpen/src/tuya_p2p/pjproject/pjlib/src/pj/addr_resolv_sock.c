...
PJ_DEF(pj_status_t) pj_getaddrinfo(int af, const pj_str_t *nodename, unsigned *count, pj_addrinfo ai[])
{
...
#if defined(PJ_GETADDRINFO_USE_CFHOST) && PJ_GETADDRINFO_USE_CFHOST != 0
    hostname = CFStringCreateWithCStringNoCopy(kCFAllocatorDefault, nodecopy, kCFStringEncodingASCII, kCFAllocatorNull);
    ...
    if (CFHostStartInfoResolution(hostRef, kCFHostAddresses, nil)) {
        ...
        if (addrRef != nil) {
            ...
            for (idx = 0; idx < naddr && i < *count; idx++) {
                ...
                addr = (struct sockaddr *)CFDataGetBytePtr(CFArrayGetValueAtIndex(addrRef, idx));
    ...
    for (i = 0; i < *count && res; res = res->ai_next) {
        ...
        if (res->ai_socktype != pj_SOCK_DGRAM() && res->ai_socktype != pj_SOCK_STREAM() &&
...
/* VC6 complains that "he" is uninitialized */
        ...
        *count = 0;
        ...
        for (i = 0; he.h_addr_list[i] && *count < max_count; ++i) {
            pj_ansi_strncpy(ai[*count].ai_canonname, he.h_name, sizeof(ai[*count].ai_canonname));
            ai[*count].ai_canonname[sizeof(ai[*count].ai_canonname) - 1] = '\0';

            ai[*count].ai_addr.ipv4.sin_family = PJ_AF_INET;
            pj_memcpy(&ai[*count].ai_addr.ipv4.sin_addr, he.h_addr_list[i], he.h_length);
