...
static void console_write_log(pj_cli_front_end *fe, int level, const char *data, pj_size_t len)
{
...
PJ_DEF(pj_status_t)
pj_cli_console_create(pj_cli_t *cli, const pj_cli_console_cfg *param, pj_cli_sess **p_sess, pj_cli_front_end **p_fe)
{
    pj_cli_sess *sess;
    ...
    pool = pj_pool_create(pj_cli_get_param(cli)->pf, "console_fe", PJ_CLI_CONSOLE_POOL_SIZE, PJ_CLI_CONSOLE_POOL_INC,
                          NULL);
    if (!pool)
        return PJ_ENOMEM;
    ...
    if (param->prompt_str.slen == 0) {
        ...
        fe->cfg.prompt_str.ptr = pj_pool_alloc(fe->pool, param->prompt_str.slen + 1);
...
static void send_inv_arg(pj_cli_sess *sess, const pj_cli_exec_info *info, pj_bool_t with_return)
{
    static const pj_str_t ERR_MSG = {"%Error : Invalid Arguments\r\n", 28};
    send_err_arg(sess, info, &ERR_MSG, with_return);
}

static void send_too_many_arg(pj_cli_sess *sess, const pj_cli_exec_info *info, pj_bool_t with_return)
{
    static const pj_str_t ERR_MSG = {"%Error : Too Many Arguments\r\n", 29};
    send_err_arg(sess, info, &ERR_MSG, with_return);
}

static void send_hint_arg(pj_str_t *send_data, const pj_str_t *desc, pj_ssize_t cmd_len, pj_ssize_t max_len)
{
    if ((desc) && (desc->slen > 0)) {
        int j;
...
static void send_ambi_arg(pj_cli_sess *sess, const pj_cli_exec_info *info, pj_bool_t with_return)
{
    unsigned i;
    pj_size_t len;
    pj_str_t send_data;
...
static pj_bool_t handle_hint(pj_cli_sess *sess)
{
    ...
    pool = pj_pool_create(pj_cli_get_param(cli)->pf, "handle_hint", PJ_CLI_CONSOLE_POOL_SIZE, PJ_CLI_CONSOLE_POOL_INC,
                          NULL);
...
static pj_bool_t handle_exec(pj_cli_sess *sess)
{
    ...
    pool = pj_pool_create(pj_cli_get_param(cli)->pf, "handle_exec", PJ_CLI_CONSOLE_POOL_SIZE, PJ_CLI_CONSOLE_POOL_INC,
                          NULL);
    ...
    (!defined(PJ_WIN32_WINCE) || PJ_WIN32_WINCE == 0)
            ...
            if (1) {
                ...
                if (fe->cfg.quit_command.slen) {
                    pj_memcpy(recv_buf, fe->cfg.quit_command.ptr, fe->input.maxlen);
                }
...
PJ_DEF(pj_status_t) pj_cli_console_process(pj_cli_sess *sess, char *buf, unsigned maxlen)
{
    struct cli_console_fe *fe = (struct cli_console_fe *)sess->fe;
    ...
    if (!fe->input_thread) {
        ...
        status = pj_thread_create(fe->pool, NULL, &readline_thread, fe, 0, 0, &fe->input_thread);
        if (status != PJ_SUCCESS)
            return status;
    } else {
