...
PJ_DEF(void) pj_stun_auth_cred_dup(pj_pool_t *pool, pj_stun_auth_cred *dst, const pj_stun_auth_cred *src)
{
    dst->type = src->type;

    switch (src->type) {
    case PJ_STUN_AUTH_CRED_STATIC:
        pj_strdup(pool, &dst->data.static_cred.realm, &src->data.static_cred.realm);
        pj_strdup(pool, &dst->data.static_cred.username, &src->data.static_cred.username);
        dst->data.static_cred.data_type = src->data.static_cred.data_type;
        pj_strdup(pool, &dst->data.static_cred.data, &src->data.static_cred.data);
        pj_strdup(pool, &dst->data.static_cred.nonce, &src->data.static_cred.nonce);
        break;
    case PJ_STUN_AUTH_CRED_DYNAMIC:
        pj_memcpy(&dst->data.dyn_cred, &src->data.dyn_cred, sizeof(src->data.dyn_cred));
        break;
    }
}
 ...
 */
PJ_DEF(void) pj_stun_req_cred_info_dup(pj_pool_t *pool, pj_stun_req_cred_info *dst, const pj_stun_req_cred_info *src)
{
 ...
 */
static void calc_md5_key(pj_uint8_t digest[16], const pj_str_t *realm, const pj_str_t *username, const pj_str_t *passwd)
{
 ...
 */
PJ_DEF(void)
pj_stun_create_key(pj_pool_t *pool, pj_str_t *key, const pj_str_t *realm, const pj_str_t *username,
                   pj_stun_passwd_type data_type, const pj_str_t *data)
{
...
/* Send 401 response */
static pj_status_t create_challenge(pj_pool_t *pool, const pj_stun_msg *msg, int err_code, const char *errstr,
                                    const pj_str_t *realm, const pj_str_t *nonce, pj_stun_msg **p_response)
{
    ...
    rc = pj_stun_msg_create_response(pool, msg, err_code, (errstr ? pj_cstr(&err_msg, errstr) : NULL), &response);
    if (rc != PJ_SUCCESS)
        return rc;

    /* SHOULD NOT add REALM, NONCE, USERNAME, and M-I on 400 response */
    if (err_code != 400 && realm && realm->slen) {
        rc = pj_stun_msg_add_string_attr(pool, response, PJ_STUN_ATTR_REALM, realm);
        if (rc != PJ_SUCCESS)
            return rc;
    ...
    if (err_code != 400 && nonce && nonce->slen) {
        rc = pj_stun_msg_add_string_attr(pool, response, PJ_STUN_ATTR_NONCE, nonce);
        if (rc != PJ_SUCCESS)
            return rc;
    }
...
PJ_DEF(pj_status_t)
pj_stun_authenticate_request(const pj_uint8_t *pkt, unsigned pkt_len, const pj_stun_msg *msg, pj_stun_auth_cred *cred,
                             pj_pool_t *pool, pj_stun_req_cred_info *p_info, pj_stun_msg **p_response)
{
    ...
    if (cred->type == PJ_STUN_AUTH_CRED_STATIC) {
        ...
        status = cred->data.dyn_cred.get_auth(cred->data.dyn_cred.user_data, pool, &p_info->realm, &p_info->nonce);
    ...
    if (amsgi == NULL) {
        ...
        */
        err_code = p_info->realm.slen ? PJ_STUN_SC_UNAUTHORIZED : PJ_STUN_SC_BAD_REQUEST;
        goto on_auth_failed;
    }

    /* Next check that USERNAME is present */
    auser = (const pj_stun_username_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_USERNAME, 0);
    if (auser == NULL) {
        /* According to rfc3489bis-10 Sec 10.1.2/10.2.2, we should return 400
    ...
    /* Get REALM, if any */
    arealm = (const pj_stun_realm_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_REALM, 0);
    ...
    if (cred->type == PJ_STUN_AUTH_CRED_STATIC) {
        ...
        username_ok = !pj_strcmp(&auser->value, &cred->data.static_cred.username);
        if (username_ok) {
            pj_strdup(pool, &p_info->username, &cred->data.static_cred.username);
            pj_stun_create_key(pool, &p_info->auth_key, &p_info->realm, &auser->value, cred->data.static_cred.data_type,
        ...
        rc = cred->data.dyn_cred.get_password(msg, cred->data.dyn_cred.user_data, (arealm ? &arealm->value : NULL),
                                              &auser->value, pool, &data_type, &password);
        if (rc == PJ_SUCCESS) {
            pj_strdup(pool, &p_info->username, &auser->value);
            pj_stun_create_key(pool, &p_info->auth_key, (arealm ? &arealm->value : NULL), &auser->value, data_type,
                               &password);
        } else {
            err_code = PJ_STUN_SC_UNAUTHORIZED;
            goto on_auth_failed;
    ...
    /* Get NONCE attribute */
    anonce = (pj_stun_nonce_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_NONCE, 0);
    ...
    if (anonce) {
        ...
        if (cred->type == PJ_STUN_AUTH_CRED_DYNAMIC && cred->data.dyn_cred.verify_nonce != NULL) {
            ok = cred->data.dyn_cred.verify_nonce(msg, cred->data.dyn_cred.user_data, (arealm ? &arealm->value : NULL),
                                                  &auser->value, &anonce->value);
        } else if (cred->type == PJ_STUN_AUTH_CRED_DYNAMIC) {
            ok = PJ_TRUE;
        } else {
    ...
    pj_hmac_sha1_init(&ctx, (pj_uint8_t *)p_info->auth_key.ptr, (unsigned)p_info->auth_key.slen);
...
on_auth_failed:
    if (p_response) {
        create_challenge(pool, msg, err_code, err_text, &p_info->realm, &p_info->nonce, p_response);
    }
...
PJ_DEF(pj_bool_t) pj_stun_auth_valid_for_msg(const pj_stun_msg *msg)
{
    ...
    if (!PJ_STUN_IS_ERROR_RESPONSE(msg_type) && !PJ_STUN_IS_INDICATION(msg_type)) {
        return PJ_TRUE;
    }
     ...
     * code.
     */
    err_attr = (const pj_stun_errcode_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_ERROR_CODE, 0);
    if (err_attr == NULL) {
...
PJ_DEF(pj_status_t)
pj_stun_authenticate_response(const pj_uint8_t *pkt, unsigned pkt_len, const pj_stun_msg *msg, const pj_str_t *key)
{
    const pj_stun_msgint_attr *amsgi;
    unsigned i, amsgi_pos;
    pj_bool_t has_attr_beyond_mi;
    ...
    amsgi = (const pj_stun_msgint_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_MESSAGE_INTEGRITY, 0);
    if (amsgi == NULL) {
