...
static pj_xml_node *xml_parse_node(pj_pool_t *pool, pj_scanner *scanner)
{
    ...
    /* CDATA content. */
    if (*scanner->curptr == '<' && *(scanner->curptr + 1) == '!' && pj_scan_strcmp(scanner, "<![CDATA[", 9) == 0) {
...
PJ_DEF(pj_xml_node *) pj_xml_parse(pj_pool_t *pool, char *msg, pj_size_t len)
{
    ...
    {
        PJ_LOG(4,
               (THIS_FILE, "Syntax error parsing XML in line %d column %d", scanner.line, pj_scan_get_col(&scanner)));
    }
    PJ_END;
...
/* This is a recursive function. */
static int xml_print_node(const pj_xml_node *node, int indent, char *buf, pj_size_t len)
{
    int i;
    char *p = buf;
    pj_xml_attr *attr;
...
#define SIZE_LEFT() ((int)(len - (p - buf)))
    ...
    *p++ = '<';
    ...
    if (node->content.slen == 0 && node->node_head.next == (pj_xml_node *)&node->node_head) {
        if (SIZE_LEFT() < 3)
            return -1;
        *p++ = ' ';
        *p++ = '/';
...
PJ_DEF(int) pj_xml_print(const pj_xml_node *node, char *buf, pj_size_t len, pj_bool_t include_prolog)
{
    int prolog_len = 0;
    int printed;
...
PJ_DEF(pj_xml_attr *) pj_xml_attr_new(pj_pool_t *pool, const pj_str_t *name, const pj_str_t *value)
{
    pj_xml_attr *attr = alloc_attr(pool);
...
PJ_DEF(pj_xml_node *) pj_xml_find_node(const pj_xml_node *parent, const pj_str_t *name)
{
...
PJ_DEF(pj_xml_node *) pj_xml_find_node_rec(const pj_xml_node *parent, const pj_str_t *name)
{
    const pj_xml_node *node = parent->node_head.next;
...
PJ_DEF(pj_xml_node *) pj_xml_find_next_node(const pj_xml_node *parent, const pj_xml_node *node, const pj_str_t *name)
{
    PJ_CHECK_STACK();
...
PJ_DEF(pj_xml_attr *) pj_xml_find_attr(const pj_xml_node *node, const pj_str_t *name, const pj_str_t *value)
{
...
PJ_DEF(pj_xml_node *)
pj_xml_find_rec(const pj_xml_node *parent, const pj_str_t *name, const void *data,
                pj_bool_t (*match)(const pj_xml_node *, const void *))
{
    const pj_xml_node *node = (const pj_xml_node *)parent->node_head.next;
