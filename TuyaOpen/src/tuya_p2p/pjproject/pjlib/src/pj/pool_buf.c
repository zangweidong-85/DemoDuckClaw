...
PJ_DEF(pj_pool_t *) pj_pool_create_on_buf(const char *name, void *buf, pj_size_t size)
{
    ...
    return pj_pool_create_int(&stack_based_factory, name, size, 0, pj_pool_factory_default_policy.callback);
#else
    PJ_UNUSED_ARG(buf);
