...
#if defined(PJ_HAS_SSL_SOCK) && PJ_HAS_SSL_SOCK != 0 && (PJ_SSL_SOCK_IMP == PJ_SSL_SOCK_IMP_GNUTLS)
...
static pj_str_t tls_strerror(pj_status_t status, char *buf, pj_size_t bufsize)
{
    ...
    errstr.ptr = buf;
    errstr.slen = pj_ansi_snprintf(buf, bufsize, "GnuTLS error %d: %s", tls_last_error, tmp);
...
static pj_status_t tls_init(void)
{
    ...
    pj_status_t status =
        pj_register_strerror(PJ_ERRNO_START_USER + PJ_ERRNO_SPACE_SIZE * 6, PJ_ERRNO_SPACE_SIZE, &tls_strerror);
    ...
    if (!ssl_cipher_num) {
        ...
        for (i = 0; i < PJ_ARRAY_SIZE(ssl_ciphers); i++) {
            ...
            suite = gnutls_cipher_suite_info(i, (unsigned char *)id, NULL, NULL, NULL, NULL);
            ssl_ciphers[i].id = 0;
            ...
            if (suite) {
                ssl_ciphers[i].id = (pj_ssl_cipher)(pj_uint32_t)((id[0] << 8) | id[1]);
...
static pj_status_t tls_priorities_set(pj_ssl_sock_t *ssock)
{
    ...
    for (i = 0; i < ssock->param.ciphers_num; i++) {
        for (j = 0;; j++) {
            ...
            suite = gnutls_cipher_suite_info(j, (unsigned char *)id, &kx, &algo, &mac, &proto);
            if (!suite)
                break;
...
static pj_status_t tls_trust_set(pj_ssl_sock_t *ssock)
{
    ...
    if (err > 0)
        ntrusts += err;
    err = gnutls_certificate_set_x509_trust_file(gssock->xcred, TRUST_STORE_FILE1, GNUTLS_X509_FMT_PEM);
    if (err > 0)
        ntrusts += err;

    err = gnutls_certificate_set_x509_trust_file(gssock->xcred, TRUST_STORE_FILE2, GNUTLS_X509_FMT_PEM);
    if (err > 0)
        ntrusts += err;
...
static int gnutls_certificate_set_x509_trust_dir(gnutls_certificate_credentials_t cred, const char *dirname,
                                                 unsigned type)
{
    DIR *dirp;
    struct dirent *d;
...
static pj_status_t ssl_create(pj_ssl_sock_t *ssock)
{
    ...
    ret = gnutls_init(&gssock->session, ssock->is_server ? GNUTLS_SERVER : GNUTLS_CLIENT);
    if (ret < 0)
        goto out;

    /* Set the ssock object to be retrieved by transport (send/recv) and by
     * user data from this session */
    gnutls_transport_set_ptr(gssock->session, (gnutls_transport_ptr_t)(uintptr_t)ssock);
    gnutls_session_set_ptr(gssock->session, (gnutls_transport_ptr_t)(uintptr_t)ssock);
    ...
    if (cert) {
        ...
        if (cert->CA_file.slen) {
            ret = gnutls_certificate_set_x509_trust_file(gssock->xcred, cert->CA_file.ptr, GNUTLS_X509_FMT_PEM);
            if (ret < 0)
                ret = gnutls_certificate_set_x509_trust_file(gssock->xcred, cert->CA_file.ptr, GNUTLS_X509_FMT_DER);
            if (ret < 0)
                goto out;
        }
        if (cert->CA_path.slen) {
            ret = gnutls_certificate_set_x509_trust_dir(gssock->xcred, cert->CA_path.ptr, GNUTLS_X509_FMT_PEM);
            if (ret < 0)
                ret = gnutls_certificate_set_x509_trust_dir(gssock->xcred, cert->CA_path.ptr, GNUTLS_X509_FMT_DER);
            if (ret < 0)
                goto out;
        }
        ...
        if (cert->cert_file.slen && cert->privkey_file.slen) {
            ...
            const char *prikey_pass = cert->privkey_pass.slen ? cert->privkey_pass.ptr : NULL;
            ret = gnutls_certificate_set_x509_key_file2(gssock->xcred, cert->cert_file.ptr, prikey_file,
        ...
        if (cert->CA_buf.slen) {
            ...
            ret = gnutls_certificate_set_x509_trust_mem(gssock->xcred, &ca, GNUTLS_X509_FMT_PEM);
            if (ret < 0)
                ret = gnutls_certificate_set_x509_trust_mem(gssock->xcred, &ca, GNUTLS_X509_FMT_DER);
            if (ret < 0)
                goto out;
        }
        ...
        if (cert->cert_buf.slen && cert->privkey_buf.slen) {
            ...
            const char *prikey_pass = cert->privkey_pass.slen ? cert->privkey_pass.ptr : NULL;
            ret = gnutls_certificate_set_x509_key_mem2(gssock->xcred, &cert_buf, &privkey_buf, GNUTLS_X509_FMT_PEM,
                                                       prikey_pass, 0);
            /* Load DER format */
            /*
            if (ret != GNUTLS_E_SUCCESS)
    ...
    if (ssock->is_server && ssock->param.require_client_cert)
        gnutls_certificate_server_set_request(gssock->session, GNUTLS_CERT_REQUIRE);

    /* Finally set credentials for this session */
    ret = gnutls_credentials_set(gssock->session, GNUTLS_CRD_CERTIFICATE, gssock->xcred);
    if (ret < 0)
        goto out;
...
static pj_ssl_cipher ssl_get_cipher(pj_ssl_sock_t *ssock)
{
    ...
    for (i = 0;; i++) {
        ...
        suite = gnutls_cipher_suite_info(i, (unsigned char *)id, NULL, &lookup, NULL, NULL);
        if (suite) {
            if (lookup == cipher) {
...
static void tls_cert_get_info(pj_pool_t *pool, pj_ssl_cert_info *ci, gnutls_x509_crt_t cert)
{
    ...
    update_needed = pj_strcmp2(&ci->issuer.info, buf) || pj_memcmp(ci->serial_no, serial_no, serialsize);
    if (!update_needed)
        return;
    ...
    if (ci->version >= 3) {
        ...
        for (i = 0; i < seq; i++) {
            ...
            case GNUTLS_SAN_IPADDRESS:
                ...
                pj_inet_ntop2(len == sizeof(pj_in6_addr) ? pj_AF_INET6() : pj_AF_INET(), out, buf, sizeof(buf));
                break;
            case GNUTLS_SAN_URI:
...
static void tls_cert_get_chain_raw(pj_pool_t *pool, pj_ssl_cert_info *ci, const gnutls_datum_t *certs, size_t certs_num)
{
    ...
    for (i = 0; i < certs_num; ++i) {
        const pj_str_t crt_raw = {(char *)certs[i].data, (pj_ssize_t)certs[i].size};
...
us_out:
    ...
    pj_pool_reset(ssock->info_pool);
    tls_cert_get_chain_raw(ssock->info_pool, &ssock->remote_cert_info, certs, certslen);
...
static void ssl_set_peer_name(pj_ssl_sock_t *ssock)
{
    ...
    if (ssock->param.server_name.slen && get_ip_addr_ver(&ssock->param.server_name) == 0) {
        ...
        ret = gnutls_server_name_set(gssock->session, GNUTLS_NAME_DNS, ssock->param.server_name.ptr,
                                     ssock->param.server_name.slen);
        if (ret < 0) {
            PJ_LOG(3, (ssock->pool->obj_name, "gnutls_server_name_set() failed: %s", gnutls_strerror(ret)));
        }
    }
}
...
static pj_status_t ssl_write(pj_ssl_sock_t *ssock, const void *data, pj_ssize_t size, int *nwritten)
{
    ...
    while (total_written < size) {
        ...
        nwritten_ = gnutls_record_send(gssock->session, ((read_data_t *)data) + total_written, size - total_written);
