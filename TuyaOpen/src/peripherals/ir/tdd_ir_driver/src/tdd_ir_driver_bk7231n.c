 ...
 * @brief Infrared driver implementation for BK7231N microcontroller platform.
 *
 * This file provides the hardware-specific implementation of the infrared
 * driver for the BK7231N microcontroller, which is commonly used in Tuya
 * IoT devices. The implementation handles low-level IR transmission and
 * reception operations, utilizing the platform's PWM, timer, and GPIO
 * capabilities to achieve precise infrared signal generation and capture.
 *
 * Platform-specific features:
 * - PWM-based IR carrier frequency generation (typically 38kHz)
 * - Timer-based precise timing control for IR protocol implementation
 * - GPIO interrupt handling for IR signal reception
 * - Hardware capture mode support for high-precision timing measurement
 * - Platform-specific timing corrections and optimizations
 * - Power management integration with low-power mode support
 * - Interrupt-driven reception with edge detection and timing measurement
 *
 * The driver implementation addresses BK7231N-specific characteristics
 * including timer accuracy limitations, interrupt latency considerations,
 * and hardware-specific timing adjustments to ensure reliable IR
 * communication performance across various operating conditions.
 *
...
// tbl_ir_cloud_service commit 09e62b3f17838d1e95db54f6fe2e572c84e27c70
// The inter-code delay between consecutive multi-code transmissions has been ensured, but this is kept here due to chip characteristics. Tests found that bk7231n has significant timer time deviation on the first timer, so a delay is added here.
// According to the actual chip platform conditions, this value can be appropriately reduced or removed.
#define IR_SEND_INTER_CODE_DELAY    (150 * 1000) // unit: us, default: 300ms
...
static BOOL_T is_ir_pre_start = FALSE;
/***********************************************************
***********************function define**********************
***********************************************************/
...
static int __tdd_ir_send_start(IR_DRV_INFO_T *drv_info)
{
    ...
    rt = tkl_timer_start(drv_info->hw_cfg.send_timer, drv_info->inter_code_delay);
    if (OPRT_OK != rt) {
        return rt;
    }
...
int tdd_ir_driver_register(char *dev_name, IR_DRIVER_TYPE_E driver_type, IR_DRV_CFG_T drv_cfg)
{
    ...
    drv_info->inter_code_delay = IR_SEND_INTER_CODE_DELAY;
