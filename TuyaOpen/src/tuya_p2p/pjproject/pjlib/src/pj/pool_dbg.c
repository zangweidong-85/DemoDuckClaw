...
#if ((defined(PJ_WIN32) && PJ_WIN32 != 0) || (defined(PJ_WIN64) && PJ_WIN64 != 0)) && defined(PJ_DEBUG) &&             \
    PJ_DEBUG != 0 && !PJ_NATIVE_STRING_IS_UNICODE
#include <windows.h>
...
PJ_DEF(void *) pj_pool_alloc_imp(const char *file, int line, pj_pool_t *pool, pj_size_t sz)
{
    struct pj_pool_mem *mem;
...
PJ_DEF(void *) pj_pool_calloc_imp(const char *file, int line, pj_pool_t *pool, unsigned cnt, unsigned elemsz)
{
    void *mem;
...
/* Allocate memory from the pool and zero the memory */
PJ_DEF(void *) pj_pool_zalloc_imp(const char *file, int line, pj_pool_t *pool, pj_size_t sz)
{
