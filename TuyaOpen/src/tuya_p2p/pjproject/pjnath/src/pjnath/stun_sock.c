...
static pj_status_t sess_on_send_msg(pj_stun_session *sess, void *token, const void *pkt, pj_size_t pkt_size,
                                    const pj_sockaddr_t *dst_addr, unsigned addr_len);

/* This callback is called by the STUN session when outgoing transaction
 * is complete
 */
static void sess_on_request_complete(pj_stun_session *sess, pj_status_t status, void *token, pj_stun_tx_data *tdata,
...
static pj_bool_t on_data_recvfrom(pj_activesock_t *asock, void *data, pj_size_t size, const pj_sockaddr_t *src_addr,
...
{
    const char *names[] = {"?", "DNS resolution", "STUN Binding request", "Keep-alive", "Mapped addr. changed"};
 ...
 */
PJ_DEF(pj_status_t)
pj_stun_sock_create(pj_stun_config *stun_cfg, const char *name, int af, const pj_stun_sock_cb *cb,
                    const pj_stun_sock_cfg *cfg, void *user_data, pj_stun_sock **p_stun_sock)
{
    pj_pool_t *pool;
    ...
    pj_grp_lock_add_handler(stun_sock->grp_lock, pool, stun_sock, &stun_sock_destructor);
    ...
    status = pj_sock_apply_qos2(stun_sock->sock_fd, cfg->qos_type, &cfg->qos_params, 2, stun_sock->obj_name, NULL);
    ...
    if (cfg->so_rcvbuf_size > 0) {
        ...
        status = pj_sock_setsockopt_sobuf(stun_sock->sock_fd, pj_SO_RCVBUF(), PJ_TRUE, &sobuf_size);
        if (status != PJ_SUCCESS) {
    ...
    if (cfg->so_sndbuf_size > 0) {
        ...
        status = pj_sock_setsockopt_sobuf(stun_sock->sock_fd, pj_SO_SNDBUF(), PJ_TRUE, &sobuf_size);
        if (status != PJ_SUCCESS) {
    ...
    if (cfg->bound_addr.addr.sa_family == pj_AF_INET() || cfg->bound_addr.addr.sa_family == pj_AF_INET6()) {
        pj_sockaddr_cp(&bound_addr, &cfg->bound_addr);
    }
    status = pj_sock_bind_random(stun_sock->sock_fd, &bound_addr, cfg->port_range, max_bind_retry);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    /* Init active socket configuration */
    {
        ...
        status = pj_activesock_create(pool, stun_sock->sock_fd, pj_SOCK_DGRAM(), &activesock_cfg, stun_cfg->ioqueue,
                                      &activesock_cb, stun_sock, &stun_sock->active_sock);
        if (status != PJ_SUCCESS)
            goto on_error;

        /* Start asynchronous read operations */
        status = pj_activesock_start_recvfrom(stun_sock->active_sock, pool, cfg->max_pkt_size, 0);
        if (status != PJ_SUCCESS)
            goto on_error;

        /* Init send keys */
        pj_ioqueue_op_key_init(&stun_sock->send_key, sizeof(stun_sock->send_key));
        pj_ioqueue_op_key_init(&stun_sock->int_send_key, sizeof(stun_sock->int_send_key));
    }
    ...
    /* Create STUN session */
    {
        ...
        status = pj_stun_session_create(&stun_sock->stun_cfg, stun_sock->obj_name, &sess_cb, PJ_FALSE,
...
/* Start socket. */
PJ_DEF(pj_status_t)
pj_stun_sock_start(pj_stun_sock *stun_sock, const pj_str_t *domain, pj_uint16_t default_port, pj_dns_resolver *resolver)
{
    pj_status_t status;
    ...
    status = pj_inet_pton(stun_sock->af, domain, pj_sockaddr_get_addr(&stun_sock->srv_addr));
    if (status != PJ_SUCCESS) {
    ...
    if (status != PJ_SUCCESS && resolver) {
        ...
        status = pj_dns_srv_resolve(domain, &res_name, default_port, stun_sock->pool, resolver, opt, stun_sock,
                                    &dns_srv_resolver_cb, &stun_sock->q);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (stun_sock->obj_name, status, "Failed in pj_dns_srv_resolve()"));
        } else {
            ...
            if (stun_sock->last_err != PJ_SUCCESS) {
                PJ_PERROR(4, (stun_sock->obj_name, status, "Failed in sending Binding request (2)"));
            }
        }
        ...
        if (status != PJ_SUCCESS) {
            ...
            if (status != PJ_SUCCESS) {
                PJ_PERROR(4, (stun_sock->obj_name, status, "Failed in pj_getaddrinfo()"));
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (stun_sock->obj_name, status, "Failed in sending Binding request"));
        }
    }
...
{
    TRACE_(
        (stun_sock->obj_name, "STUN sock %p request, ref_cnt=%d", stun_sock, pj_grp_lock_get_ref(stun_sock->grp_lock)));
    ...
    pj_timer_heap_cancel_if_active(stun_sock->stun_cfg.timer_heap, &stun_sock->ka_timer, 0);
...
/* Associate user data */
PJ_DEF(pj_status_t) pj_stun_sock_set_user_data(pj_stun_sock *stun_sock, void *user_data)
{
...
static pj_bool_t sess_fail(pj_stun_sock *stun_sock, pj_stun_sock_op op, pj_status_t status)
{
    pj_bool_t ret;

    PJ_PERROR(4, (stun_sock->obj_name, status, "Session failed because %s failed", pj_stun_sock_op_name(op)));
...
static void dns_srv_resolver_cb(void *user_data, pj_status_t status, const pj_dns_srv_record *rec)
{
    ...
    /* Set the address */
    pj_sockaddr_init(stun_sock->af, &stun_sock->srv_addr, NULL, rec->entry[0].port);
    if (stun_sock->af == pj_AF_INET6()) {
...
static pj_status_t get_mapped_addr(pj_stun_sock *stun_sock)
{
    ...
    ++stun_sock->tsx_id[5];
    status = pj_stun_session_create_req(stun_sock->stun_sess, PJ_STUN_BINDING_REQUEST, PJ_STUN_MAGIC,
                                        (const pj_uint8_t *)stun_sock->tsx_id, &tdata);
    if (status != PJ_SUCCESS)
        goto on_error;

    /* Send request */
    status = pj_stun_session_send_msg(stun_sock->stun_sess, INTERNAL_MSG_TOKEN, PJ_FALSE, PJ_TRUE, &stun_sock->srv_addr,
...
/* Get info */
PJ_DEF(pj_status_t) pj_stun_sock_get_info(pj_stun_sock *stun_sock, pj_stun_sock_info *info)
{
    int addr_len;
    pj_status_t status;
    ...
    status = pj_sock_getsockname(stun_sock->sock_fd, &info->bound_addr, &addr_len);
    if (status != PJ_SUCCESS) {
        pj_grp_lock_release(stun_sock->grp_lock);
        return status;
    }
    ...
    if (pj_sockaddr_has_addr(&info->bound_addr)) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (stun_sock->obj_name, status, "Failed in getting default address for STUN info"));
            pj_grp_lock_release(stun_sock->grp_lock);
            return status;
        }
        ...
        status = pj_enum_ip_interface2(&enum_opt, &info->alias_cnt, info->aliases);
...
PJ_DEF(pj_status_t)
pj_stun_sock_sendto(pj_stun_sock *stun_sock, pj_ioqueue_op_key_t *send_key, const void *pkt, unsigned pkt_len,
                    unsigned flag, const pj_sockaddr_t *dst_addr, unsigned addr_len)
{
    pj_ssize_t size;
    pj_status_t status;
    ...
    size = pkt_len;
    status = pj_activesock_sendto(stun_sock->active_sock, send_key, pkt, &size, flag, dst_addr, addr_len);
                                    ...
                                    const pj_sockaddr_t *dst_addr, unsigned addr_len)
{
    ...
    size = pkt_size;
    return pj_activesock_sendto(stun_sock->active_sock, &stun_sock->int_send_key, pkt, &size, 0, dst_addr, addr_len);
}
                                     ...
                                     const pj_stun_msg *response, const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    ...
    if (mapped_changed) {
        /* Print mapped adress */
        {
            ...
            PJ_LOG(4, (stun_sock->obj_name, "STUN mapped address found/changed: %s",
                       pj_sockaddr_print(&mapped_attr->sockaddr, addrinfo, sizeof(addrinfo), 3)));
        }
...
static void start_ka_timer(pj_stun_sock *stun_sock)
{
    ...
    if (stun_sock->ka_interval > 0 && !stun_sock->is_destroying) {
        ...
        pj_timer_heap_schedule_w_grp_lock(stun_sock->stun_cfg.timer_heap, &stun_sock->ka_timer, &delay, PJ_TRUE,
                                          stun_sock->grp_lock);
    }
}
                                  ...
                                  int addr_len, pj_status_t status)
{
    ...
    status = pj_stun_msg_check((const pj_uint8_t *)data, size, PJ_STUN_IS_DATAGRAM | PJ_STUN_CHECK_PACKET);
    if (status != PJ_SUCCESS) {
     ...
     */
    if (!PJ_STUN_IS_RESPONSE(type) || PJ_STUN_GET_METHOD(type) != PJ_STUN_BINDING_METHOD ||
    ...
    status = pj_stun_session_on_rx_pkt(stun_sock->stun_sess, data, size, PJ_STUN_IS_DATAGRAM, NULL, NULL, src_addr,
                                       addr_len);
...
process_app_data:
    if (stun_sock->cb.on_rx_data) {
        (*stun_sock->cb.on_rx_data)(stun_sock, data, (unsigned)size, src_addr, addr_len);
        status = pj_grp_lock_release(stun_sock->grp_lock);
        return status != PJ_EGONE ? PJ_TRUE : PJ_FALSE;
    }
