...
static void stun_tsx_on_complete(pj_stun_client_tsx *tsx, pj_status_t status, const pj_stun_msg *response,
                                 const pj_sockaddr_t *src_addr, unsigned src_addr_len);
static pj_status_t stun_tsx_on_send_msg(pj_stun_client_tsx *tsx, const void *stun_pkt, pj_size_t pkt_size);
...
static pj_stun_tsx_cb tsx_cb = {&stun_tsx_on_complete, &stun_tsx_on_send_msg, &stun_tsx_on_destroy};
...
static pj_stun_tx_data *tsx_lookup(pj_stun_session *sess, const pj_stun_msg *msg)
{
    ...
    while (tdata != &sess->pending_request_list) {
        ...
        if (tdata->msg_magic == msg->hdr.magic &&
            pj_memcmp(tdata->msg_key, msg->hdr.tsx_id, sizeof(msg->hdr.tsx_id)) == 0) {
            return tdata;
        }
        tdata = tdata->next;
    }
...
static pj_status_t create_tdata(pj_stun_session *sess, pj_stun_tx_data **p_tdata)
{
    pj_pool_t *pool;
    pj_stun_tx_data *tdata;

    /* Create pool and initialize basic tdata attributes */
    pool = pj_pool_create(sess->cfg->pf, "tdata%p", TDATA_POOL_SIZE, TDATA_POOL_INC, NULL);
...
static void destroy_tdata(pj_stun_tx_data *tdata, pj_bool_t force)
{
    TRACE_((THIS_FILE, "tdata %p destroy request, force=%d, tsx=%p, destroying=%d", tdata, force, tdata->client_tsx,
            tdata->is_destroying));
    ...
    if (tdata->res_timer.id != PJ_FALSE) {
        pj_timer_heap_cancel_if_active(tdata->sess->cfg->timer_heap, &tdata->res_timer, PJ_FALSE);
    }
 ...
 */
PJ_DEF(void) pj_stun_msg_destroy_tdata(pj_stun_session *sess, pj_stun_tx_data *tdata)
{
    PJ_UNUSED_ARG(sess);
...
static void on_cache_timeout(pj_timer_heap_t *timer_heap, struct pj_timer_entry *entry)
{
    pj_stun_tx_data *tdata;
...
static pj_status_t apply_msg_options(pj_stun_session *sess, pj_pool_t *pool, const pj_stun_req_cred_info *auth_info,
                                     pj_stun_msg *msg)
{
     ...
     * responses.
     */
    if (sess->srv_name.slen && pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_SOFTWARE, 0) == NULL &&
        (PJ_STUN_IS_RESPONSE(msg->hdr.type) ||
         (PJ_STUN_IS_REQUEST(msg->hdr.type) && msg->hdr.magic == PJ_STUN_MAGIC))) {
        pj_stun_msg_add_string_attr(pool, msg, PJ_STUN_ATTR_SOFTWARE, &sess->srv_name);
    }
    ...
    if (username.slen && PJ_STUN_IS_REQUEST(msg->hdr.type)) {
        status = pj_stun_msg_add_string_attr(pool, msg, PJ_STUN_ATTR_USERNAME, &username);
        PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    }

    /* Add REALM only when long term credential is used */
    if (realm.slen && PJ_STUN_IS_REQUEST(msg->hdr.type)) {
        status = pj_stun_msg_add_string_attr(pool, msg, PJ_STUN_ATTR_REALM, &realm);
        PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    }

    /* Add NONCE when desired */
    if (nonce.slen && (PJ_STUN_IS_REQUEST(msg->hdr.type) || PJ_STUN_IS_ERROR_RESPONSE(msg->hdr.type))) {
        status = pj_stun_msg_add_string_attr(pool, msg, PJ_STUN_ATTR_NONCE, &nonce);
    }
...
static pj_status_t handle_auth_challenge(pj_stun_session *sess, const pj_stun_tx_data *request,
                                         const pj_stun_msg *response, const pj_sockaddr_t *src_addr,
                                         unsigned src_addr_len, pj_bool_t *notify_user)
{
    ...
    *notify_user = PJ_TRUE;
    ...
    ea = (const pj_stun_errcode_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_ERROR_CODE, 0);
    if (!ea) {
    ...
    if (ea->err_code == PJ_STUN_SC_UNAUTHORIZED || ea->err_code == PJ_STUN_SC_STALE_NONCE) {
        ...
        anonce = (const pj_stun_nonce_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_NONCE, 0);
        if (!anonce) {
        ...
        arealm = (pj_stun_realm_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_REALM, 0);
        if (arealm) {
            pj_strdup(sess->pool, &sess->server_realm, &arealm->value);
            while (sess->server_realm.slen && !sess->server_realm.ptr[sess->server_realm.slen - 1]) {
                --sess->server_realm.slen;
            }
        }

        /* Create new request */
        status = pj_stun_session_create_req(sess, request->msg->hdr.type, request->msg->hdr.magic, NULL, &tdata);
        ...
        for (i = 0; i < request->msg->attr_count; ++i) {
            ...
            if (asrc->type == PJ_STUN_ATTR_USERNAME || asrc->type == PJ_STUN_ATTR_REALM ||
                asrc->type == PJ_STUN_ATTR_MESSAGE_INTEGRITY || asrc->type == PJ_STUN_ATTR_NONCE) {
                continue;
            }

            tdata->msg->attr[tdata->msg->attr_count++] = pj_stun_attr_clone(tdata->pool, asrc);
        }
        ...
        *notify_user = PJ_FALSE;
        ...
        /* Retry the request */
        status =
            pj_stun_session_send_msg(sess, request->token, PJ_TRUE, request->retransmit, src_addr, src_addr_len, tdata);
                                 ...
                                 const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    ...
    handle_auth_challenge(sess, tdata, response, src_addr, src_addr_len, &notify_user);

    if (notify_user && sess->cb.on_request_complete) {
        (*sess->cb.on_request_complete)(sess, status, tdata->token, tdata, response, src_addr, src_addr_len);
    }
...
static pj_status_t stun_tsx_on_send_msg(pj_stun_client_tsx *tsx, const void *stun_pkt, pj_size_t pkt_size)
{
    ...
    status = sess->cb.on_send_msg(tdata->sess, tdata->token, stun_pkt, pkt_size, tdata->dst_addr, tdata->addr_len);
...
PJ_DEF(pj_status_t)
pj_stun_session_create(pj_stun_config *cfg, const char *name, const pj_stun_session_cb *cb, pj_bool_t fingerprint,
                       pj_grp_lock_t *grp_lock, pj_stun_session **p_sess)
{
    ...
    pool = pj_pool_create(cfg->pf, name, PJNATH_POOL_LEN_STUN_SESS, PJNATH_POOL_INC_STUN_SESS, NULL);
    PJ_ASSERT_RETURN(pool, PJ_ENOMEM);
    ...
    sess->rx_pool = pj_pool_create(sess->cfg->pf, name, PJNATH_POOL_LEN_STUN_TDATA, PJNATH_POOL_INC_STUN_TDATA, NULL);
...
PJ_DEF(pj_status_t) pj_stun_session_destroy(pj_stun_session *sess)
{
    ...
    TRACE_((SNAME(sess), "STUN session %p destroy request, ref_cnt=%d", sess, pj_grp_lock_get_ref(sess->grp_lock)));
...
PJ_DEF(pj_status_t) pj_stun_session_set_user_data(pj_stun_session *sess, void *user_data)
{
    PJ_ASSERT_RETURN(sess, PJ_EINVAL);
    pj_grp_lock_acquire(sess->grp_lock);
...
PJ_DEF(pj_status_t) pj_stun_session_set_software_name(pj_stun_session *sess, const pj_str_t *sw)
{
    PJ_ASSERT_RETURN(sess, PJ_EINVAL);
    pj_grp_lock_acquire(sess->grp_lock);
    if (sw && sw->slen)
...
PJ_DEF(pj_status_t)
pj_stun_session_set_credential(pj_stun_session *sess, pj_stun_auth_type auth_type, const pj_stun_auth_cred *cred)
{
    PJ_ASSERT_RETURN(sess, PJ_EINVAL);
...
PJ_DEF(pj_bool_t) pj_stun_session_use_fingerprint(pj_stun_session *sess, pj_bool_t use)
{
    pj_bool_t old_use;
...
static pj_status_t get_auth(pj_stun_session *sess, pj_stun_tx_data *tdata)
{
    if (sess->cred.type == PJ_STUN_AUTH_CRED_STATIC) {
        ...
        pj_stun_create_key(tdata->pool, &tdata->auth_info.auth_key, &tdata->auth_info.realm, &tdata->auth_info.username,
                           sess->cred.data.static_cred.data_type, &sess->cred.data.static_cred.data);
        ...
        rc = (*sess->cred.data.dyn_cred.get_cred)(tdata->msg, user_data, tdata->pool, &tdata->auth_info.realm,
...
PJ_DEF(pj_status_t)
pj_stun_session_create_req(pj_stun_session *sess, int method, pj_uint32_t magic, const pj_uint8_t tsx_id[12],
...
PJ_DEF(pj_status_t) pj_stun_session_create_ind(pj_stun_session *sess, int msg_type, pj_stun_tx_data **p_tdata)
{
    pj_stun_tx_data *tdata = NULL;
    pj_status_t status;
    ...
    status = pj_stun_msg_create(tdata->pool, msg_type, PJ_STUN_MAGIC, NULL, &tdata->msg);
    if (status != PJ_SUCCESS) {
        pj_pool_safe_release(&tdata->pool);
        pj_grp_lock_release(sess->grp_lock);
        return status;
 ...
 */
PJ_DEF(pj_status_t)
pj_stun_session_create_res(pj_stun_session *sess, const pj_stun_rx_data *rdata, unsigned err_code,
                           const pj_str_t *err_msg, pj_stun_tx_data **p_tdata)
{
    pj_status_t status;
    pj_stun_tx_data *tdata = NULL;
    ...
    status = pj_stun_msg_create_response(tdata->pool, rdata->msg, err_code, err_msg, &tdata->msg);
    if (status != PJ_SUCCESS) {
        pj_pool_safe_release(&tdata->pool);
        pj_grp_lock_release(sess->grp_lock);
        return status;
    ...
    pj_memcpy(tdata->msg_key, rdata->msg->hdr.tsx_id, sizeof(rdata->msg->hdr.tsx_id));
...
static void dump_tx_msg(pj_stun_session *sess, const pj_stun_msg *msg, unsigned pkt_size, const pj_sockaddr_t *addr)
{
    char dst_name[PJ_INET6_ADDRSTRLEN + 10];

    if ((PJ_STUN_IS_REQUEST(msg->hdr.type) && (sess->log_flag & PJ_STUN_SESS_LOG_TX_REQ) == 0) ||
        (PJ_STUN_IS_RESPONSE(msg->hdr.type) && (sess->log_flag & PJ_STUN_SESS_LOG_TX_RES) == 0) ||
        (PJ_STUN_IS_INDICATION(msg->hdr.type) && (sess->log_flag & PJ_STUN_SESS_LOG_TX_IND) == 0)) {
        return;
    }
    ...
    PJ_LOG(5, (SNAME(sess),
               ...
               "%s"
               "--- end of STUN message ---\n",
               pkt_size, dst_name, pj_stun_msg_dump(msg, sess->dump_buf, sizeof(sess->dump_buf), NULL)));
}

PJ_DEF(pj_status_t)
pj_stun_session_send_msg(pj_stun_session *sess, void *token, pj_bool_t cache_res, pj_bool_t retransmit,
                         const pj_sockaddr_t *server, unsigned addr_len, pj_stun_tx_data *tdata)
{
    pj_status_t status;
    ...
    /* Apply options */
    status = apply_msg_options(sess, tdata->pool, &tdata->auth_info, tdata->msg);
    if (status != PJ_SUCCESS) {
        pj_stun_msg_destroy_tdata(sess, tdata);
    ...
    /* Encode message */
    status = pj_stun_msg_encode(tdata->msg, (pj_uint8_t *)tdata->pkt, tdata->max_len, 0, &tdata->auth_info.auth_key,
                                &tdata->pkt_size);
    if (status != PJ_SUCCESS) {
        pj_stun_msg_destroy_tdata(sess, tdata);
    ...
    if (PJ_STUN_IS_REQUEST(tdata->msg->hdr.type)) {
        ...
        status = pj_stun_client_tsx_create(sess->cfg, tdata->pool, sess->grp_lock, &tsx_cb, &tdata->client_tsx);
        PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
        ...
        /* Send the request! */
        status = pj_stun_client_tsx_send_msg(tdata->client_tsx, retransmit, tdata->pkt, (unsigned)tdata->pkt_size);
        ...
        if (cache_res &&
            (PJ_STUN_IS_SUCCESS_RESPONSE(tdata->msg->hdr.type) || PJ_STUN_IS_ERROR_RESPONSE(tdata->msg->hdr.type))) {
            ...
            pj_grp_lock_add_handler(tdata->grp_lock, tdata->pool, tdata, &tdata_on_destroy);
            ...
            pj_timer_entry_init(&tdata->res_timer, PJ_FALSE, tdata, &on_cache_timeout);

            timeout.sec = sess->cfg->res_cache_msec / 1000;
            timeout.msec = sess->cfg->res_cache_msec % 1000;

            status = pj_timer_heap_schedule_w_grp_lock(sess->cfg->timer_heap, &tdata->res_timer, &timeout, PJ_TRUE,
                                                       tdata->grp_lock);
            if (status != PJ_SUCCESS) {
                pj_stun_msg_destroy_tdata(sess, tdata);
        ...
        status = sess->cb.on_send_msg(sess, token, tdata->pkt, tdata->pkt_size, server, addr_len);
 ...
 */
PJ_DEF(pj_status_t)
pj_stun_session_respond(pj_stun_session *sess, const pj_stun_rx_data *rdata, unsigned code, const char *errmsg,
                        void *token, pj_bool_t cache, const pj_sockaddr_t *dst_addr, unsigned addr_len)
{
    pj_status_t status;
    pj_str_t reason;
    pj_stun_tx_data *tdata;
    ...
    status = pj_stun_session_create_res(sess, rdata, code, (errmsg ? pj_cstr(&reason, errmsg) : NULL), &tdata);
    if (status != PJ_SUCCESS) {
        pj_grp_lock_release(sess->grp_lock);
        return status;
    }

    status = pj_stun_session_send_msg(sess, token, cache, PJ_FALSE, dst_addr, addr_len, tdata);
 ...
 */
PJ_DEF(pj_status_t)
pj_stun_session_cancel_req(pj_stun_session *sess, pj_stun_tx_data *tdata, pj_bool_t notify, pj_status_t notify_status)
{
    ...
    if (notify) {
        (sess->cb.on_request_complete)(sess, notify_status, tdata->token, tdata, NULL, NULL, 0);
    }
 ...
 */
PJ_DEF(pj_status_t) pj_stun_session_retransmit_req(pj_stun_session *sess, pj_stun_tx_data *tdata, pj_bool_t mod_count)
{
    pj_status_t status;
...
/* Send response */
static pj_status_t send_response(pj_stun_session *sess, void *token, pj_pool_t *pool, pj_stun_msg *response,
                                 const pj_stun_req_cred_info *auth_info, pj_bool_t retransmission,
                                 const pj_sockaddr_t *addr, unsigned addr_len)
{
    ...
    /* Encode */
    status = pj_stun_msg_encode(response, out_pkt, out_max_len, 0, &auth_info->auth_key, &out_len);
    if (status != PJ_SUCCESS) {
    ...
    /* Send packet */
    status = sess->cb.on_send_msg(sess, token, out_pkt, (unsigned)out_len, addr, addr_len);

    return status;
}

/* Authenticate incoming message */
static pj_status_t authenticate_req(pj_stun_session *sess, void *token, const pj_uint8_t *pkt, unsigned pkt_len,
                                    pj_stun_rx_data *rdata, pj_pool_t *tmp_pool, const pj_sockaddr_t *src_addr,
                                    unsigned src_addr_len)
{
    pj_stun_msg *response;
    pj_status_t status;

    if (PJ_STUN_IS_ERROR_RESPONSE(rdata->msg->hdr.type) || sess->auth_type == PJ_STUN_AUTH_NONE) {
        return PJ_SUCCESS;
    }

    status = pj_stun_authenticate_request(pkt, pkt_len, rdata->msg, &sess->cred, tmp_pool, &rdata->info, &response);
    if (status != PJ_SUCCESS && response != NULL) {
        PJ_PERROR(5, (SNAME(sess), status, "Message authentication failed"));
        send_response(sess, token, tmp_pool, response, &rdata->info, PJ_FALSE, src_addr, src_addr_len);
    }
...
static pj_status_t on_incoming_response(pj_stun_session *sess, unsigned options, const pj_uint8_t *pkt,
                                        unsigned pkt_len, pj_stun_msg *msg, const pj_sockaddr_t *src_addr,
                                        unsigned src_addr_len)
{
    pj_stun_tx_data *tdata;
    pj_status_t status;
    ...
    if (tdata == NULL) {
        PJ_LOG(5, (SNAME(sess), "Transaction not found, response silently discarded"));
        return PJ_SUCCESS;
    }
     ...
     */
    if ((options & PJ_STUN_NO_AUTHENTICATE) == 0 && tdata->auth_info.auth_key.slen != 0 &&
        pj_stun_auth_valid_for_msg(msg)) {
        status = pj_stun_authenticate_response(pkt, pkt_len, msg, &tdata->auth_info.auth_key);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(5, (SNAME(sess), status, "Response authentication failed"));
            return status;
        }
    }
     ...
     */
    status = pj_stun_client_tsx_on_rx_msg(tdata->client_tsx, msg, src_addr, src_addr_len);
    if (status != PJ_SUCCESS) {
        return status;
    }
...
static pj_status_t check_cached_response(pj_stun_session *sess, pj_pool_t *tmp_pool, const pj_stun_msg *msg,
                                         const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    pj_stun_tx_data *t;
    ...
    while (t != &sess->cached_response_list) {
        if (t->msg_magic == msg->hdr.magic && t->msg->hdr.type == msg->hdr.type &&
            pj_memcmp(t->msg_key, msg->hdr.tsx_id, sizeof(msg->hdr.tsx_id)) == 0) {
            break;
        }
        t = t->next;
    }
    ...
    if (t != &sess->cached_response_list) {
        ...
        PJ_LOG(5, (SNAME(sess), "Request retransmission, sending cached response"));

        send_response(sess, t->token, tmp_pool, t->msg, &t->auth_info, PJ_TRUE, src_addr, src_addr_len);
        return PJ_SUCCESS;
    }
...
static pj_status_t on_incoming_request(pj_stun_session *sess, unsigned options, void *token, pj_pool_t *tmp_pool,
                                       const pj_uint8_t *in_pkt, unsigned in_pkt_len, pj_stun_msg *msg,
                                       const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    ...
    if ((options & PJ_STUN_NO_AUTHENTICATE) == 0) {
        status = authenticate_req(sess, token, (const pj_uint8_t *)in_pkt, in_pkt_len, &rdata, tmp_pool, src_addr,
                                  src_addr_len);
        if (status != PJ_SUCCESS) {
            return status;
        }
    ...
    if (sess->cb.on_rx_request) {
        status = (*sess->cb.on_rx_request)(sess, in_pkt, in_pkt_len, &rdata, token, src_addr, src_addr_len);
    } else {
        pj_str_t err_text;
        pj_stun_msg *response;

        err_text = pj_str("Callback is not set to handle request");
        status = pj_stun_msg_create_response(tmp_pool, msg, PJ_STUN_SC_BAD_REQUEST, &err_text, &response);
        if (status == PJ_SUCCESS && response) {
            status = send_response(sess, token, tmp_pool, response, NULL, PJ_FALSE, src_addr, src_addr_len);
        }
    }
...
static pj_status_t on_incoming_indication(pj_stun_session *sess, void *token, pj_pool_t *tmp_pool,
                                          ...
                                          const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    ...
    if (sess->cb.on_rx_indication) {
        return (*sess->cb.on_rx_indication)(sess, in_pkt, in_pkt_len, msg, token, src_addr, src_addr_len);
    } else {
        return PJ_SUCCESS;
    }
}
...
/* Print outgoing message to log */
static void dump_rx_msg(pj_stun_session *sess, const pj_stun_msg *msg, unsigned pkt_size, const pj_sockaddr_t *addr)
{
    char src_info[PJ_INET6_ADDRSTRLEN + 10];

    if ((PJ_STUN_IS_REQUEST(msg->hdr.type) && (sess->log_flag & PJ_STUN_SESS_LOG_RX_REQ) == 0) ||
        (PJ_STUN_IS_RESPONSE(msg->hdr.type) && (sess->log_flag & PJ_STUN_SESS_LOG_RX_RES) == 0) ||
        (PJ_STUN_IS_INDICATION(msg->hdr.type) && (sess->log_flag & PJ_STUN_SESS_LOG_RX_IND) == 0)) {
        return;
    }
    ...
    PJ_LOG(5, (SNAME(sess),
               ...
               "%s"
               "--- end of STUN message ---\n",
               pkt_size, src_info, pj_stun_msg_dump(msg, sess->dump_buf, sizeof(sess->dump_buf), NULL)));
}

/* Incoming packet */
PJ_DEF(pj_status_t)
pj_stun_session_on_rx_pkt(pj_stun_session *sess, const void *packet, pj_size_t pkt_size, unsigned options, void *token,
                          pj_size_t *parsed_len, const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    ...
    status =
        pj_stun_msg_decode(sess->rx_pool, (const pj_uint8_t *)packet, pkt_size, options, &msg, parsed_len, &response);
    if (status != PJ_SUCCESS) {
        LOG_ERR_(sess, "STUN msg_decode() error", status);
        if (response) {
            send_response(sess, token, sess->rx_pool, response, NULL, PJ_FALSE, src_addr, src_addr_len);
        }
        goto on_return;
    }
    ...
    status = check_cached_response(sess, sess->rx_pool, msg, src_addr, src_addr_len);
    if (status == PJ_SUCCESS) {
        goto on_return;
    }
    ...
    if (PJ_STUN_IS_SUCCESS_RESPONSE(msg->hdr.type) || PJ_STUN_IS_ERROR_RESPONSE(msg->hdr.type)) {
        status = on_incoming_response(sess, options, (const pj_uint8_t *)packet, (unsigned)pkt_size, msg, src_addr,
                                      src_addr_len);

    } else if (PJ_STUN_IS_REQUEST(msg->hdr.type)) {

        status = on_incoming_request(sess, options, token, sess->rx_pool, (const pj_uint8_t *)packet,
                                     (unsigned)pkt_size, msg, src_addr, src_addr_len);

    } else if (PJ_STUN_IS_INDICATION(msg->hdr.type)) {

        status = on_incoming_indication(sess, token, sess->rx_pool, (const pj_uint8_t *)packet, (unsigned)pkt_size, msg,
