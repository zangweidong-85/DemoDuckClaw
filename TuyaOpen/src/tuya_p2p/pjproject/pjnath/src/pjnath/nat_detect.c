...
static const char *nat_type_names[] = {"Unknown",   "ErrUnknown", "Open",       "Blocked",        "Symmetric UDP",
...
enum test_type { ST_TEST_1, ST_TEST_2, ST_TEST_3, ST_TEST_1B, ST_MAX };

static const char *test_names[] = {
    "Test I: Binding request", "Test II: Binding request with change address and port request",
    "Test III: Binding request with change port request", "Test IB: Binding request to alternate address"};

enum timer_type { TIMER_TEST = 1, TIMER_DESTROY = 2 };
...
static void on_request_complete(pj_stun_session *sess, pj_status_t status, void *token, pj_stun_tx_data *tdata,
                                const pj_stun_msg *response, const pj_sockaddr_t *src_addr, unsigned src_addr_len);
static pj_status_t on_send_msg(pj_stun_session *sess, void *token, const void *pkt, pj_size_t pkt_size,
                               const pj_sockaddr_t *dst_addr, unsigned addr_len);

static pj_status_t send_test(nat_detect_session *sess, enum test_type test_id, const pj_sockaddr *alt_addr,
...
{
    PJ_ASSERT_RETURN(type >= 0 && type <= PJ_STUN_NAT_TYPE_PORT_RESTRICTED, "*Invalid*");
...
static pj_status_t get_local_interface(const pj_sockaddr *server, pj_sockaddr *local_addr)
{
    pj_sock_t sock;
...
PJ_DEF(pj_status_t)
pj_stun_detect_nat_type(const pj_sockaddr_in *server, pj_stun_config *stun_cfg, void *user_data,
                        pj_stun_nat_detect_cb *cb)
{
    pj_sockaddr srv;
...
PJ_DEF(pj_status_t)
pj_stun_detect_nat_type2(const pj_sockaddr *server, pj_stun_config *stun_cfg, void *user_data,
                         pj_stun_nat_detect_cb *cb)
{
    pj_pool_t *pool;
    ...
    PJ_LOG(5, (sess->pool->obj_name, "Local address is %s:%d",
               pj_sockaddr_print(&sess->local_addr, addr, sizeof(addr), 2), pj_sockaddr_get_port(&sess->local_addr)));

    PJ_LOG(5, (sess->pool->obj_name, "Server set to %s:%d", pj_sockaddr_print(server, addr, sizeof(addr), 2),
    ...
    status = pj_ioqueue_register_sock2(sess->pool, stun_cfg->ioqueue, sess->sock, sess->grp_lock, sess, &ioqueue_cb,
                                       &sess->key);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    status = pj_stun_session_create(stun_cfg, pool->obj_name, &sess_cb, PJ_FALSE, sess->grp_lock, &sess->stun_sess);
    if (status != PJ_SUCCESS)
        goto on_error;
...
static void end_session(nat_detect_session *sess, pj_status_t status, pj_stun_nat_type nat_type)
{
...
static void on_read_complete(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, pj_ssize_t bytes_read)
{
    ...
    if (bytes_read < 0) {
        if (-bytes_read != PJ_STATUS_FROM_OS(OSERR_EWOULDBLOCK) &&
            -bytes_read != PJ_STATUS_FROM_OS(OSERR_EINPROGRESS) && -bytes_read != PJ_STATUS_FROM_OS(OSERR_ECONNRESET)) {
            /* Permanent error */
        ...
        pj_stun_session_on_rx_pkt(sess->stun_sess, sess->rx_pkt, bytes_read, PJ_STUN_IS_DATAGRAM | PJ_STUN_CHECK_PACKET,
                                  NULL, NULL, &sess->src_addr, sess->src_addr_len);
    }

    sess->rx_pkt_len = sizeof(sess->rx_pkt);
    sess->src_addr_len = sizeof(sess->src_addr);
    status = pj_ioqueue_recvfrom(key, op_key, sess->rx_pkt, &sess->rx_pkt_len, PJ_IOQUEUE_ALWAYS_ASYNC, &sess->src_addr,
                                 &sess->src_addr_len);
 ...
 */
static pj_status_t on_send_msg(pj_stun_session *stun_sess, void *token, const void *pkt, pj_size_t pkt_size,
                               const pj_sockaddr_t *dst_addr, unsigned addr_len)
{
    nat_detect_session *sess;
    pj_ssize_t pkt_len;
    ...
    pkt_len = pkt_size;
    status = pj_ioqueue_sendto(sess->key, &sess->write_op, pkt, &pkt_len, 0, dst_addr, addr_len);
 ...
 */
static void on_request_complete(pj_stun_session *stun_sess, pj_status_t status, void *token, pj_stun_tx_data *tdata,
                                const pj_stun_msg *response, const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    nat_detect_session *sess;
    ...
    if (status == PJ_SUCCESS) {
        ...
        if (PJ_STUN_IS_ERROR_RESPONSE(response->hdr.type)) {
            ...
            eattr = (pj_stun_errcode_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_ERROR_CODE, 0);
            ...
            mattr = (pj_stun_sockaddr_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_XOR_MAPPED_ADDR, 0);
            if (mattr == NULL) {
            ...
            ca = (pj_stun_changed_addr_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_CHANGED_ADDR, 0);
    ...
    PJ_PERROR(5, (sess->pool->obj_name, status, "Completed %s", test_names[test_id]));
     ...
     */
    if (!sess->result[ST_TEST_1B].executed && sess->result[ST_TEST_2].complete &&
        sess->result[ST_TEST_2].status != PJ_SUCCESS && sess->result[ST_TEST_1].complete &&
        ...
        if (cmp == 0) {
            ...
            case PJNATH_ESTUNTIMEDOUT:
                ...
                case PJ_SUCCESS:
                     ...
                     */
                    cmp = pj_sockaddr_cmp(&sess->result[ST_TEST_1].ma, &sess->result[ST_TEST_1B].ma);
                    if (cmp != 0) {
                        /*
                        ...
                        default:
                             ...
                             */
                            end_session(sess, sess->result[ST_TEST_3].status, PJ_STUN_NAT_TYPE_ERR_UNKNOWN);
                            break;
                        }
                    }
                    break;
                             ...
                             pj_uint32_t change_flag)
{
    ...
    /* Create BIND request */
    status = pj_stun_session_create_req(sess->stun_sess, PJ_STUN_BINDING_REQUEST, magic, (pj_uint8_t *)tsx_id,
                                        &sess->result[test_id].tdata);
    if (status != PJ_SUCCESS)
        goto on_error;

    /* Add CHANGE-REQUEST attribute */
    status = pj_stun_msg_add_uint_attr(sess->pool, sess->result[test_id].tdata->msg, PJ_STUN_ATTR_CHANGE_REQUEST,
                                       change_flag);
    if (status != PJ_SUCCESS)
        goto on_error;
    ...
    if (alt_addr) {
        status = pj_sockaddr_synthesize(sess->server.addr.sa_family, &sess->cur_addr, alt_addr);
        if (status != PJ_SUCCESS)
            goto on_error;
    ...
    PJ_LOG(5, (sess->pool->obj_name, "Performing %s to %s:%d", test_names[test_id],
               pj_sockaddr_print(sess->cur_server, addr, sizeof(addr), 2), pj_sockaddr_get_port(sess->cur_server)));

    /* Send the request */
    status = pj_stun_session_send_msg(sess->stun_sess, NULL, PJ_TRUE, PJ_TRUE, sess->cur_server,
