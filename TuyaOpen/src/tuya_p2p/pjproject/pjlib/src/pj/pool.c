...
static pj_pool_block *pj_pool_create_block(pj_pool_t *pool, pj_size_t size)
{
    ...
    LOG((pool->obj_name, "create_block(sz=%u), cur.cap=%u, cur.used=%u", size, pool->capacity,
...
PJ_DEF(void *) pj_pool_allocate_find(pj_pool_t *pool, pj_size_t size)
{
    ...
    if (pool->increment_size < size + sizeof(pj_pool_block) + PJ_POOL_ALIGNMENT) {
        pj_size_t count;
        count = (size + pool->increment_size + sizeof(pj_pool_block) + PJ_POOL_ALIGNMENT) / pool->increment_size;
    ...
    LOG((pool->obj_name, "%u bytes requested, resizing pool by %u bytes (used=%u, cap=%u)", size, block_size,
 ...
 */
PJ_DEF(void) pj_pool_init_int(pj_pool_t *pool, const char *name, pj_size_t increment_size, pj_pool_callback *callback)
{
    PJ_CHECK_STACK();
    ...
    if (name) {
        if (strchr(name, '%') != NULL) {
            pj_ansi_snprintf(pool->obj_name, sizeof(pool->obj_name), name, pool);
        } else {
 ...
 */
PJ_DEF(pj_pool_t *)
pj_pool_create_int(pj_pool_factory *f, const char *name, pj_size_t initial_size, pj_size_t increment_size,
                   pj_pool_callback *callback)
{
    ...
    PJ_ASSERT_RETURN(initial_size >= sizeof(pj_pool_t) + sizeof(pj_pool_block), NULL);
...
static void reset_pool(pj_pool_t *pool)
{
    ...
    */
    ...
    while (block != &pool->block_list) {
        ...
        pj_list_erase(block);
        (*pool->factory->policy.block_free)(pool->factory, block, block->end - (unsigned char *)block);
        block = prev;
    }
...
{
    LOG((pool->obj_name, "reset(): cap=%d, used=%d(%d%%)", pool->capacity, pj_pool_get_used_size(pool),
...
PJ_DEF(void) pj_pool_destroy_int(pj_pool_t *pool)
{
    ...
    LOG((pool->obj_name, "destroy(): cap=%d, used=%d(%d%%), block0=%p-%p", pool->capacity, pj_pool_get_used_size(pool),
         pj_pool_get_used_size(pool) * 100 / pool->capacity, ((pj_pool_block *)pool->block_list.next)->buf,
         ((pj_pool_block *)pool->block_list.next)->end));

    reset_pool(pool);
    initial_size = ((pj_pool_block *)pool->block_list.next)->end - (unsigned char *)pool;
