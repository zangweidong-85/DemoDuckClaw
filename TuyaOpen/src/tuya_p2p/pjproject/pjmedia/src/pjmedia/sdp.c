...
pjmedia_sdp_attr_find(unsigned count, pjmedia_sdp_attr *const attr_array[], const pj_str_t *name, const pj_str_t *c_fmt)
{
    unsigned i;
...
PJ_DEF(pjmedia_sdp_attr *)
pjmedia_sdp_attr_find2(unsigned count, pjmedia_sdp_attr *const attr_array[], const char *c_name, const pj_str_t *c_fmt)
{
    pj_str_t name;
...
PJ_DEF(pj_status_t) pjmedia_sdp_attr_get_rtpmap(const pjmedia_sdp_attr *attr, pjmedia_sdp_rtpmap *rtpmap)
{
     ...
     * null terminated.
     */
    if (attr->value.ptr[attr->value.slen] != 0 && attr->value.ptr[attr->value.slen] != '\r' &&
...
{
    enum { ATTR_LEN = PJ_INET6_ADDRSTRLEN + 16 };
    ...
    if (a->addr.sa_family == pj_AF_INET()) {
        attr->value.slen = pj_ansi_snprintf(attr->value.ptr, ATTR_LEN, "%u IN IP4 %s", pj_sockaddr_get_port(a),
                                            pj_sockaddr_print(a, tmp_addr, sizeof(tmp_addr), 0));
    } else if (a->addr.sa_family == pj_AF_INET6()) {
        attr->value.slen = pj_ansi_snprintf(attr->value.ptr, ATTR_LEN, "%u IN IP6 %s", pj_sockaddr_get_port(a),
                                            pj_sockaddr_print(a, tmp_addr, sizeof(tmp_addr), 0));
...
PJ_DEF(pjmedia_sdp_attr *) pjmedia_sdp_attr_create_ssrc(pj_pool_t *pool, pj_uint32_t ssrc, const pj_str_t *cname)
{
    ...
    attr->value.ptr = (char *)pj_pool_alloc(pool, cname->slen + 7 /* " cname:"*/
                                                      ...
                                                      + 1 /* NULL */);
    attr->value.slen =
        pj_ansi_snprintf(attr->value.ptr, cname->slen + 18, "%u cname:%.*s", ssrc, (int)cname->slen, cname->ptr);
...
pjmedia_sdp_rtpmap_to_attr(pj_pool_t *pool, const pjmedia_sdp_rtpmap *rtpmap, pjmedia_sdp_attr **p_attr)
{
    ...
    len = pj_ansi_snprintf(tempbuf, sizeof(tempbuf), "%.*s %.*s/%u%s%.*s", (int)rtpmap->pt.slen, rtpmap->pt.ptr,
                           (int)rtpmap->enc_name.slen, rtpmap->enc_name.ptr, rtpmap->clock_rate,
                           (rtpmap->param.slen ? "/" : ""), (int)rtpmap->param.slen, rtpmap->param.ptr);
...
static void apply_media_direction(pjmedia_sdp_session *sdp)
{
    ...
    for (i = 0; i < sdp->attr_count && !dir_attr; ++i) {
        if (!pj_strcmp(&sdp->attr[i]->name, &sendonly) || !pj_strcmp(&sdp->attr[i]->name, &recvonly) ||
    ...
    if (dir_attr) {
        ...
        for (i = 0; i < sdp->media_count; ++i) {
            ...
            for (j = 0; j < m->attr_count; ++j) {
                if (!pj_strcmp(&m->attr[j]->name, &sendrecv) || !pj_strcmp(&m->attr[j]->name, &sendonly) ||
                    !pj_strcmp(&m->attr[j]->name, &recvonly) || !pj_strcmp(&m->attr[j]->name, &inactive)) {
                    break;
                }
            }
...
PJ_DEF(pj_status_t) pjmedia_sdp_parse(pj_pool_t *pool, char *buf, pj_size_t len, pjmedia_sdp_session **p_sdp)
{
        ...
        while (!pj_scan_is_eof(&scanner)) {
            ...
            case 'm':
                ...
                if (session->media_count < PJMEDIA_MAX_SDP_MEDIA)
                    ...
                    PJ_PERROR(2, (THIS_FILE, PJ_ETOOMANY, "Error adding media, media is ignored"));
                break;
            case 'v':
    ...
    PJ_CATCH_ANY
    {
        PJ_PERROR(4, (THIS_FILE, ctx.last_error, "Error parsing SDP in line %d col %d", scanner.line,
...
PJ_DEF(pj_status_t) pjmedia_sdp_validate2(const pjmedia_sdp_session *sdp, pj_bool_t strict)
{
    ...
    CHECK(pj_strcmp2(&sdp->origin.addr_type, "IP4") == 0 || pj_strcmp2(&sdp->origin.addr_type, "IP6") == 0,
