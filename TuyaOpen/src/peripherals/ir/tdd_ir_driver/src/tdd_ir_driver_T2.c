 ...
 * @brief Infrared driver implementation for T2 microcontroller platform.
 *
 * This file provides the hardware-specific implementation of the infrared
 * driver for the T2 microcontroller platform used in Tuya IoT devices.
 * The implementation utilizes the T2's advanced hardware features including
 * enhanced PWM capabilities, high-resolution timers, and improved GPIO
 * functionality to deliver high-performance infrared communication.
 *
 * T2 platform-specific features:
 * - Enhanced PWM modules with improved frequency accuracy
 * - High-resolution timer support for precise IR timing control
 * - Advanced GPIO interrupt handling with reduced latency
 * - Optimized timing calculations for improved signal fidelity
 * - Enhanced power management for extended battery life
 * - Hardware-specific optimizations for T2 architecture
 * - Support for multiple concurrent IR channels
 *
 * The T2 implementation takes advantage of improved hardware capabilities
 * to provide better timing accuracy and reduced power consumption compared
 * to earlier platform implementations, while maintaining compatibility
 * with the standard TDL IR interface.
 *
