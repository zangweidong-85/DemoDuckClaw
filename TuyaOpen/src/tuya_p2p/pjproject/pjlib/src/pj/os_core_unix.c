...
struct pj_event_t {
    enum event_state { EV_STATE_OFF, EV_STATE_SET, EV_STATE_PULSED } state;
...
static pj_status_t set_android_thread_priority(int priority)
{
    ...
    (*jni_env)->CallStaticVoidMethod(jni_env, process_class, set_prio_method, priority);
    ...
    if (exc) {
        ...
        PJ_LOG(5, (THIS_FILE, "Setting thread priority to %d successful", priority));
    }
...
PJ_DEF(pj_status_t) pj_thread_register(const char *cstr_thread_name, pj_thread_desc desc, pj_thread_t **ptr_thread)
{
     ...
     */
    pj_assert(thread->signature1 != SIGNATURE1 || thread->signature2 != SIGNATURE2 ||
    ...
    if (cstr_thread_name && pj_strlen(&thread_name) < sizeof(thread->obj_name) - 1)
        pj_ansi_snprintf(thread->obj_name, sizeof(thread->obj_name), cstr_thread_name, thread->thread);
    else
        pj_ansi_snprintf(thread->obj_name, sizeof(thread->obj_name), "thr%p", (void *)thread->thread);
...
{
#if (defined(PJ_LINUX) && PJ_LINUX != 0) || (defined(PJ_ANDROID) && PJ_ANDROID != 0)
    char xname[16];
...
#if (defined(PJ_LINUX) && PJ_LINUX != 0) || defined(PJ_HAS_PTHREAD_MUTEXATTR_SETTYPE)
        rc = pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_NORMAL);
#elif (defined(PJ_RTEMS) && PJ_RTEMS != 0) || defined(PJ_PTHREAD_MUTEXATTR_T_HAS_RECURSIVE)
...
#if PJ_DEBUG
    PJ_LOG(6, (mutex->obj_name, "Mutex: thread %s is waiting (mutex owner=%s)", pj_thread_this()->obj_name,
               mutex->owner_name));
#else
    PJ_LOG(6, (mutex->obj_name, "Mutex: thread %s is waiting", pj_thread_this()->obj_name));
#endif
...
#else
    PJ_LOG(6, (mutex->obj_name, (status == 0 ? "Mutex acquired by thread %s" : "FAILED by %s"),
...
PJ_DEF(pj_status_t) pj_mutex_unlock(pj_mutex_t *mutex)
{
    ...
    PJ_LOG(6, (mutex->obj_name, "Mutex released by thread %s (level=%d)", pj_thread_this()->obj_name,
               mutex->nesting_level));
#else
    PJ_LOG(6, (mutex->obj_name, "Mutex released by thread %s", pj_thread_this()->obj_name));
#endif
...
PJ_DEF(pj_status_t) pj_mutex_trylock(pj_mutex_t *mutex)
{
    ...
    PJ_LOG(6, (mutex->obj_name, "Mutex: thread %s is trying", pj_thread_this()->obj_name));
    ...
    if (status == 0) {
        ...
        PJ_LOG(6, (mutex->obj_name, "Mutex acquired by thread %s (level=%d)", pj_thread_this()->obj_name,
                   mutex->nesting_level));
#else
        PJ_LOG(6, (mutex->obj_name, "Mutex acquired by thread %s", pj_thread_this()->obj_name));
#endif
    } else {
        PJ_LOG(6, (mutex->obj_name, "Mutex: thread %s's trylock() failed", pj_thread_this()->obj_name));
    }
...
#if PJ_HAS_THREADS
    PJ_LOG(6, (mutex->obj_name, "Mutex destroyed by thread %s", pj_thread_this()->obj_name));
...
PJ_DEF(pj_status_t) pj_rwmutex_create(pj_pool_t *pool, const char *name, pj_rwmutex_t **p_mutex)
{
    pj_rwmutex_t *rwm;
    pj_status_t status;
...
#define MAX_SEM_NAME_LEN 23
        ...
        /* Create semaphore */
        sem->sem = sem_open(sem_name, O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, initial);
...
PJ_DEF(pj_status_t) pj_sem_wait(pj_sem_t *sem)
{
    ...
    PJ_LOG(6, (sem->obj_name, "Semaphore: thread %s is waiting", pj_thread_this()->obj_name));
...
#if PJ_HAS_THREADS
    int result;
    PJ_LOG(6, (sem->obj_name, "Semaphore released by thread %s", pj_thread_this()->obj_name));
#if defined(PJ_SEMAPHORE_USE_DISPATCH_SEM) && PJ_SEMAPHORE_USE_DISPATCH_SEM != 0
...
PJ_DEF(pj_status_t) pj_sem_destroy(pj_sem_t *sem)
{
    ...
    PJ_LOG(6, (sem->obj_name, "Semaphore destroyed by thread %s", pj_thread_this()->obj_name));
#if defined(PJ_DARWINOS) && PJ_DARWINOS != 0
#if defined(PJ_SEMAPHORE_USE_DISPATCH_SEM) && PJ_SEMAPHORE_USE_DISPATCH_SEM != 0
 ...
 */
PJ_DEF(pj_status_t)
pj_event_create(pj_pool_t *pool, const char *name, pj_bool_t manual_reset, pj_bool_t initial, pj_event_t **ptr_event)
{
    pj_event_t *event;
