...
static unsigned detect_epoll_support()
{
    ...
    PJ_LOG(3, (THIS_FILE, "epoll EXCLUSIVE/ONESHOT support disabled, reason: %s", DISABLE_EXCLUSIVE_ONESHOT));
    epoll_support = 0;
    return epoll_support;
#endif
    ...
    pj_ansi_snprintf(ioq_name, sizeof(ioq_name), "epoll[0x%x]", tmp_epoll_support);
...
pj_ioqueue_create2(pj_pool_t *pool, pj_size_t max_fd, const pj_ioqueue_cfg *cfg, pj_ioqueue_t **p_ioqueue)
{
    ...
    pj_status_t rc;
    pj_lock_t *lock;
    const unsigned type_mask = PJ_IOQUEUE_EPOLL_EXCLUSIVE | PJ_IOQUEUE_EPOLL_ONESHOT;
   ...
   */
    ...
    PJ_LOG(4, ("pjlib", "epoll I/O Queue created (flags:0x%x, ptr=%p)", ioqueue->cfg.epoll_flags, ioqueue));
...
PJ_DEF(pj_status_t) pj_ioqueue_unregister(pj_ioqueue_key_t *key)
{
    ...
    if (status != 0) {
        status = pj_get_os_error();
        PJ_PERROR(2, (THIS_FILE, status, "Ignoring pj_ioqueue_unregister error: os_epoll_ctl"));
...
static void update_epoll_event_set(pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *key, pj_uint32_t events)
{
    int rc;
    /* From epoll_ctl(2):
    ...
    if (rc != 0) {
        ...
        PJ_PERROR(1, (THIS_FILE, status, "epol_ctl(MOD) error (events=0x%x)", events));
    }
}
...
PJ_DEF(int) pj_ioqueue_poll(pj_ioqueue_t *ioqueue, const pj_time_val *timeout)
{
    ...
    pj_get_timestamp(&t2);
    TRACE_((THIS_FILE, "  os_epoll_wait returns %d, time=%d usec", count, pj_elapsed_usec(&t1, &t2)));

    /* Lock ioqueue. */
    pj_lock_acquire(ioqueue->lock);

    for (event_cnt = 0, i = 0; i < count; ++i) {
        pj_ioqueue_key_t *h = (pj_ioqueue_key_t *)(epoll_data_type)events[i].epoll_data;
         ...
         */
        TRACE_WARN((THIS_FILE, "     UNHANDLED event %d: events=0x%x, h=%p", i, events[i].events, h));
    }
    ...
    for (i = 0; i < event_cnt; ++i) {
        ...
        if (processed_cnt < PJ_IOQUEUE_MAX_EVENTS_IN_SINGLE_POLL) {
            ...
            case WRITEABLE_EVENT:
                event_done = ioqueue_dispatch_write_event(ioqueue, queue[i].key);

                break;
            case EXCEPTION_EVENT:
                event_done = ioqueue_dispatch_exception_event(ioqueue, queue[i].key);
                break;
            case NO_EVENT:
         ...
         */
        if ((ioqueue->cfg.epoll_flags & PJ_IOQUEUE_EPOLL_ONESHOT) && (queue[i].key->ev.events & IO_MASK)) {
