...
enum { F_DONT_CALL = 1, F_DONT_ASSERT = 2, F_SET_ID = 4 };
...
static void copy_node(pj_timer_heap_t *ht, pj_size_t slot, pj_timer_entry_dup *moved_node)
{
    PJ_CHECK_STACK();
...
static void reheap_down(pj_timer_heap_t *ht, pj_timer_entry_dup *moved_node, size_t slot, size_t child)
{
    PJ_CHECK_STACK();
...
static void reheap_up(pj_timer_heap_t *ht, pj_timer_entry_dup *moved_node, size_t slot, size_t parent)
{
...
static pj_timer_entry_dup *remove_node(pj_timer_heap_t *ht, size_t slot)
{
    ...
    // Set the ID
    if (GET_FIELD(removed_node, _timer_id) != GET_ENTRY(removed_node)->_timer_id) {
#if PJ_TIMER_DEBUG
        PJ_LOG(3, (THIS_FILE,
                   "Bug! Trying to remove entry %p from %s "
                   "line %d, which has been deallocated "
                   "without being cancelled",
                   GET_ENTRY(removed_node), removed_node->src_file, removed_node->src_line));
#else
        PJ_LOG(3, (THIS_FILE,
...
static pj_status_t grow_heap(pj_timer_heap_t *ht)
{
    ...
    PJ_LOG(6, (THIS_FILE, "Growing heap size from %d to %d", ht->max_size, new_size));

    // First grow the heap itself.
    new_heap = (pj_timer_entry_dup **)pj_pool_calloc(ht->pool, new_size, sizeof(pj_timer_entry_dup *));
    if (!new_heap)
        return PJ_ENOMEM;
    ...
    new_timer_dups = (pj_timer_entry_dup *)pj_pool_alloc(ht->pool, sizeof(pj_timer_entry_dup) * new_size);
    if (!new_timer_dups)
        return PJ_ENOMEM;

    memcpy(new_timer_dups, ht->timer_dups, ht->max_size * sizeof(pj_timer_entry_dup));
...
static pj_status_t insert_node(pj_timer_heap_t *ht, pj_timer_entry *new_node, const pj_time_val *future_time)
{
    ...
    if (ht->cur_size == 0) {
        ...
        while (tmp_node->next != &ht->head_list && PJ_TIME_VAL_GT(*future_time, tmp_node->_timer_value)) {
...
PJ_DEF(pj_size_t) pj_timer_heap_mem_size(pj_size_t count)
{
    return /* size of the timer heap itself: */
        ...
        /* size of each entry: */
        (count + 2) * (sizeof(pj_timer_entry_dup *) + sizeof(pj_timer_id_t) + sizeof(pj_timer_entry_dup)) +
        /* lock, pool etc: */
        132;
}
...
PJ_DEF(pj_status_t) pj_timer_heap_create(pj_pool_t *pool, pj_size_t size, pj_timer_heap_t **p_heap)
{
    ...
    *p_heap = NULL;
    ...
    ht->heap = (pj_timer_entry_dup **)pj_pool_calloc(pool, size, sizeof(pj_timer_entry_dup *));
    if (!ht->heap)
        return PJ_ENOMEM;

#if PJ_TIMER_USE_COPY
    // Create the timer entry copies array.
    ht->timer_dups = (pj_timer_entry_dup *)pj_pool_alloc(pool, sizeof(pj_timer_entry_dup) * size);
    if (!ht->timer_dups)
        return PJ_ENOMEM;
#endif
...
PJ_DEF(void) pj_timer_heap_set_lock(pj_timer_heap_t *ht, pj_lock_t *lock, pj_bool_t auto_del)
{
    if (ht->lock && ht->auto_delete_lock)
        pj_lock_destroy(ht->lock);
...
PJ_DEF(unsigned) pj_timer_heap_set_max_timed_out_per_poll(pj_timer_heap_t *ht, unsigned count)
{
...
#if PJ_TIMER_DEBUG
static pj_status_t schedule_w_grp_lock_dbg(pj_timer_heap_t *ht, pj_timer_entry *entry, const pj_time_val *delay,
                                           pj_bool_t set_id, int id_val, pj_grp_lock_t *grp_lock, const char *src_file,
                                           int src_line)
#else
static pj_status_t schedule_w_grp_lock(pj_timer_heap_t *ht, pj_timer_entry *entry, const pj_time_val *delay,
                                       pj_bool_t set_id, int id_val, pj_grp_lock_t *grp_lock)
#endif
{
    pj_status_t status;
    pj_time_val expires;
    ...
    if (pj_timer_entry_running(entry)) {
        ...
        PJ_LOG(3, (THIS_FILE, "Warning! Rescheduling outstanding entry (%p)", entry));
...
#if PJ_TIMER_DEBUG
PJ_DEF(pj_status_t)
pj_timer_heap_schedule_dbg(pj_timer_heap_t *ht, pj_timer_entry *entry, const pj_time_val *delay, const char *src_file,
                           int src_line)
{
...
PJ_DEF(pj_status_t)
pj_timer_heap_schedule_w_grp_lock_dbg(pj_timer_heap_t *ht, pj_timer_entry *entry, const pj_time_val *delay, int id_val,
...
PJ_DEF(pj_status_t)
pj_timer_heap_schedule_w_grp_lock(pj_timer_heap_t *ht, pj_timer_entry *entry, const pj_time_val *delay, int id_val,
...
PJ_DEF(unsigned) pj_timer_heap_poll(pj_timer_heap_t *ht, pj_time_val *next_delay)
{
    ...
    while (ht->cur_size && PJ_TIME_VAL_LTE(min_time_node, now) && count < ht->max_entries_per_poll) {
        ...
        node->_grp_lock = NULL;
        if (GET_FIELD(node, cb) != entry->cb || GET_FIELD(node, user_data) != entry->user_data) {
            valid = PJ_FALSE;
#if PJ_TIMER_DEBUG
            PJ_LOG(3, (THIS_FILE,
...
PJ_DEF(void) pj_timer_heap_dump(pj_timer_heap_t *ht)
{
    ...
    PJ_LOG(3, (THIS_FILE, "  Cur size: %d entries, max: %d", (int)ht->cur_size, (int)ht->max_size));
...
#else
        for (tmp_dup = ht->head_list.next; tmp_dup != &ht->head_list; tmp_dup = tmp_dup->next) {
            ...
            PJ_LOG(3, (THIS_FILE, "    %d\t%d\t%d.%03d\t%s:%d", GET_FIELD(e, _timer_id), GET_FIELD(e, id),
                       (int)delta.sec, (int)delta.msec, e->src_file, e->src_line));
        }
    }
