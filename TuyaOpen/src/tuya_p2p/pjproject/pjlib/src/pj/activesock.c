...
enum read_type { TYPE_NONE, TYPE_RECV, TYPE_RECV_FROM };

enum shutdown_dir { SHUT_NONE = 0, SHUT_RX = 1, SHUT_TX = 2 };
...
static void ioqueue_on_read_complete(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, pj_ssize_t bytes_read);
static void ioqueue_on_write_complete(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, pj_ssize_t bytes_sent);
#if PJ_HAS_TCP
static void ioqueue_on_accept_complete(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, pj_sock_t sock,
                                       pj_status_t status);
static void ioqueue_on_connect_complete(pj_ioqueue_key_t *key, pj_status_t status);
#endif
...
static void activesock_create_iphone_os_stream(pj_activesock_t *asock)
{
    if (ios_bg_support && asock->bg_setting && asock->stream_oriented) {
        ...
        CFStreamCreatePairWithSocket(kCFAllocatorDefault, asock->sock, &asock->readStream, NULL);

        if (!asock->readStream ||
            CFReadStreamSetProperty(asock->readStream, kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeVoIP) !=
                TRUE ||
...
PJ_DEF(pj_status_t)
pj_activesock_create(pj_pool_t *pool, pj_sock_t sock, int sock_type, const pj_activesock_cfg *opt,
                     pj_ioqueue_t *ioqueue, const pj_activesock_cb *cb, void *user_data, pj_activesock_t **p_asock)
{
    ...
    PJ_ASSERT_RETURN(sock_type == pj_SOCK_STREAM() || sock_type == pj_SOCK_DGRAM(), PJ_EINVAL);
    ...
    status = pj_ioqueue_register_sock2(pool, ioqueue, sock, (opt ? opt->grp_lock : NULL), asock, &ioq_cb, &asock->key);
...
PJ_DEF(pj_status_t)
pj_activesock_create_udp(pj_pool_t *pool, const pj_sockaddr *addr, const pj_activesock_cfg *opt, pj_ioqueue_t *ioqueue,
                         const pj_activesock_cb *cb, void *user_data, pj_activesock_t **p_asock,
                         pj_sockaddr *bound_addr)
{
    ...
    status = pj_activesock_create(pool, sock_fd, pj_SOCK_DGRAM(), opt, ioqueue, cb, user_data, p_asock);
    if (status != PJ_SUCCESS) {
        pj_sock_close(sock_fd);
        return status;
    }
...
PJ_DEF(pj_status_t) pj_activesock_set_user_data(pj_activesock_t *asock, void *user_data)
{
    PJ_ASSERT_RETURN(asock, PJ_EINVAL);
    asock->user_data = user_data;
    return PJ_SUCCESS;
...
PJ_DEF(pj_status_t)
pj_activesock_start_read(pj_activesock_t *asock, pj_pool_t *pool, unsigned buff_size, pj_uint32_t flags)
{
    void **readbuf;
    unsigned i;
...
PJ_DEF(pj_status_t)
pj_activesock_start_read2(pj_activesock_t *asock, pj_pool_t *pool, unsigned buff_size, void *readbuf[],
                          pj_uint32_t flags)
{
    unsigned i;
    pj_status_t status;
    ...
    asock->read_op = (struct read_op *)pj_pool_calloc(pool, asock->async_count, sizeof(struct read_op));
    ...
    for (i = 0; i < asock->async_count; ++i) {
        ...
        status = pj_ioqueue_recv(asock->key, &r->op_key, r->pkt, &size_to_read, PJ_IOQUEUE_ALWAYS_ASYNC | flags);
...
PJ_DEF(pj_status_t)
pj_activesock_start_recvfrom(pj_activesock_t *asock, pj_pool_t *pool, unsigned buff_size, pj_uint32_t flags)
{
    void **readbuf;
    unsigned i;
    ...
    return pj_activesock_start_recvfrom2(asock, pool, buff_size, readbuf, flags);
}

PJ_DEF(pj_status_t)
pj_activesock_start_recvfrom2(pj_activesock_t *asock, pj_pool_t *pool, unsigned buff_size, void *readbuf[],
                              pj_uint32_t flags)
{
    unsigned i;
    pj_status_t status;
    ...
    for (i = 0; i < asock->async_count; ++i) {
        ...
        status = pj_ioqueue_recvfrom(asock->key, &r->op_key, r->pkt, &size_to_read, PJ_IOQUEUE_ALWAYS_ASYNC | flags,
    ...
    do {
        ...
        if (bytes_read > 0) {
            ...
            if (asock->read_type == TYPE_RECV && asock->cb.on_data_read) {
                ret = (*asock->cb.on_data_read)(asock, r->pkt, r->size, PJ_SUCCESS, &remainder);
            } else if (asock->read_type == TYPE_RECV_FROM && asock->cb.on_data_recvfrom) {
                ret = (*asock->cb.on_data_recvfrom)(asock, r->pkt, r->size, &r->src_addr, r->src_addr_len, PJ_SUCCESS);
            }
        ...
        } else if (bytes_read <= 0 && -bytes_read != PJ_STATUS_FROM_OS(OSERR_EWOULDBLOCK) &&
                   -bytes_read != PJ_STATUS_FROM_OS(OSERR_EINPROGRESS) &&
                   (asock->stream_oriented || -bytes_read != PJ_STATUS_FROM_OS(OSERR_ECONNRESET))) {
            pj_size_t remainder;
            pj_bool_t ret;
            ...
            if (asock->read_type == TYPE_RECV && asock->cb.on_data_read) {
                ...
                ret = (*asock->cb.on_data_read)(asock, r->pkt, r->size, status, &remainder);
                 ...
                 */
                if (status != PJ_SUCCESS) {
                    ret = (*asock->cb.on_data_recvfrom)(asock, NULL, 0, NULL, 0, status);
                }
            }
        ...
        if (asock->read_type == TYPE_RECV) {
            status = pj_ioqueue_recv(key, op_key, r->pkt + r->size, &bytes_read, flags);
        } else {
            r->src_addr_len = sizeof(r->src_addr);
            status =
                pj_ioqueue_recvfrom(key, op_key, r->pkt + r->size, &bytes_read, flags, &r->src_addr, &r->src_addr_len);
        }
...
static pj_status_t send_remaining(pj_activesock_t *asock, pj_ioqueue_op_key_t *send_key)
{
    ...
    do {
        ...
        size = sd->len - sd->sent;
        status = pj_ioqueue_send(asock->key, send_key, sd->data + sd->sent, &size, sd->flags);
        if (status != PJ_SUCCESS) {
            /* Pending or error */
            break;
        }
...
PJ_DEF(pj_status_t)
pj_activesock_send(pj_activesock_t *asock, pj_ioqueue_op_key_t *send_key, const void *data, pj_ssize_t *size,
                   unsigned flags)
{
...
PJ_DEF(pj_status_t)
pj_activesock_sendto(pj_activesock_t *asock, pj_ioqueue_op_key_t *send_key, const void *data, pj_ssize_t *size,
                     unsigned flags, const pj_sockaddr_t *addr, int addr_len)
{
    PJ_ASSERT_RETURN(asock && send_key && data && size && addr && addr_len, PJ_EINVAL);

    if (asock->shutdown & SHUT_TX)
        return PJ_EINVALIDOP;

    return pj_ioqueue_sendto(asock->key, send_key, data, size, flags, addr, addr_len);
}
...
#if PJ_HAS_TCP
PJ_DEF(pj_status_t) pj_activesock_start_accept(pj_activesock_t *asock, pj_pool_t *pool)
{
    unsigned i;
    ...
    asock->accept_op = (struct accept_op *)pj_pool_calloc(pool, asock->async_count, sizeof(struct accept_op));
    for (i = 0; i < asock->async_count; ++i) {
        ...
        do {
            ...
            status = pj_ioqueue_accept(asock->key, &a->op_key, &a->new_sock, NULL, &a->rem_addr, &a->rem_addr_len);
            if (status == PJ_SUCCESS) {
...
static void ioqueue_on_accept_complete(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, pj_sock_t new_sock,
                                       pj_status_t status)
{
    ...
    do {
        if (status == asock->last_err && status != PJ_SUCCESS) {
            ...
            if (asock->err_counter >= PJ_ACTIVESOCK_MAX_CONSECUTIVE_ACCEPT_ERROR) {
                ...
                if ((status == PJ_STATUS_FROM_OS(OSERR_EWOULDBLOCK)) && (asock->cb.on_accept_complete2)) {
                    (*asock->cb.on_accept_complete2)(asock, accept_op->new_sock, &accept_op->rem_addr,
        ...
        if (status == PJ_SUCCESS && (asock->cb.on_accept_complete2 || asock->cb.on_accept_complete)) {
            pj_bool_t ret;

            /* Notify callback */
            if (asock->cb.on_accept_complete2) {
                ret = (*asock->cb.on_accept_complete2)(asock, accept_op->new_sock, &accept_op->rem_addr,
                                                       accept_op->rem_addr_len, status);
            } else {
                ret = (*asock->cb.on_accept_complete)(asock, accept_op->new_sock, &accept_op->rem_addr,
        ...
        status = pj_ioqueue_accept(asock->key, op_key, &accept_op->new_sock, NULL, &accept_op->rem_addr,
                                   &accept_op->rem_addr_len);
...
PJ_DEF(pj_status_t)
pj_activesock_start_connect(pj_activesock_t *asock, pj_pool_t *pool, const pj_sockaddr_t *remaddr, int addr_len)
{
    PJ_UNUSED_ARG(pool);
