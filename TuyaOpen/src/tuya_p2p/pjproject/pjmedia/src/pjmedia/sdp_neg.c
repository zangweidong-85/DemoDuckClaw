...
static const char *state_str[] = {
    "STATE_NULL", "STATE_LOCAL_OFFER", "STATE_REMOTE_OFFER", "STATE_WAIT_NEGO", "STATE_DONE",
};
...
static struct fmt_match_cb_t fmt_match_cb[PJMEDIA_SDP_NEG_MAX_CUSTOM_FMT_NEG_CB];
...
static pj_status_t custom_fmt_match(pj_pool_t *pool, const pj_str_t *fmt_name, pjmedia_sdp_media *offer,
                                    unsigned o_fmt_idx, pjmedia_sdp_media *answer, unsigned a_fmt_idx, unsigned option);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_create_w_local_offer(pj_pool_t *pool, const pjmedia_sdp_session *local, pjmedia_sdp_neg **p_neg)
{
    pjmedia_sdp_neg *neg;
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_create_w_remote_offer(pj_pool_t *pool, const pjmedia_sdp_session *initial,
                                      const pjmedia_sdp_session *remote, pjmedia_sdp_neg **p_neg)
{
    pjmedia_sdp_neg *neg;
    pj_status_t status;
    ...
    *p_neg = NULL;
    ...
    if (initial) {
        PJ_ASSERT_RETURN((status = pjmedia_sdp_validate(initial)) == PJ_SUCCESS, status);
 ...
 */
PJ_DEF(pj_status_t) pjmedia_sdp_neg_set_prefer_remote_codec_order(pjmedia_sdp_neg *neg, pj_bool_t prefer_remote)
{
 ...
 */
PJ_DEF(pj_status_t) pjmedia_sdp_neg_set_answer_multiple_codecs(pjmedia_sdp_neg *neg, pj_bool_t answer_multiple)
{
    PJ_ASSERT_RETURN(neg, PJ_EINVAL);
...
PJ_DEF(pj_status_t) pjmedia_sdp_neg_get_active_local(pjmedia_sdp_neg *neg, const pjmedia_sdp_session **local)
{
...
PJ_DEF(pj_status_t) pjmedia_sdp_neg_get_active_remote(pjmedia_sdp_neg *neg, const pjmedia_sdp_session **remote)
{
...
PJ_DEF(pj_status_t) pjmedia_sdp_neg_get_neg_remote(pjmedia_sdp_neg *neg, const pjmedia_sdp_session **remote)
{
    PJ_ASSERT_RETURN(neg && remote, PJ_EINVAL);
...
PJ_DEF(pj_status_t) pjmedia_sdp_neg_get_neg_local(pjmedia_sdp_neg *neg, const pjmedia_sdp_session **local)
{
    PJ_ASSERT_RETURN(neg && local, PJ_EINVAL);
...
static pjmedia_sdp_media *sdp_media_clone_deactivate(pj_pool_t *pool, const pjmedia_sdp_media *rem_med,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_modify_local_offer(pj_pool_t *pool, pjmedia_sdp_neg *neg, const pjmedia_sdp_session *local)
{
    return pjmedia_sdp_neg_modify_local_offer2(pool, neg, 0, local);
}

PJ_DEF(pj_status_t)
pjmedia_sdp_neg_modify_local_offer2(pj_pool_t *pool, pjmedia_sdp_neg *neg, unsigned flags,
                                    const pjmedia_sdp_session *local)
{
    ...
    PJ_ASSERT_RETURN(neg->state == PJMEDIA_SDP_NEG_STATE_DONE, PJMEDIA_SDPNEG_EINSTATE);
    ...
    if ((flags & PJMEDIA_SDP_NEG_ALLOW_MEDIA_CHANGE) == 0) {
        ...
        for (oi = 0; oi < old_offer->media_count; ++oi) {
            ...
            if (!found) {
                ...
                pj_array_insert(new_offer->media, sizeof(new_offer->media[0]), new_offer->media_count++, oi, &m);
            }
        }
    } else {
        ...
        for (oi = new_offer->media_count; oi < old_offer->media_count; ++oi) {
            ...
            m = sdp_media_clone_deactivate(pool, old_offer->media[oi], old_offer->media[oi], local);
...
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_send_local_offer(pj_pool_t *pool, pjmedia_sdp_neg *neg, const pjmedia_sdp_session **offer)
{
    /* Check arguments are valid. */
    ...
    *offer = NULL;
    ...
    PJ_ASSERT_RETURN(neg->state == PJMEDIA_SDP_NEG_STATE_DONE || neg->state == PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER,
    ...
    if (neg->state == PJMEDIA_SDP_NEG_STATE_DONE) {
        ...
        neg->state = PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER;
        neg->neg_local_sdp = pjmedia_sdp_session_clone(pool, neg->active_local_sdp);

#if PJMEDIA_SDP_NEG_COMPARE_BEFORE_INC_VERSION
        if (pjmedia_sdp_session_cmp(neg->neg_local_sdp, neg->initial_sdp, 0) != PJ_SUCCESS) {
...
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_set_remote_answer(pj_pool_t *pool, pjmedia_sdp_neg *neg, const pjmedia_sdp_session *remote)
{
    /* Check arguments are valid. */
     ...
     */
    PJ_ASSERT_RETURN(neg->state == PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER, PJMEDIA_SDPNEG_EINSTATE);
...
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_set_remote_offer(pj_pool_t *pool, pjmedia_sdp_neg *neg, const pjmedia_sdp_session *remote)
{
    /* Check arguments are valid. */
    PJ_ASSERT_RETURN(pool && neg && remote, PJ_EINVAL);
...
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_set_local_answer(pj_pool_t *pool, pjmedia_sdp_neg *neg, const pjmedia_sdp_session *local)
{
    /* Check arguments are valid. */
    PJ_ASSERT_RETURN(pool && neg && local, PJ_EINVAL);
     ...
     * be called instead of this function.
     */
    PJ_ASSERT_RETURN(neg->state == PJMEDIA_SDP_NEG_STATE_REMOTE_OFFER || neg->state == PJMEDIA_SDP_NEG_STATE_WAIT_NEGO,
                     PJMEDIA_SDPNEG_EINSTATE);
...
static void update_media_direction(pj_pool_t *pool, const pjmedia_sdp_media *remote, pjmedia_sdp_media *local)
{
 ...
 * from remote.
 */
static pj_status_t process_m_answer(pj_pool_t *pool, pjmedia_sdp_media *offer, pjmedia_sdp_media *answer,
                                    pj_bool_t allow_asym)
{
     ...
     */
    if (pjmedia_sdp_transport_cmp(&answer->desc.transport, &offer->desc.transport) != PJ_SUCCESS) {
    ...
    if (allow_asym) {
        ...
        for (i = 0; i < offer->desc.fmt_count;) {
            ...
            if (pt < 96) {
                 ...
                 */
                for (j = 0; j < answer->desc.fmt_count; ++j) {
                    a = pjmedia_sdp_media_find_attr2(answer, "rtpmap", &answer->desc.fmt[j]);
                    if (a) {
                         ...
                         * count match
                         */
                        if (!pj_stricmp(&or_.enc_name, &ar.enc_name) && or_.clock_rate == ar.clock_rate &&
                            (pj_stricmp(&or_.param, &ar.param) == 0 || (ar.param.slen == 1 && *ar.param.ptr == '1'))) {
                            /* Call custom format matching callbacks */
                            if (custom_fmt_match(pool, &or_.enc_name, offer, i, answer, j, 0) == PJ_SUCCESS) {
                                /* Match! */
                                break;
                            }
                        }
            ...
            if (j == answer->desc.fmt_count) {
                ...
                pj_array_erase(offer->desc.fmt, sizeof(offer->desc.fmt[0]), offer->desc.fmt_count, i);
        ...
        /* Remove unmatched answer formats */
        {
            ...
            for (i = 0; i < answer->desc.fmt_count;) {
                ...
                if (i >= offer->desc.fmt_count || offer_fmt_prior[i] - del_cnt != i) {
                    ...
                    pj_array_erase(answer->desc.fmt, sizeof(answer->desc.fmt[0]), answer->desc.fmt_count, i);
 ...
 */
static pj_status_t process_answer(pj_pool_t *pool, pjmedia_sdp_session *local_offer, pjmedia_sdp_session *answer,
                                  pj_bool_t allow_asym, pjmedia_sdp_session **p_active)
{
    ...
    for (; omi < offer->media_count; ++omi) {
        if (ami == answer->media_count) {
            ...
            am = sdp_media_clone_deactivate(pool, offer->media[omi], offer->media[omi], offer);
        ...
        status = process_m_answer(pool, offer->media[omi], answer->media[ami], allow_asym);
 ...
 * and fmtp.
 */
PJ_INLINE(void) rewrite_pt(pj_pool_t *pool, pj_str_t *attr_val, const pj_str_t *old_pt, const pj_str_t *new_pt)
{
...
static void apply_answer_symmetric_pt(pj_pool_t *pool, pjmedia_sdp_media *answer, unsigned pt_cnt,
                                      const pj_str_t pt_offer[], const pj_str_t pt_answer[])
{
...
static pj_status_t match_offer(pj_pool_t *pool, pj_bool_t prefer_remote_codec_order,
                               pj_bool_t answer_with_multiple_codecs, const pjmedia_sdp_media *offer,
                               const pjmedia_sdp_media *preanswer, const pjmedia_sdp_session *preanswer_sdp,
                               pjmedia_sdp_media **p_answer)
{
    unsigned i;
    pj_bool_t master_has_codec = 0, master_has_other = 0, found_matching_codec = 0, found_matching_telephone_event = 0,
    ...
    if (offer->desc.port == 0) {
        answer = sdp_media_clone_deactivate(pool, offer, preanswer, preanswer_sdp);
        *p_answer = answer;
        return PJ_SUCCESS;
    }
    ...
    for (i = 0; i < master->desc.fmt_count; ++i) {
        ...
        if (pj_isdigit(*master->desc.fmt[i].ptr)) {
            ...
            if (pt < 96) {
                ...
                a = pjmedia_sdp_media_find_attr2(master, "rtpmap", &master->desc.fmt[i]);
                if (!a) {
                    pj_assert(!"Bug! Offer should have been validated");
                 ...
                 */
                for (j = 0; j < slave->desc.fmt_count; ++j) {
                    a = pjmedia_sdp_media_find_attr2(slave, "rtpmap", &slave->desc.fmt[j]);
                    if (a) {
                         ...
                         * channel count  match
                         */
                        if (!pj_stricmp(&or_.enc_name, &lr.enc_name) && or_.clock_rate == lr.clock_rate &&
                            (pj_stricmp(&or_.param, &lr.param) == 0 ||
                             (lr.param.slen == 0 && or_.param.slen == 1 && *or_.param.ptr == '1') ||
                             (or_.param.slen == 0 && lr.param.slen == 1 && *lr.param.ptr == '1'))) {
                            /* Match! */
                            if (is_codec) {
                                ...
                                /* Call custom format matching callbacks */
                                if (custom_fmt_match(pool, &or_.enc_name, o_med, o_fmt_idx, a_med, a_fmt_idx,
                            ...
                            pt_offer[pt_answer_count] =
                                prefer_remote_codec_order ? offer->desc.fmt[i] : offer->desc.fmt[j];
                            pt_answer[pt_answer_count++] =
                                prefer_remote_codec_order ? preanswer->desc.fmt[j] : preanswer->desc.fmt[i];
                            break;
                        }
                    }
                }
            ...
            for (j = 0; j < slave->desc.fmt_count; ++j) {
                if (!pj_strcmp(&master->desc.fmt[i], &slave->desc.fmt[j])) {
                    ...
                    pt_offer[pt_answer_count] = prefer_remote_codec_order ? offer->desc.fmt[i] : offer->desc.fmt[j];
                    pt_answer[pt_answer_count++] =
                        prefer_remote_codec_order ? preanswer->desc.fmt[j] : preanswer->desc.fmt[i];
                    break;
                }
    ...
    */
    ...
    if (found_matching_telephone_event) {
        ...
        for (i = 0; i < pt_answer_count;) {
            ...
            a = pjmedia_sdp_media_find_attr2(preanswer, "rtpmap", &pt_answer[i]);
            pj_assert(a);
            ...
            if (j == nclockrate) {
                pj_array_erase(pt_answer, sizeof(pt_answer[0]), pt_answer_count, i);
                pj_array_erase(pt_offer, sizeof(pt_offer[0]), pt_answer_count, i);
                pt_answer_count--;
            } else {
                ++matched_cnt;
                ++i;
    ...
    for (i = pt_answer_count; i < answer->desc.fmt_count; ++i) {
        ...
        a = pjmedia_sdp_media_find_attr2(answer, "rtpmap", &answer->desc.fmt[i]);
        if (a) {
            pjmedia_sdp_media_remove_attr(answer, a);
        }
...
static pj_status_t create_answer(pj_pool_t *pool, pj_bool_t prefer_remote_codec_order,
                                 pj_bool_t answer_with_multiple_codecs, const pjmedia_sdp_session *initial,
                                 const pjmedia_sdp_session *offer, pjmedia_sdp_session **p_answer)
{
    ...
    for (i = 0; i < offer->media_count; ++i) {
        ...
        for (im = NULL, j = 0; j < initial->media_count; ++j) {
            ...
            if (pj_strcmp(&om->desc.media, &im->desc.media) == 0 &&
                pj_strcmp(&om->desc.transport, &im->desc.transport) == 0 && media_used[j] == 0) {
                pj_status_t status2;

                /* See if it has matching codec. */
                status2 =
                    match_offer(pool, prefer_remote_codec_order, answer_with_multiple_codecs, om, im, initial, &am);
...
PJ_DEF(pj_status_t) pjmedia_sdp_neg_cancel_offer(pjmedia_sdp_neg *neg)
{
    ...
    if (neg->state == PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER && neg->active_local_sdp) {
...
PJ_DEF(pj_status_t) pjmedia_sdp_neg_negotiate(pj_pool_t *pool, pjmedia_sdp_neg *neg, pj_bool_t allow_asym)
{
    pj_status_t status;
    ...
    PJ_ASSERT_RETURN(neg->state == PJMEDIA_SDP_NEG_STATE_WAIT_NEGO, PJMEDIA_SDPNEG_EINSTATE);
    ...
    if (neg->has_remote_answer) {
        ...
        status = process_answer(pool, neg->neg_local_sdp, neg->neg_remote_sdp, allow_asym, &active);
        if (status == PJ_SUCCESS) {
        ...
        status = create_answer(pool, neg->prefer_remote_codec_order, neg->answer_with_multiple_codecs,
                               neg->neg_local_sdp, neg->neg_remote_sdp, &answer);
        if (status == PJ_SUCCESS) {
            ...
            if ((neg->active_local_sdp == NULL) ||
                (pjmedia_sdp_session_cmp(answer, neg->active_local_sdp, 0) != PJ_SUCCESS)) {
                                    ...
                                    unsigned o_fmt_idx, pjmedia_sdp_media *answer, unsigned a_fmt_idx, unsigned option)
{
    ...
    for (i = 0; i < fmt_match_cb_cnt; ++i) {
        if (pj_stricmp(fmt_name, &fmt_match_cb[i].fmt_name) == 0) {
            ...
            return (*fmt_match_cb[i].cb)(pool, offer, o_fmt_idx, answer, a_fmt_idx, option);
        }
    }
...
PJ_DEF(pj_status_t) pjmedia_sdp_neg_register_fmt_match_cb(const pj_str_t *fmt_name, pjmedia_sdp_neg_fmt_match_cb cb)
{
    ...
    if (cb == NULL) {
        ...
        pj_array_erase(fmt_match_cb, sizeof(fmt_match_cb[0]), fmt_match_cb_cnt, i);
        fmt_match_cb_cnt--;
...
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_fmt_match(pj_pool_t *pool, pjmedia_sdp_media *offer, unsigned o_fmt_idx, pjmedia_sdp_media *answer,
                          unsigned a_fmt_idx, unsigned option)
{
    ...
    attr = pjmedia_sdp_media_find_attr2(offer, "rtpmap", &offer->desc.fmt[o_fmt_idx]);
    if (!attr) {
    ...
    attr = pjmedia_sdp_media_find_attr2(answer, "rtpmap", &answer->desc.fmt[a_fmt_idx]);
    if (!attr) {
    ...
    if (pj_stricmp(&o_rtpmap.enc_name, &a_rtpmap.enc_name) != 0 || (o_rtpmap.clock_rate != a_rtpmap.clock_rate) ||
        (!(pj_stricmp(&o_rtpmap.param, &a_rtpmap.param) == 0 ||
           (a_rtpmap.param.slen == 0 && o_rtpmap.param.slen == 1 && *o_rtpmap.param.ptr == '1') ||
           (o_rtpmap.param.slen == 0 && a_rtpmap.param.slen == 1 && *a_rtpmap.param.ptr == '1')))) {
        return PJMEDIA_SDP_EFORMATNOTEQUAL;
    }

    return custom_fmt_match(pool, &o_rtpmap.enc_name, offer, o_fmt_idx, answer, a_fmt_idx, option);
}
