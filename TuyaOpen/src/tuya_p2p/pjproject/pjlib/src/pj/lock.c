 ...
 */
static pj_lock_t mutex_lock_template = {NULL, (FPTR)&pj_mutex_lock, (FPTR)&pj_mutex_trylock, (FPTR)&pj_mutex_unlock,
                                        (FPTR)&pj_mutex_destroy};

static pj_status_t create_mutex_lock(pj_pool_t *pool, const char *name, int type, pj_lock_t **lock)
{
    pj_lock_t *p_lock;
    pj_mutex_t *mutex;
    pj_status_t rc;
...
PJ_DEF(pj_status_t) pj_lock_create_simple_mutex(pj_pool_t *pool, const char *name, pj_lock_t **lock)
{
    return create_mutex_lock(pool, name, PJ_MUTEX_SIMPLE, lock);
}

PJ_DEF(pj_status_t) pj_lock_create_recursive_mutex(pj_pool_t *pool, const char *name, pj_lock_t **lock)
{
...
static pj_lock_t null_lock_template = {NULL, &null_op, &null_op, &null_op, &null_op};

PJ_DEF(pj_status_t) pj_lock_create_null_mutex(pj_pool_t *pool, const char *name, pj_lock_t **lock)
{
    PJ_UNUSED_ARG(name);
    PJ_UNUSED_ARG(pool);
...
static pj_lock_t sem_lock_template = {NULL, (FPTR)&pj_sem_wait, (FPTR)&pj_sem_trywait, (FPTR)&pj_sem_post,
                                      (FPTR)&pj_sem_destroy};

PJ_DEF(pj_status_t)
pj_lock_create_semaphore(pj_pool_t *pool, const char *name, unsigned initial, unsigned max, pj_lock_t **lock)
{
    pj_lock_t *p_lock;
    pj_sem_t *sem;
    pj_status_t rc;
...
static pj_status_t grp_lock_add_handler(pj_grp_lock_t *glock, pj_pool_t *pool, void *comp, void (*destroy)(void *comp),
...
PJ_DEF(pj_status_t) pj_grp_lock_create(pj_pool_t *pool, const pj_grp_lock_config *cfg, pj_grp_lock_t **p_grp_lock)
{
    pj_grp_lock_t *glock;
    ...
    status = pj_lock_create_recursive_mutex(pool, pool->obj_name, &glock->own_lock);
    if (status != PJ_SUCCESS)
        goto on_error;
...
PJ_DEF(pj_status_t)
pj_grp_lock_create_w_handler(pj_pool_t *pool, const pj_grp_lock_config *cfg, void *member,
                             void (*handler)(void *member), pj_grp_lock_t **p_grp_lock)
{
    pj_status_t status;
...
PJ_DEF(pj_status_t) pj_grp_lock_replace(pj_grp_lock_t *old_lock, pj_grp_lock_t *new_lock)
{
...
PJ_DEF(pj_status_t)
pj_grp_lock_add_handler(pj_grp_lock_t *glock, pj_pool_t *pool, void *comp, void (*destroy)(void *comp))
{
    return grp_lock_add_handler(glock, pool, comp, destroy, PJ_TRUE);
}

PJ_DEF(pj_status_t) pj_grp_lock_del_handler(pj_grp_lock_t *glock, void *comp, void (*destroy)(void *comp))
{
    grp_destroy_callback *cb;
...
PJ_DEF(pj_status_t) pj_grp_lock_add_ref_dbg(pj_grp_lock_t *glock, const char *file, int line)
{
    grp_lock_ref *ref;
    pj_status_t status;
...
PJ_DEF(pj_status_t) pj_grp_lock_dec_ref_dbg(pj_grp_lock_t *glock, const char *file, int line)
{
    grp_lock_ref *ref;
...
PJ_DEF(pj_status_t) pj_grp_lock_chain_lock(pj_grp_lock_t *glock, pj_lock_t *lock, int pos)
{
...
PJ_DEF(pj_status_t) pj_grp_lock_unchain_lock(pj_grp_lock_t *glock, pj_lock_t *lock)
{
    grp_lock_item *lck;
...
PJ_DEF(void) pj_grp_lock_dump(pj_grp_lock_t *grp_lock)
{
    ...
    PJ_LOG(4, (THIS_FILE, "Group lock %p, ref_cnt=%d. Reference holders:\n%s", grp_lock,
               pj_grp_lock_get_ref(grp_lock) - 1, info.ptr));

    grp_lock_dec_ref(grp_lock);
#else
    PJ_LOG(4, (THIS_FILE, "Group lock %p, ref_cnt=%d.", grp_lock, pj_grp_lock_get_ref(grp_lock)));
#endif
}
