...
#define ALL_FLAGS (PJ_QOS_PARAM_HAS_DSCP | PJ_QOS_PARAM_HAS_SO_PRIO | PJ_QOS_PARAM_HAS_WMM)
...
PJ_DEF(pj_status_t) pj_qos_get_type(const pj_qos_params *param, pj_qos_type *p_type)
{
    unsigned dscp_type = PJ_QOS_TYPE_BEST_EFFORT, prio_type = PJ_QOS_TYPE_BEST_EFFORT,
...
/* Apply QoS */
PJ_DEF(pj_status_t)
pj_sock_apply_qos(pj_sock_t sock, pj_qos_type qos_type, pj_qos_params *qos_params, unsigned log_level,
                  const char *log_sender, const char *sock_name)
{
    ...
    if (qos_type != PJ_QOS_TYPE_BEST_EFFORT) {
        ...
        if (qos_type_rc != PJ_SUCCESS) {
            pj_perror(log_level, log_sender, qos_type_rc, "Error setting QoS type %d to %s", qos_type, sock_name);
        }
    }
    ...
    if (qos_params && qos_params->flags) {
        ...
        if (qos_params_rc != PJ_SUCCESS) {
            pj_perror(log_level, log_sender, qos_params_rc, "Error setting QoS params (flags=%d) to %s",
...
PJ_DEF(pj_status_t)
pj_sock_apply_qos2(pj_sock_t sock, pj_qos_type qos_type, const pj_qos_params *qos_params, unsigned log_level,
                   const char *log_sender, const char *sock_name)
{
    ...
    return pj_sock_apply_qos(sock, qos_type, qos_params_copy, log_level, log_sender, sock_name);
}
