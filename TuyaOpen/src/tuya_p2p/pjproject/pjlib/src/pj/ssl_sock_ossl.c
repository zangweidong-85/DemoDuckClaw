 ...
 * OpenSSL.
 */
#if defined(PJ_HAS_SSL_SOCK) && PJ_HAS_SSL_SOCK != 0 && (PJ_SSL_SOCK_IMP == PJ_SSL_SOCK_IMP_OPENSSL)
...
#if !USING_LIBRESSL && !defined(OPENSSL_NO_EC) && OPENSSL_VERSION_NUMBER >= 0x1000200fL
...
static void update_certs_info(pj_ssl_sock_t *ssock, X509_STORE_CTX *ctx, pj_ssl_cert_info *local_cert_info,
                              pj_ssl_cert_info *remote_cert_info, pj_bool_t is_verify);
...
#define ERROR_LOG(msg, err, ssock)                                                                                     \
    {                                                                                                                  \
        ...
        PJ_LOG(2,                                                                                                      \
               ("SSL", "%s (%s): Level: %d err: <%lu> <%s> len: %d peer: %s", msg, action, level, err, err_str, len,   \
...
#define ERROR_LOG(msg, err, ssock)                                                                                     \
    {                                                                                                                  \
        ...
        PJ_LOG(2, ("SSL",                                                                                              \
                   ...
                   "peer: %s",                                                                                         \
                   msg, action, level, err, (ERR_lib_error_string(err) ? ERR_lib_error_string(err) : "???"),           \
                   (ERR_func_error_string(err) ? ERR_func_error_string(err) : "???"),                                  \
                   (ERR_reason_error_string(err) ? ERR_reason_error_string(err) : "???"), len,                         \
                   (ssock && pj_sockaddr_has_addr(&ssock->rem_addr)                                                    \
                        ? pj_sockaddr_print(&ssock->rem_addr, buf, sizeof(buf), 3)                                     \
                        : "???")));                                                                                    \
    }
...
/* Additional ciphers recognized by SSL_set_cipher_list()
   ...
   NOTE: ids are designed to not conflict with those from
         ...
         bits before use.
*/
static const struct ssl_ciphers_t ADDITIONAL_CIPHERS[] = {{0xFF000000, "DEFAULT"},     {0xFF000001, "@SECLEVEL=1"},
...
#if defined(PJ_SSL_SOCK_OSSL_USE_THREAD_CB) && PJ_SSL_SOCK_OSSL_USE_THREAD_CB != 0 &&                                  \
...
static pj_status_t init_openssl(void)
{
    ...
    if (ssl_cipher_num == 0 || ssl_curves_num == 0) {
...
#if (USING_LIBRESSL && LIBRESSL_VERSION_NUMBER < 0x2020100fL) || OPENSSL_VERSION_NUMBER < 0x10100000L
        ...
        for (i = 0; i < n; ++i) {
            ...
            ssl_ciphers[i].id = (pj_ssl_cipher)(pj_uint32_t)SSL_CIPHER_get_id(c) & 0x00FFFFFF;
    ...
    if (sslsock_idx == -1) {
        ...
        PJ_LOG(1, (THIS_FILE, "Fatal error: failed to get application data index for "
                              "SSL socket"));
        return status;
    }
...
static int verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
    ...
    if (!ossl_ssl) {
        PJ_LOG(1, (THIS_FILE, "SSL verification callback failed to get SSL instance"));
        goto on_return;
    }
    ...
    if (ssock->param.cb.on_verify_cb) {
        update_certs_info(ssock, x509_ctx, &ssock->local_cert_info, &ssock->remote_cert_info, PJ_TRUE);
...
static void set_dh_use_option(BIO *bio, const pj_ssl_sock_t *ssock, const pj_str_t *pass, SSL_CTX *ctx)
{
#if OPENSSL_VERSION_NUMBER < 0x30000000L
    ...
    dctx = OSSL_DECODER_CTX_new_for_pkey(&dh_pkey, format, structure, keytype, 0, NULL, NULL);
    if (dctx != NULL) {
        if (pass->slen) {
            OSSL_DECODER_CTX_set_passphrase(dctx, (const unsigned char *)pass->ptr, pass->slen);
        }
...
static pj_status_t init_ossl_ctx(pj_ssl_sock_t *ssock)
{
    ...
    if (ssock->is_server) {
        ...
        if (!SSL_CTX_set_session_id_context(ctx, (const unsigned char *)&sid_ctx, sizeof(sid_ctx))) {
    ...
    if (cert) {
        ...
        if (cert->CA_file.slen || cert->CA_path.slen) {
            ...
            rc = SSL_CTX_load_verify_locations(ctx, cert->CA_file.slen == 0 ? NULL : cert->CA_file.ptr,
            ...
            if (rc != 1) {
                ...
                PJ_LOG(4, (ssock->pool->obj_name, "CA certificates loaded from '%s%s%s'", cert->CA_file.ptr,
                           ((cert->CA_file.slen && cert->CA_path.slen) ? " + " : ""), cert->CA_path.ptr));
            }
        }
        ...
        if (cert->cert_file.slen) {
            ...
            if (rc != 1) {
                status = GET_SSL_STATUS(ssock);
                PJ_PERROR(1, (ssock->pool->obj_name, status, "Error loading certificate chain file '%s'",
                              cert->cert_file.ptr));
                SSL_CTX_free(ctx);
                ossock->ossl_ctx = NULL;
                return status;
        ...
        if (cert->privkey_file.slen) {
            ...
            if (rc != 1) {
                status = GET_SSL_STATUS(ssock);
                PJ_PERROR(
                    1, (ssock->pool->obj_name, status, "Error adding private key from '%s'", cert->privkey_file.ptr));
                SSL_CTX_free(ctx);
                ossock->ossl_ctx = NULL;
                return status;
            } else {
                ...
                if (inf != NULL) {
                    ...
                    for (; i < sk_X509_INFO_num(inf); i++) {
                        ...
                        if (rc == 1) {
                            ...
                            char buf[256];
                            PJ_LOG(4, (ssock->pool->obj_name, "Error adding CA cert: %s",
                                       X509_NAME_oneline(X509_get_subject_name(itmp->x509), buf, sizeof(buf))));
#endif
                        }
                    }
        ...
        if (p) {
            ...
            for (i = 0; i < PJ_ARRAY_SIZE(cert_types); ++i) {
                ...
                if (err == 1) {
                    ...
                    PJ_PERROR(
                        1, (ssock->pool->obj_name, GET_SSL_STATUS(ssock), "Error loading certificate file '%s'", cf));
                    ERR_clear_error();
                }
            }
...
#define SSL_CTRL_SET_ECDH_AUTO 94
    ...
    if (cert && ssock->is_server) {
        ...
        if (ca_dn != NULL) {
            ...
            PJ_LOG(1, (ssock->pool->obj_name, "Error reading CA certificates from %s",
                       (cert->CA_file.slen ? cert->CA_file.ptr : "buffer")));
        }
    }
...
static void ssl_reset_sock_state(pj_ssl_sock_t *ssock)
{
    ...
    if (ossock->ossl_ssl && SSL_in_init(ossock->ossl_ssl) == 0) {
        if (ssock->handshake_status == PJ_ETIMEDOUT ||
            (ssock->last_err != SSL_ERROR_SYSCALL && ssock->last_err != SSL_ERROR_SSL)) {
 ...
 */
static void ssl_update_remote_cert_chain_info(pj_pool_t *pool, pj_ssl_cert_info *ci, STACK_OF(X509) * chain,
                                              pj_bool_t get_pem)
{
    int i;
...
static void ssl_update_certs_info(pj_ssl_sock_t *ssock)
{
    ...
    update_certs_info(ssock, NULL, &ssock->local_cert_info, &ssock->remote_cert_info, PJ_FALSE);
}
                              ...
                              pj_ssl_cert_info *remote_cert_info, pj_bool_t is_verify)
{
    ...
    if (chain) {
        ...
        ssl_update_remote_cert_chain_info(ssock->info_pool, remote_cert_info, chain, PJ_TRUE);
...
static void ssl_set_peer_name(pj_ssl_sock_t *ssock)
{
    ...
    if (ssock->param.server_name.slen && get_ip_addr_ver(&ssock->param.server_name) == 0) {
        if (ssock->is_server) {
#if defined(SSL_CTX_set_tlsext_servername_callback) && defined(SSL_CTX_set_tlsext_servername_arg)
    ...
    if (SSL_is_init_finished(ossock->ossl_ssl)) {
        ...
        if (ssock->is_server && ssock->ssl_state != SSL_STATE_ESTABLISHED) {
            ...
            PJ_LOG(5, (THIS_FILE,
                       ...
                       "timeout=%d",
                       SSL_session_reused(ossock->ossl_ssl), SSL_SESSION_is_resumable(sess),
                       SSL_SESSION_get_timeout(sess)));
#else
            PJ_LOG(5, (THIS_FILE,
                       "Session info: reused=%d, resumable=%d, "
                       "timeout=%d",
                       SSL_session_reused(ossock->ossl_ssl), -1, SSL_SESSION_get_timeout(sess)));
#endif
