...
PJ_DEF(pj_status_t)
pjstun_get_mapped_addr(pj_pool_factory *pf, int sock_cnt, pj_sock_t sock[], const pj_str_t *srv1, int port1,
                       const pj_str_t *srv2, int port2, pj_sockaddr_in mapped_addr[])
{
    pjstun_setting opt;
...
PJ_DEF(pj_status_t)
pjstun_get_mapped_addr2(pj_pool_factory *pf, const pjstun_setting *opt, int sock_cnt, pj_sock_t sock[],
                        pj_sockaddr_in mapped_addr[])
{
    ...
    status = pjstun_create_bind_req(pool, &out_msg, &out_msg_len, pj_rand(), pj_rand());
    ...
    if (srv2 && port2) {
        status = pj_sockaddr_init(opt->af, &srv_addr[1], srv2, (pj_uint16_t)port2);
        if (status != PJ_SUCCESS)
            goto on_error;
    ...
    for (send_cnt = 0; send_cnt < MAX_REQUEST; ++send_cnt) {
        ...
        for (i = 0; i < sock_cnt && status == PJ_SUCCESS; ++i) {
            ...
            for (j = 0; j < srv_cnt && status == PJ_SUCCESS; ++j) {
                ...
                status = pj_sock_sendto(sock[i], out_msg, &sent_len, 0, (pj_sockaddr_t *)&srv_addr[j],
        ...
        for (pj_gettickcount(&now), select_rc = 1;
             status == PJ_SUCCESS && select_rc >= 1 && wait_resp > 0 && PJ_TIME_VAL_LT(now, next_tx);
             pj_gettickcount(&now)) {
            pj_time_val timeout;
            ...
            for (i = 0; i < sock_cnt; ++i) {
                ...
                len = sizeof(recv_buf);
                status = pj_sock_recvfrom(sock[i], recv_buf, &len, 0, (pj_sockaddr_t *)&addr, &addrlen);

                if (status != PJ_SUCCESS) {
                    PJ_PERROR(4, (THIS_FILE, status, "recvfrom() error ignored"));
                ...
                if (status != PJ_SUCCESS) {
                    PJ_PERROR(4, (THIS_FILE, status, "STUN parsing error ignored"));
                ...
                if (sock_idx < 0 || sock_idx >= sock_cnt || sock_idx != i || srv_idx < 0 || srv_idx >= 2) {
                ...
                attr = (pjstun_mapped_addr_attr *)pjstun_msg_find_attr(&msg, PJSTUN_ATTR_MAPPED_ADDR);
                if (!attr) {
                    attr = (pjstun_mapped_addr_attr *)pjstun_msg_find_attr(&msg, PJSTUN_ATTR_XOR_MAPPED_ADDR);
    ...
    TRACE_((THIS_FILE, "  Pool usage=%d of %d", pj_pool_get_used_size(pool), pj_pool_get_capacity(pool)));
