...
static void on_timeout(pj_timer_heap_t *timer_heap, struct pj_timer_entry *entry);

/* Select which nameserver to use */
static pj_status_t select_nameservers(pj_dns_resolver *resolver, unsigned *count, unsigned servers[]);
...
static pj_status_t init_sock(pj_dns_resolver *resv)
{
    ...
    status = pj_ioqueue_register_sock2(resv->pool, resv->ioqueue, resv->udp_sock, resv->grp_lock, resv, &socket_cb,
                                       &resv->udp_key);
    if (status != PJ_SUCCESS)
        return status;
    ...
    status = pj_ioqueue_recvfrom(resv->udp_key, &resv->udp_op_rx_key, resv->udp_rx_pkt, &rx_pkt_size,
    ...
    /* Create the UDP socket */
    status = pj_sock_socket(pj_AF_INET6(), pj_SOCK_DGRAM(), 0, &resv->udp6_sock);
    if (status != PJ_SUCCESS) {
        /* Skip IPv6 socket on system without IPv6 (see ticket #1953) */
        if (status == PJ_STATUS_FROM_OS(OSERR_EAFNOSUPPORT)) {
            PJ_LOG(3, (resv->name.ptr, "System does not support IPv6, resolver will "
    ...
    status = pj_sock_bind(resv->udp6_sock, &bound_addr, pj_sockaddr_get_len(&bound_addr));
    if (status != PJ_SUCCESS)
        return status;
    ...
    pj_bzero(&socket_cb, sizeof(socket_cb));
    socket_cb.on_read_complete = &on_read_complete;
    status = pj_ioqueue_register_sock2(resv->pool, resv->ioqueue, resv->udp6_sock, resv->grp_lock, resv, &socket_cb,
                                       &resv->udp6_key);
    if (status != PJ_SUCCESS)
        return status;
    ...
    status = pj_ioqueue_recvfrom(resv->udp6_key, &resv->udp6_op_rx_key, resv->udp6_rx_pkt, &rx_pkt_size,
 ...
 */
PJ_DEF(pj_status_t)
pj_dns_resolver_create(pj_pool_factory *pf, const char *name, unsigned options, pj_timer_heap_t *timer,
                       pj_ioqueue_t *ioqueue, pj_dns_resolver **p_resolver)
{
    pj_pool_t *pool;
    ...
    /* Create group lock */
    status = pj_grp_lock_create_w_handler(pool, NULL, resv, &dns_resolver_on_destroy, &resv->grp_lock);
    if (status != PJ_SUCCESS)
        goto on_error;
 ...
 */
PJ_DEF(pj_status_t) pj_dns_resolver_destroy(pj_dns_resolver *resolver, pj_bool_t notify)
{
    ...
    if (notify) {
        ...
        while (it) {
            pj_dns_async_query *q = (pj_dns_async_query *)pj_hash_this(resolver->hquerybyid, it);
    ...
    while (it) {
        ...
        pj_hash_set(NULL, resolver->hrescache, &cache->key, sizeof(cache->key), 0, NULL);
 ...
 */
PJ_DEF(pj_status_t)
pj_dns_resolver_set_ns(pj_dns_resolver *resolver, unsigned count, const pj_str_t servers[], const pj_uint16_t ports[])
{
    unsigned i;
    pj_time_val now;
    pj_status_t status;
    ...
    for (i = 0; i < count; ++i) {
        ...
        status = pj_sockaddr_init(pj_AF_INET(), &ns->addr, &servers[i], (pj_uint16_t)(ports ? ports[i] : PORT));
        if (status != PJ_SUCCESS)
            status = pj_sockaddr_init(pj_AF_INET6(), &ns->addr, &servers[i], (pj_uint16_t)(ports ? ports[i] : PORT));
        if (status != PJ_SUCCESS) {
 ...
 */
PJ_DEF(pj_status_t) pj_dns_resolver_set_settings(pj_dns_resolver *resolver, const pj_dns_settings *st)
{
 ...
 */
PJ_DEF(pj_status_t) pj_dns_resolver_get_settings(pj_dns_resolver *resolver, pj_dns_settings *st)
{
    PJ_ASSERT_RETURN(resolver && st, PJ_EINVAL);
 ...
 */
PJ_DEF(void) pj_dns_resolver_handle_events(pj_dns_resolver *resolver, const pj_time_val *timeout)
{
 ...
 * a new one.
 */
static pj_dns_async_query *alloc_qnode(pj_dns_resolver *resolver, unsigned options, void *user_data,
                                       pj_dns_callback *cb)
{
 ...
 * Transmit query.
 */
static pj_status_t transmit_query(pj_dns_resolver *resolver, pj_dns_async_query *q)
{
    unsigned pkt_size;
    ...
    status = pj_timer_heap_schedule_w_grp_lock(resolver->timer, &q->timer_entry, &delay, 1, resolver->grp_lock);
    if (status != PJ_SUCCESS) {
        return status;
    }
...
#if PJ_HAS_IPV6
        || (resolver->udp6_key && pj_ioqueue_is_pending(resolver->udp6_key, &resolver->udp6_op_tx_key))
#endif
    ) {
        ++q->transmit_cnt;
        PJ_LOG(4, (resolver->name.ptr, "Socket busy in transmitting DNS %s query for %s%s",
                   pj_dns_get_type_name(q->key.qtype), q->key.name,
                   (q->transmit_cnt < resolver->settings.qretr_count ? ", will try again later" : "")));
        return PJ_SUCCESS;
    }
    ...
    status = pj_dns_make_query(resolver->udp_tx_pkt, &pkt_size, q->id, q->key.qtype, &name);
    if (status != PJ_SUCCESS) {
    ...
    for (i = 0; i < server_cnt; ++i) {
        ...
        if (ns->addr.addr.sa_family == pj_AF_INET()) {
            status = pj_ioqueue_sendto(resolver->udp_key, &resolver->udp_op_tx_key, resolver->udp_tx_pkt, &sent, 0,
                                       &ns->addr, pj_sockaddr_get_len(&ns->addr));
            if (status == PJ_SUCCESS || status == PJ_EPENDING)
                send_cnt++;
        }
#if PJ_HAS_IPV6
        else if (resolver->udp6_key) {
            status = pj_ioqueue_sendto(resolver->udp6_key, &resolver->udp6_op_tx_key, resolver->udp_tx_pkt, &sent, 0,
                                       &ns->addr, pj_sockaddr_get_len(&ns->addr));
            if (status == PJ_SUCCESS || status == PJ_EPENDING)
                send_cnt++;
        }
        ...
        PJ_PERROR(4, (resolver->name.ptr, status, "%s %d bytes to NS %d (%s:%d): DNS %s query for %s",
                      (q->transmit_cnt == 0 ? "Transmitting" : "Re-transmitting"), (int)pkt_size, servers[i],
                      pj_sockaddr_print(&ns->addr, addr, sizeof(addr), 2), pj_sockaddr_get_port(&ns->addr),
                      pj_dns_get_type_name(q->key.qtype), q->key.name));
...
static struct cached_res *alloc_entry(pj_dns_resolver *resolver)
{
    ...
    pool = pj_pool_create(resolver->pool->factory, "dnscache", RES_BUF_SZ, 256, NULL);
 ...
 */
PJ_DEF(pj_status_t)
pj_dns_resolver_start_query(pj_dns_resolver *resolver, const pj_str_t *name, int type, unsigned options,
                            pj_dns_callback *cb, void *user_data, pj_dns_async_query **p_query)
{
    pj_time_val now;
    struct res_key key;
    struct cached_res *cache;
    ...
    PJ_ASSERT_RETURN(name->slen > 0 && name->slen < PJ_MAX_HOSTNAME, PJ_ENAMETOOLONG);
     ...
     */
    hval = 0;
    cache = (struct cached_res *)pj_hash_get(resolver->hrescache, &key, sizeof(key), &hval);
    if (cache) {
        ...
        if (PJ_TIME_VAL_GT(cache->expiry_time, now)) {
            ...
            /* Log */
            PJ_LOG(5,
                   (resolver->name.ptr, "Picked up DNS %s record for %.*s from cache, ttl=%d",
                    pj_dns_get_type_name(type), (int)name->slen, name->ptr, (int)(cache->expiry_time.sec - now.sec)));
    ...
    q = (pj_dns_async_query *)pj_hash_get(resolver->hquerybyres, &key, sizeof(key), NULL);
    if (q) {
    ...
    pj_hash_set_np(resolver->hquerybyid, &q->id, sizeof(q->id), 0, q->hbufid, q);
    pj_hash_set_np(resolver->hquerybyres, &q->key, sizeof(q->key), 0, q->hbufkey, q);
 ...
 */
PJ_DEF(pj_status_t) pj_dns_resolver_cancel_query(pj_dns_async_query *query, pj_bool_t notify)
{
    pj_dns_callback *cb;
    ...
    if (query->timer_entry.id == 1) {
        pj_timer_heap_cancel_if_active(query->resolver->timer, &query->timer_entry, 0);
    }
 ...
 */
PJ_DEF(pj_status_t) pj_dns_parse_a_response(const pj_dns_parsed_packet *pkt, pj_dns_a_record *rec)
{
    enum { MAX_SEARCH = 20 };
    ...
    while (pkt->ans[ansidx].type == PJ_DNS_TYPE_CNAME && search_cnt++ < MAX_SEARCH) {
    ...
    for (i = 0; i < pkt->hdr.anscount; ++i) {
        if (pkt->ans[i].type == PJ_DNS_TYPE_A && pj_stricmp(&pkt->ans[i].name, resname) == 0 &&
            rec->addr_count < PJ_DNS_MAX_IP_IN_A_REC) {
            rec->addr[rec->addr_count++].s_addr = pkt->ans[i].rdata.a.ip_addr.s_addr;
        }
    }
 ...
 */
PJ_DEF(pj_status_t) pj_dns_parse_addr_response(const pj_dns_parsed_packet *pkt, pj_dns_addr_record *rec)
{
    enum { MAX_SEARCH = 20 };
    pj_str_t hostname, alias = {NULL, 0}, *resname;
    pj_size_t bufstart = 0;
    ...
    if (pkt->ans[ansidx].type != PJ_DNS_TYPE_A && pkt->ans[ansidx].type != PJ_DNS_TYPE_AAAA) {
        return PJLIB_UTIL_EDNSINANSWER;
    }
    ...
    for (i = 0; i < pkt->hdr.anscount && cnt < PJ_DNS_MAX_IP_IN_A_REC; ++i) {
        if ((pkt->ans[i].type == PJ_DNS_TYPE_A || pkt->ans[i].type == PJ_DNS_TYPE_AAAA) &&
...
static void set_nameserver_state(pj_dns_resolver *resolver, unsigned index, enum ns_state state, const pj_time_val *now)
{
    ...
    if (state == STATE_PROBING)
        ns->state_expiry.sec += ((resolver->settings.qretr_count + 2) * resolver->settings.qretr_delay) / 1000;
...
static void report_nameserver_status(pj_dns_resolver *resolver, const pj_sockaddr *ns_addr,
                                     const pj_dns_parsed_packet *pkt)
{
    ...
    for (i = 0; i < resolver->ns_count; ++i) {
        ...
        if (pj_sockaddr_cmp(&ns->addr, ns_addr) == 0) {
            ...
            set_nameserver_state(resolver, i, (is_good ? STATE_ACTIVE : STATE_BAD), &now);
            break;
        }
    }
}
...
static void update_res_cache(pj_dns_resolver *resolver, const struct res_key *key, pj_status_t status,
                             pj_bool_t set_expiry, const pj_dns_parsed_packet *pkt)
{
    struct cached_res *cache;
     ...
     */
    pj_dns_packet_dup(cache->pool, pkt, PJ_DNS_NO_NS | PJ_DNS_NO_AR, &cache->pkt);
    ...
    pj_hash_set_np(resolver->hrescache, &cache->key, sizeof(*key), hval, cache->hbuf, cache);
}
...
static void on_timeout(pj_timer_heap_t *timer_heap, struct pj_timer_entry *entry)
{
    ...
    if (q->transmit_cnt < resolver->settings.qretr_count) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            /* Error occurs */
            PJ_PERROR(4, (resolver->name.ptr, status, "Error transmitting request"));
...
static void on_read_complete(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, pj_ssize_t bytes_read)
{
    ...
    PJ_LOG(5, (resolver->name.ptr, "Received %d bytes DNS response from %s:%d", (int)bytes_read,
               pj_sockaddr_print(src_addr, addr, sizeof(addr), 2), pj_sockaddr_get_port(src_addr)));
    ...
    pool = pj_pool_create_on_buf("restmp", resolver->tmp_pool, sizeof(resolver->tmp_pool));
    ...
    PJ_TRY
    {
        status = pj_dns_parse_packet(pool, rx_pkt, (unsigned)bytes_read, &dns_pkt);
    }
    PJ_CATCH_ANY
    {
        status = PJ_ENOMEM;
    ...
    /* Handle parse error */
    if (status != PJ_SUCCESS) {
        PJ_PERROR(3, (resolver->name.ptr, status, "Error parsing DNS response from %s:%d",
                      pj_sockaddr_print(src_addr, addr, sizeof(addr), 2), pj_sockaddr_get_port(src_addr)));
        goto read_next_packet;
    }

    /* Find the query based on the transaction ID */
    q = (pj_dns_async_query *)pj_hash_get(resolver->hquerybyid, &dns_pkt->hdr.id, sizeof(dns_pkt->hdr.id), NULL);
    if (!q) {
        PJ_LOG(5, (resolver->name.ptr, "DNS response from %s:%d id=%d discarded",
                   pj_sockaddr_print(src_addr, addr, sizeof(addr), 2), pj_sockaddr_get_port(src_addr),
...
read_next_packet:
    ...
    status = pj_ioqueue_recvfrom(key, op_key, rx_pkt, &rx_pkt_size, PJ_IOQUEUE_ALWAYS_ASYNC, src_addr, src_addr_len);

    if (status != PJ_EPENDING && status != PJ_ECANCELLED) {
        PJ_PERROR(4, (resolver->name.ptr, status, "DNS resolver ioqueue read error"));
 ...
 */
PJ_DEF(pj_status_t)
pj_dns_resolver_add_entry(pj_dns_resolver *resolver, const pj_dns_parsed_packet *pkt, pj_bool_t set_ttl)
{
    struct res_key key;
    ...
    PJ_ASSERT_RETURN((pkt->hdr.anscount && pkt->ans) || (pkt->hdr.qdcount && pkt->q), PJLIB_UTIL_EDNSNOANSWERREC);
    ...
    if (pkt->hdr.anscount) {
        ...
        PJ_ASSERT_RETURN(pkt->ans[0].name.slen < PJ_MAX_HOSTNAME, PJ_ENAMETOOLONG);

        init_res_key(&key, pkt->ans[0].type, &pkt->ans[0].name);

    } else {
        /* Make sure name is not too long. */
        PJ_ASSERT_RETURN(pkt->q[0].name.slen < PJ_MAX_HOSTNAME, PJ_ENAMETOOLONG);
...
PJ_DEF(void) pj_dns_resolver_dump(pj_dns_resolver *resolver, pj_bool_t detail)
{
    ...
    for (i = 0; i < resolver->ns_count; ++i) {
        ...
        PJ_LOG(3, (resolver->name.ptr, "   NS %d: %s:%d (state=%s until %ds, rtt=%d ms)", i,
                   pj_sockaddr_print(&ns->addr, addr, sizeof(addr), 2), pj_sockaddr_get_port(&ns->addr),
                   state_names[ns->state], ns->state_expiry.sec - now.sec, PJ_TIME_VAL_MSEC(ns->rt_delay)));
    }

    PJ_LOG(3, (resolver->name.ptr, "  Nb. of cached responses: %u", pj_hash_count(resolver->hrescache)));
    if (detail) {
        ...
        while (it) {
            struct cached_res *cache;
            cache = (struct cached_res *)pj_hash_this(resolver->hrescache, it);
            PJ_LOG(3, (resolver->name.ptr, "   Type %s: %s", pj_dns_get_type_name(cache->key.qtype), cache->key.name));
            it = pj_hash_next(resolver->hrescache, it);
        }
    }
    PJ_LOG(3, (resolver->name.ptr, "  Nb. of pending queries: %u (%u)", pj_hash_count(resolver->hquerybyid),
    ...
    if (detail) {
        ...
        while (it) {
            ...
            PJ_LOG(3, (resolver->name.ptr, "   Type %s: %s", pj_dns_get_type_name(q->key.qtype), q->key.name));
            it = pj_hash_next(resolver->hquerybyid, it);
        }
    }
    PJ_LOG(3, (resolver->name.ptr, "  Nb. of pending query free nodes: %u", pj_list_size(&resolver->query_free_nodes)));
    PJ_LOG(3, (resolver->name.ptr, "  Nb. of timer entries: %u", pj_timer_heap_count(resolver->timer)));
    PJ_LOG(3, (resolver->name.ptr, "  Pool capacity: %d, used size: %d", pj_pool_get_capacity(resolver->pool),
