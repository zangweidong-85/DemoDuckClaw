...
PJ_DEF(pj_status_t)
pj_dns_server_create(pj_pool_factory *pf, pj_ioqueue_t *ioqueue, int af, unsigned port, unsigned flags,
                     pj_dns_server **p_srv)
{
    pj_pool_t *pool;
    pj_dns_server *srv;
    ...
    status = pj_activesock_create_udp(pool, &sock_addr, NULL, ioqueue, &sock_cb, srv, &srv->asock, NULL);
...
static struct rr *find_rr(pj_dns_server *srv, unsigned dns_class, unsigned type /* pj_dns_type */, const pj_str_t *name)
{
    struct rr *r;

    r = srv->rr_list.next;
    while (r != &srv->rr_list) {
        if (r->rec.dnsclass == dns_class && r->rec.type == type && pj_stricmp(&r->rec.name, name) == 0) {
            return r;
        }
        r = r->next;
    }
...
PJ_DEF(pj_status_t) pj_dns_server_add_rec(pj_dns_server *srv, unsigned count, const pj_dns_parsed_rr rr_param[])
{
    unsigned i;
    ...
    for (i = 0; i < count; ++i) {
        ...
        PJ_ASSERT_RETURN(find_rr(srv, rr_param[i].dnsclass, rr_param[i].type, &rr_param[i].name) == NULL, PJ_EEXISTS);
...
PJ_DEF(pj_status_t) pj_dns_server_del_rec(pj_dns_server *srv, int dns_class, pj_dns_type type, const pj_str_t *name)
{
    struct rr *rr;
...
static int print_name(pj_uint8_t *pkt, int size, pj_uint8_t *pos, const pj_str_t *name, struct label_tab *tab)
{
    pj_uint8_t *p = pos;
...
static int print_rr(pj_uint8_t *pkt, int size, pj_uint8_t *pos, const pj_dns_parsed_rr *rr, struct label_tab *tab)
{
    pj_uint8_t *p = pos;
    int len;
...
static int print_packet(const pj_dns_parsed_packet *rec, pj_uint8_t *pkt, int size)
{
    pj_uint8_t *p = pkt;
                                  ...
                                  int addr_len, pj_status_t status)
{
    ...
    if (status != PJ_SUCCESS) {
        ...
        PJ_PERROR(4, (THIS_FILE, status, "Error parsing query from %s", addrinfo));
        goto on_return;
    }
    ...
    /* Init answer record */
    ans.hdr.anscount = 0;
    ans.ans = (pj_dns_parsed_rr *)pj_pool_calloc(pool, MAX_ANS, sizeof(pj_dns_parsed_rr));
    ...
    if (req->q->type == PJ_DNS_TYPE_SRV) {
        ...
        r = srv->rr_list.next;
        while (r != &srv->rr_list) {
            if (r->rec.dnsclass == req->q->dnsclass && r->rec.type == PJ_DNS_TYPE_SRV &&
                pj_stricmp(&r->rec.name, &req->q->name) == 0 && ans.hdr.anscount < MAX_ANS) {
                pj_memcpy(&ans.ans[ans.hdr.anscount], &r->rec, sizeof(pj_dns_parsed_rr));
                ++ans.hdr.anscount;
            }
            r = r->next;
        }
        ...
        pj_memcpy(&ans.ans[ans.hdr.anscount], &rr->rec, sizeof(pj_dns_parsed_rr));
        ++ans.hdr.anscount;
    }
    ...
    for (i = 0; i < ans.hdr.anscount && ans.hdr.anscount < MAX_ANS; ++i) {
        if (ans.ans[i].type == PJ_DNS_TYPE_CNAME) {
            ...
            r = find_rr(srv, ans.ans[i].dnsclass, PJ_DNS_TYPE_A, &ans.ans[i].name);
            pj_memcpy(&ans.ans[ans.hdr.anscount], &r->rec, sizeof(pj_dns_parsed_rr));
            ++ans.hdr.anscount;
        }
    }
...
send_pkt:
    ...
    status = pj_activesock_sendto(srv->asock, &srv->send_key, data, &pkt_len, 0, src_addr, addr_len);
