 ...
 * pjnath_strerror()
 */
static pj_str_t pjnath_strerror(pj_status_t statcode, char *buf, pj_size_t bufsize)
{
    pj_str_t errstr;

#if defined(PJ_HAS_ERROR_STRING) && (PJ_HAS_ERROR_STRING != 0)

    if (statcode >= PJNATH_ERRNO_START && statcode < PJNATH_ERRNO_START + PJ_ERRNO_SPACE_SIZE) {
    ...
    /* Error not found. */
    errstr.ptr = buf;
    errstr.slen = pj_ansi_snprintf(buf, bufsize, "Unknown pjnath error %d", statcode);
...
static pj_str_t pjnath_strerror2(pj_status_t statcode, char *buf, pj_size_t bufsize)
{
    ...
    if (cmsg.slen == 0) {
        /* Not found */
        errstr.ptr = buf;
        errstr.slen = pj_ansi_snprintf(buf, bufsize, "Unknown STUN err-code %d", stun_code);
    } else {
        errstr.ptr = buf;
...
PJ_DEF(pj_status_t) pjnath_init(void)
{
    ...
    status = pj_register_strerror(PJ_STATUS_FROM_STUN_CODE(300), 699 - 300, &pjnath_strerror2);
    pj_assert(status == PJ_SUCCESS);
...
PJ_DEF(void) pjnath_perror(const char *sender, const char *title, pj_status_t status)
{
