...
static pj_status_t elem_write(const pj_json_elem *elem, struct write_state *st, unsigned flags);
static pj_json_elem *parse_elem_throw(struct parse_state *st, pj_json_elem *elem);
...
PJ_DEF(void) pj_json_elem_string(pj_json_elem *el, pj_str_t *name, pj_str_t *value)
{
    ...
    while (ip != iend) {
        if (*ip == '\\') {
            ...
            if (*ip == 'u') {
                 ...
                 * ASCII */
                *op++ = (char)(pj_hex_digit_to_val(ip[2]) * 16 + pj_hex_digit_to_val(ip[3]));
                ip += 4;
            } else if (*ip == '"' || *ip == '\\' || *ip == '/') {
                *op++ = *ip++;
            } else if (*ip == 'b') {
...
static pj_json_elem *parse_elem_throw(struct parse_state *st, pj_json_elem *elem)
{
    ...
    /* Parse value */
    if (pj_cis_match(&st->float_spec, *st->scanner.curptr) || *st->scanner.curptr == '-') {
        float val;
...
PJ_DEF(pj_json_elem *) pj_json_parse(pj_pool_t *pool, char *buffer, unsigned *size, pj_json_err_info *err_info)
{
    ...
    pj_scan_init(&st.scanner, buffer, *size, PJ_SCAN_AUTOSKIP_WS | PJ_SCAN_AUTOSKIP_NEWLINE, &on_syntax_error);
...
PJ_DEF(pj_status_t) pj_json_write(const pj_json_elem *elem, char *buffer, unsigned *size)
{
...
static pj_status_t write_string_escaped(const pj_str_t *value, struct write_state *st)
{
...
static pj_status_t write_children(const pj_json_list *list, const char quotes[2], struct write_state *st)
{
...
static pj_status_t elem_write(const pj_json_elem *elem, struct write_state *st, unsigned flags)
{
    ...
    if (elem->name.slen) {
        ...
        if ((flags & NO_NAME) == 0) {
            ...
            {
                CHECK(st->writer(st->space, (unsigned)(PJ_JSON_NAME_MIN_LEN - elem->name.slen), st->user_data));
            }
        }
    }
    ...
    case PJ_JSON_VAL_NUMBER: {
        ...
        if (elem->value.num == (int)elem->value.num)
            len = pj_ansi_snprintf(num_buf, sizeof(num_buf), "%d", (int)elem->value.num);
        else
            len = pj_ansi_snprintf(num_buf, sizeof(num_buf), "%f", elem->value.num);
...
PJ_DEF(pj_status_t) pj_json_writef(const pj_json_elem *elem, pj_json_writer writer, void *user_data)
{
