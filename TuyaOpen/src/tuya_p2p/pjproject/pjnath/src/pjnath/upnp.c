...
static const char *UPNP_IGD_DEVICE = "urn:schemas-upnp-org:device:InternetGatewayDevice:1";
static const char *UPNP_WANIP_SERVICE = "urn:schemas-upnp-org:service:WANIPConnection:1";
static const char *UPNP_WANPPP_SERVICE = "urn:schemas-upnp-org:service:WANPPPConnection:1";
...
static const char *check_error_response(IXML_Document *doc)
{
    ...
    if (error_code) {
        ...
        PJ_LOG(3, (THIS_FILE, "Response error code: %s (%s)", error_code, error_desc));
    }
...
static const char *action_get_external_ip(struct igd *igd)
{
    ...
    /* Send the action XML. */
    upnp_err =
        UpnpSendAction(upnp_mgr.client_hnd, igd->control_url.ptr, igd->service_type.ptr, NULL, action, &response);
    if (upnp_err != UPNP_E_SUCCESS || !response) {
        PJ_LOG(3, (THIS_FILE, "Failed to send GetExternalIPAddress action: %s", UpnpGetErrorMessage(upnp_err)));
        goto on_error;
    }
...
static void download_igd_xml(unsigned dev_idx)
{
    ...
    if (upnp_err != UPNP_E_SUCCESS || !doc) {
        PJ_LOG(3, (THIS_FILE, "Error downloading device XML doc from %s: %s", url, UpnpGetErrorMessage(upnp_err)));
        goto on_error;
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!service_node || (pj_ansi_strcmp(ixmlNode_getNodeName(service_node), "service"))) {
            continue;
        }

        /* We only want serviceType of WANIPConnection or WANPPPConnection. */
        service_type = get_node_value(service_type_node);
        if (pj_ansi_strcmp(service_type, UPNP_WANIP_SERVICE) && pj_ansi_strcmp(service_type, UPNP_WANPPP_SERVICE)) {
            continue;
        }
        ...
        if (upnp_err == UPNP_E_SUCCESS) {
            pj_strdup2_with_null(upnp_mgr.pool, &igd_dev->control_url, abs_control_url);
            free(abs_control_url);
        } else {
            PJ_LOG(4, (THIS_FILE, "Error resolving absolute controlURL: %s", UpnpGetErrorMessage(upnp_err)));
            pj_strdup2_with_null(upnp_mgr.pool, &igd_dev->control_url, control_url);
        }
        ...
        PJ_LOG(4, (THIS_FILE,
                   ...
                   "\tControl URL  : %s\n"
                   "\tPublic IP    : %s",
                   igd_dev->dev_id.ptr, friendly_name, igd_dev->service_type.ptr, igd_dev->control_url.ptr, public_ip));
...
static void add_device(const char *dev_id, const char *url)
{
    ...
    for (i = 0; i < upnp_mgr.igd_cnt; i++) {
        if (!pj_strcmp2(&upnp_mgr.igd_devs[i].dev_id, dev_id) && !pj_strcmp2(&upnp_mgr.igd_devs[i].url, url)) {
            /* Device exists. */
            pj_mutex_unlock(upnp_mgr.mutex);
            return;
        }
    ...
    pj_strdup2_with_null(upnp_mgr.pool, &upnp_mgr.igd_devs[upnp_mgr.igd_cnt].dev_id, dev_id);
    pj_strdup2_with_null(upnp_mgr.pool, &upnp_mgr.igd_devs[upnp_mgr.igd_cnt++].url, url);
    pj_mutex_unlock(upnp_mgr.mutex);
    ...
    for (i = 0; i < upnp_mgr.igd_cnt; i++) {
        ...
        if (!pj_strcmp2(&igd->dev_id, dev_id) && igd->valid) {
            ...
            if (upnp_mgr.primary_igd_idx < 0) {
                ...
                PJ_LOG(4, (THIS_FILE, "Using primary IGD %s", upnp_mgr.igd_devs[i].dev_id.ptr));
            }
        }
    }
}
    ...
    for (i = 0; i < (int)upnp_mgr.igd_cnt; i++) {
        ...
        if (!pj_strcmp2(&igd->dev_id, dev_id) && igd->valid) {
            ...
            if (i == upnp_mgr.primary_igd_idx) {
                ...
                PJ_LOG(4, (THIS_FILE, "Device %s offline, now using IGD %s", upnp_mgr.igd_devs[i].dev_id.ptr,
                           (upnp_mgr.primary_igd_idx < 0 ? "(none)" : igd->dev_id.ptr)));
            }
            pj_mutex_unlock(upnp_mgr.mutex);
        }
    }
...
static int client_cb(Upnp_EventType event_type, const void *event, void *user_data)
{
    ...
    case UPNP_DISCOVERY_SEARCH_RESULT: {
        ...
        if (upnp_status != UPNP_E_SUCCESS) {
            PJ_LOG(4, (THIS_FILE, "UPnP discovery error: %s", UpnpGetErrorMessage(upnp_status)));
            break;
        }
...
static void search_igd(int search_time)
{
    ...
    err = UpnpSearchAsync(upnp_mgr.client_hnd, search_time, UPNP_ROOT_DEVICE, &upnp_mgr);
    if (err != UPNP_E_SUCCESS) {
        PJ_LOG(4, (THIS_FILE, "Searching for UPNP_ROOT_DEVICE failed: %s", UpnpGetErrorMessage(err)));
    }

    err = UpnpSearchAsync(upnp_mgr.client_hnd, search_time, UPNP_IGD_DEVICE, &upnp_mgr);
    if (err != UPNP_E_SUCCESS) {
        PJ_LOG(4, (THIS_FILE, "Searching for UPNP_IGD_DEVICE failed: %s", UpnpGetErrorMessage(err)));
    }

    err = UpnpSearchAsync(upnp_mgr.client_hnd, search_time, UPNP_WANIP_SERVICE, &upnp_mgr);
    if (err != UPNP_E_SUCCESS) {
        PJ_LOG(4, (THIS_FILE, "Searching for UPNP_WANIP_SERVICE failed: %s", UpnpGetErrorMessage(err)));
    }

    err = UpnpSearchAsync(upnp_mgr.client_hnd, search_time, UPNP_WANPPP_SERVICE, &upnp_mgr);
    if (err != UPNP_E_SUCCESS) {
        PJ_LOG(4, (THIS_FILE, "Searching for UPNP_WANPPP_SERVICE failed: %s", UpnpGetErrorMessage(err)));
    }
}
...
PJ_DEF(pj_status_t) pj_upnp_init(const pj_upnp_init_param *param)
{
    ...
    if (upnp_err != UPNP_E_SUCCESS) {
        PJ_LOG(4, (THIS_FILE, "Failed to initialize UPnP log: %s", UpnpGetErrorMessage(upnp_err)));
    }
#endif
    ...
    if (upnp_err != UPNP_E_SUCCESS) {
        PJ_LOG(1, (THIS_FILE,
                   ...
                   "interface %s: %s",
                   (param->if_name ? param->if_name : "NULL"), UpnpGetErrorMessage(upnp_err)));
        return PJ_EUNKNOWN;
    }
    ...
    if (upnp_err != UPNP_E_SUCCESS) {
        PJ_LOG(1, (THIS_FILE, "Failed to register client: %s", UpnpGetErrorMessage(upnp_err)));
        UpnpFinish();
...
#endif
    if (param->if_name) {
        PJ_LOG(4, (THIS_FILE, "UPnP initialized with interface %s", param->if_name));
    }
    ...
    search_igd(param->search_time > 0 ? param->search_time : PJ_UPNP_DEFAULT_SEARCH_TIME);
...
PJ_DECL(pj_status_t)
pj_upnp_add_port_mapping(unsigned sock_cnt, const pj_sock_t sock[], unsigned ext_port[], pj_sockaddr mapped_addr[])
{
    ...
    for (i = 0; i < sock_cnt; i++) {
        ...
        /* Create action XML. */
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr, "NewRemoteHost", "");
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr, "NewExternalPort", pext_port);
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr, "NewProtocol", "UDP");
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr, "NewInternalPort", int_port_buf);
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr, "NewInternalClient", addr_buf);
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr, "NewEnabled", "1");
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr, "NewPortMappingDescription",
                        PORT_MAPPING_DESCRIPTION);
        UpnpAddToAction(&action, ACTION_ADD_PORT_MAPPING, igd->service_type.ptr, "NewLeaseDuration", "0");
        ...
        if (upnp_err != UPNP_E_SUCCESS || !response) {
            PJ_LOG(3, (THIS_FILE,
                       ...
                       "for %s:%s -> %s:%s: %d (%s)",
                       response ? "send action to" : "get response from", igd->dev_id.ptr, igd->public_ip.ptr,
                       pext_port, addr_buf, int_port_buf, upnp_err, UpnpGetErrorMessage(upnp_err)));
            status = PJ_ETIMEDOUT;
        }

        TRACE_("Add port mapping XML action:\n%s", ixmlPrintDocument(action));
        TRACE_("Add port mapping XML response:\n%s", (response ? ixmlPrintDocument(response) : "empty"));
        ...
        pj_sockaddr_set_str_addr(bound_addr.addr.sa_family, &mapped_addr[i], &igd->public_ip);
        pj_sockaddr_set_port(&mapped_addr[i], (pj_uint16_t)(ext_port ? ext_port[i] : int_port));
        ...
        PJ_LOG(4, (THIS_FILE,
                   ...
                   "%s:%s -> %s:%s",
                   igd->dev_id.ptr, igd->public_ip.ptr, pext_port, addr_buf, int_port_buf));
    }
...
PJ_DEF(pj_status_t) pj_upnp_del_port_mapping(const pj_sockaddr *mapped_addr)
{
    ...
    if (pj_sockaddr_cmp(&igd->public_ip_addr, &host_addr)) {
        ...
        for (i = 0; i < upnp_mgr.igd_cnt; i++, igd = NULL) {
            ...
            if (igd->valid && igd->alive && !pj_sockaddr_cmp(&igd->public_ip_addr, &host_addr)) {
                break;
            }
        }
    }
    ...
    /* Create action XML. */
    UpnpAddToAction(&action, ACTION_DELETE_PORT_MAPPING, igd->service_type.ptr, "NewRemoteHost", "");
    UpnpAddToAction(&action, ACTION_DELETE_PORT_MAPPING, igd->service_type.ptr, "NewExternalPort", ext_port_buf);
    UpnpAddToAction(&action, ACTION_DELETE_PORT_MAPPING, igd->service_type.ptr, "NewProtocol", "UDP");

    /* For mapping deletion, send the action XML async, to avoid long
     * wait in network disconnection scenario.
     */
    upnp_err = UpnpSendActionAsync(upnp_mgr.client_hnd, igd->control_url.ptr, igd->service_type.ptr, NULL, action,
                                   client_cb, &upnp_mgr);
    if (upnp_err == UPNP_E_SUCCESS) {
        PJ_LOG(4, (THIS_FILE,
        ...
        PJ_LOG(3, (THIS_FILE,
                   ...
                   igd->dev_id.ptr, igd->public_ip.ptr, ext_port_buf, upnp_err, UpnpGetErrorMessage(upnp_err)));
        status = PJ_EINVALIDOP;
    }
