...
static int rtcp_psfb_pli_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_sli_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_rpsi_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                 const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_fir_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_tstr_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                 const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_tstn_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                 const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_vbcm_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                 const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_pslei_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                  const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_lrr_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_roi_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                const uint8_t *ptr, size_t bytes);
static int rtcp_psfb_afb_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                                const uint8_t *ptr, size_t bytes);
                                ...
                                const uint8_t *ptr, size_t bytes)
{
    ...
    // 2. PLI does not require parameters.  Therefore, the length field MUST be 2, and there MUST NOT be any Feedback
    // Control Information.
    assert(0 == bytes);
...
static int rtcp_psfb_lrr_pack(const rtcp_lrr_t *lrr, int count, uint8_t *ptr, uint32_t bytes)
{
    ...
    for (i = 0; i < count && bytes >= 12; i++) {
        ...
        nbo_w32(ptr, ((lrr[i].ttid & 0x07) << 24) | ((lrr[i].tlid & 0xFF) << 16) | ((lrr[i].ctid & 0x07) << 8) |
                         ((lrr[i].clid & 0xFF) << 0));
...
static void rtcp_on_psfb_test(void *param, const struct rtcp_msg_t *msg)
{
    ...
    case RTCP_PSFB:
        ...
        case RTCP_PSFB_REMB:
            ...
            assert(0x23456789 == msg->u.psfb.u.afb.remb[0].ssrc && 1 == msg->u.psfb.u.afb.remb[0].exp &&
                   0x3fb93 == msg->u.psfb.u.afb.remb[0].mantissa);
            assert(0x2345678a == msg->u.psfb.u.afb.remb[1].ssrc && 1 == msg->u.psfb.u.afb.remb[1].exp &&
                   0x3fb93 == msg->u.psfb.u.afb.remb[1].mantissa);
            assert(0x2345678b == msg->u.psfb.u.afb.remb[2].ssrc && 1 == msg->u.psfb.u.afb.remb[2].exp &&
                   0x3fb93 == msg->u.psfb.u.afb.remb[2].mantissa);
...
{
    const uint8_t data[] = {'R',  'E',  'M',  'B',  0x03, 0x07, 0xfb, 0x93, 0x23, 0x45,
                            0x67, 0x89, 0x23, 0x45, 0x67, 0x8a, 0x23, 0x45, 0x67, 0x8b};
