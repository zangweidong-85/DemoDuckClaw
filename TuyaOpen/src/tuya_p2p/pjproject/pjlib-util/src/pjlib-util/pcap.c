...
/* Open pcap file */
PJ_DEF(pj_status_t) pj_pcap_open(pj_pool_t *pool, const char *path, pj_pcap_file **p_file)
{
    pj_pcap_file *file;
    pj_ssize_t sz;
    pj_status_t status;
...
/* Setup filter */
PJ_DEF(pj_status_t) pj_pcap_set_filter(pj_pcap_file *file, const pj_pcap_filter *fil)
{
...
pj_pcap_read_udp(pj_pcap_file *file, pj_pcap_udp_hdr *udp_hdr, pj_uint8_t *udp_payload, pj_size_t *udp_payload_size)
{
    ...
    if ((file->filter.link && file->hdr.network != (pj_uint32_t)file->filter.link) ||
    ...
    for (;;) {
        ...
        if (file->filter.ip_src && tmp.ip.ip_src != file->filter.ip_src) {
            TRACE_((file->obj_name, "IP source %s mismatch, skipping",
                    pj_inet_ntop2(pj_AF_INET(), (pj_in_addr *)&tmp.ip.ip_src, addr, sizeof(addr))));
            SKIP_PKT();
            continue;
        }
        ...
        if (file->filter.ip_dst && tmp.ip.ip_dst != file->filter.ip_dst) {
            TRACE_((file->obj_name, "IP detination %s mismatch, skipping",
                    pj_inet_ntop2(pj_AF_INET(), (pj_in_addr *)&tmp.ip.ip_dst, addr, sizeof(addr))));
            SKIP_PKT();
            continue;
        }
        ...
        if (file->filter.proto && tmp.ip.proto != file->filter.proto) {
            TRACE_((file->obj_name, "IP proto %d mismatch, skipping", tmp.ip.proto));
            SKIP_PKT();
            continue;
        }
        ...
        case PJ_PCAP_PROTO_TYPE_UDP:
            ...
            if (file->filter.src_port && tmp.udp.src_port != file->filter.src_port) {
                TRACE_((file->obj_name, "UDP src port %d mismatch, skipping", pj_ntohs(tmp.udp.src_port)));
                SKIP_PKT();
                continue;
            }

            /* Skip if destination port mismatch */
            if (file->filter.dst_port && tmp.udp.dst_port != file->filter.dst_port) {
                TRACE_((file->obj_name, "UDP dst port %d mismatch, skipping", pj_ntohs(tmp.udp.dst_port)));
                SKIP_PKT();
                continue;
            }
        ...
        if (sz > (pj_ssize_t)*udp_payload_size) {
            TRACE_((file->obj_name, "Error: packet too large (%d bytes required)", sz));
            SKIP_PKT();
            return PJ_ETOOSMALL;
        }
