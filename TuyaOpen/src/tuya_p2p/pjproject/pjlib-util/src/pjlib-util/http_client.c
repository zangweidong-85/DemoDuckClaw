...
enum http_protocol { PROTOCOL_HTTP, PROTOCOL_HTTPS };
...
enum http_method { HTTP_GET, HTTP_PUT, HTTP_DELETE };
...
static pj_status_t http_headers_parse(char *hdata, pj_size_t size, pj_http_headers *headers);
/* Parse the response */
static pj_status_t http_response_parse(pj_pool_t *pool, pj_http_resp *response, void *data, pj_size_t size,
                                       pj_size_t *remainder);
/* Restart the request with authentication */
static void restart_req_with_auth(pj_http_req *hreq);
/* Parse authentication challenge */
static pj_status_t parse_auth_chal(pj_pool_t *pool, pj_str_t *input, pj_http_auth_chal *chal);
...
static pj_bool_t http_on_data_sent(pj_activesock_t *asock, pj_ioqueue_op_key_t *op_key, pj_ssize_t sent)
{
    pj_http_req *hreq = (pj_http_req *)pj_activesock_get_user_data(asock);
    ...
    TRACE_(
        (THIS_FILE, "\nData sent: %d out of %d bytes", hreq->tcp_state.current_send_size, hreq->tcp_state.send_size));
        ...
        if (hreq->param.reqdata.total_size > 0 || hreq->param.reqdata.size > 0) {
            if (hreq->state == SENDING_REQUEST) {
                ...
                pj_assert(hreq->param.reqdata.total_size == 0 ||
                          (hreq->param.reqdata.total_size > 0 && hreq->param.reqdata.size == 0));
            } else {
                /* Continue sending the next chunk of the request body */
                hreq->tcp_state.tot_chunk_size += hreq->tcp_state.send_size;
                if (hreq->tcp_state.tot_chunk_size == hreq->param.reqdata.total_size ||
            ...
            if (hreq->param.reqdata.total_size > 0 && hreq->cb.on_send_data) {
                 ...
                 */
                (*hreq->cb.on_send_data)(hreq, &hreq->param.reqdata.data, &hreq->param.reqdata.size);
                /* Make sure the total data size given by the user does not
                 * exceed what the user originally said.
                 */
                pj_assert(hreq->tcp_state.tot_chunk_size + hreq->param.reqdata.size <= hreq->param.reqdata.total_size);
            }
            http_req_start_sending(hreq);
        } else {
...
static pj_bool_t http_on_data_read(pj_activesock_t *asock, void *data, pj_size_t size, pj_status_t status,
                                   pj_size_t *remainder)
{
    pj_http_req *hreq = (pj_http_req *)pj_activesock_get_user_data(asock);
    ...
    if (hreq->state == READING_RESPONSE) {
        ...
        if (st == PJLIB_UTIL_EHTTPINCHDR) {
            ...
            *remainder = size;
             ...
             */
            if (hreq->response.status_code == 401 || hreq->response.status_code == 407) {
                ...
                for (i = 0; i < hdrs->count; i++) {
                    if (!pj_stricmp(&hdrs->header[i].name, &STR_WWW_AUTH) ||
                        ...
                        status = parse_auth_chal(hreq->pool, &hdrs->header[i].value, &response->auth_chal);
                        break;
                    }
                }

                /* Check if we should perform authentication */
                if (status == PJ_SUCCESS && hreq->auth_state == AUTH_NONE && hreq->response.auth_chal.scheme.slen &&
                    hreq->param.auth_cred.username.slen &&
                    (hreq->param.auth_cred.scheme.slen == 0 ||
                     !pj_stricmp(&hreq->response.auth_chal.scheme, &hreq->param.auth_cred.scheme)) &&
                    (hreq->param.auth_cred.realm.slen == 0 ||
                     !pj_stricmp(&hreq->response.auth_chal.realm, &hreq->param.auth_cred.realm))) {
            ...
            if (rem > 0 || hreq->response.content_length == 0)
                return http_on_data_read(asock, (rem == 0 ? NULL : (char *)data + size - rem), rem, PJ_SUCCESS, NULL);
        }
    ...
    if (hreq->cb.on_data_read) {
        ...
        if (hreq->response.size == 0) {
             ...
             */
            hreq->response.size =
                (hreq->response.content_length == -1 ? INITIAL_DATA_BUF_SIZE : hreq->response.content_length);
            hreq->response.data = pj_pool_alloc(hreq->pool, hreq->response.size);
        }
        ...
        if (hreq->tcp_state.current_read_size + size > hreq->response.size) {
            ...
            hreq->response.data = pj_pool_alloc(hreq->pool, hreq->response.size << 1);
            pj_memcpy(hreq->response.data, olddata, hreq->response.size);
            hreq->response.size <<= 1;
        }

        /* Append the response data. */
        pj_memcpy((char *)hreq->response.data + hreq->tcp_state.current_read_size, data, size);
    }
    ...
    if ((hreq->response.content_length >= 0 &&
         (pj_ssize_t)hreq->tcp_state.current_read_size >= hreq->response.content_length) ||
...
{
    pj_scanner scanner;
    const pj_str_t REALM_STR = {"realm", 5}, NONCE_STR = {"nonce", 5}, ALGORITHM_STR = {"algorithm", 9},
                   STALE_STR = {"stale", 5}, QOP_STR = {"qop", 3}, OPAQUE_STR = {"opaque", 6};
    pj_status_t status = PJ_SUCCESS;
    PJ_USE_EXCEPTION;

    pj_scan_init(&scanner, input->ptr, input->slen, PJ_SCAN_AUTOSKIP_WS, &on_syntax_error);
    PJ_TRY
    {
        /* Get auth scheme */
        if (*scanner.curptr == '"') {
        ...
        for (;;) {
            ...
            if (*scanner.curptr == '=') {
                ...
                if (!pj_scan_is_eof(&scanner)) {
                    if (*scanner.curptr == '"' || *scanner.curptr == '\'') {
                        ...
                        pj_scan_get_quote(&scanner, quote_char, quote_char, &value);
                        value.ptr++;
                        value.slen -= 2;
            ...
            if (!pj_stricmp(&name, &REALM_STR)) {
                ...
                chal->stale = value.slen && (*value.ptr != '0') && (*value.ptr != 'f') && (*value.ptr != 'F');
            }
 ...
 * its parameters.
 */
PJ_DEF(pj_status_t) pj_http_headers_add_elmt2(pj_http_headers *headers, char *name, char *val)
{
    pj_str_t f, v;
    pj_cstr(&f, name);
    pj_cstr(&v, val);
...
PJ_DEF(pj_status_t) pj_http_headers_add_elmt(pj_http_headers *headers, pj_str_t *name, pj_str_t *val)
{
    ...
    *remainder = size - 1 - i;
    ...
    for (i = 0; i < response->headers.count; i++) {
        if (!pj_stricmp(&response->headers.header[i].name, &STR_CONTENT_LENGTH)) {
            response->content_length = pj_strtoul(&response->headers.header[i].value);
...
PJ_DEF(pj_status_t) pj_http_req_parse_url(const pj_str_t *url, pj_http_url *hurl)
{
    pj_scanner scanner;
    ...
    PJ_TRY
    {
        ...
        if (!pj_stricmp2(&s, http_protocol_names[PROTOCOL_HTTP])) {
            pj_strset2(&hurl->protocol, (char *)http_protocol_names[PROTOCOL_HTTP]);
        } else if (!pj_stricmp2(&s, http_protocol_names[PROTOCOL_HTTPS])) {
            pj_strset2(&hurl->protocol, (char *)http_protocol_names[PROTOCOL_HTTPS]);
        } else {
...
PJ_DEF(void) pj_http_req_set_timeout(pj_http_req *http_req, const pj_time_val *timeout)
{
    pj_memcpy(&http_req->param.timeout, timeout, sizeof(*timeout));
}

PJ_DEF(pj_status_t)
pj_http_req_create(pj_pool_t *pool, const pj_str_t *url, pj_timer_heap_t *timer, pj_ioqueue_t *ioqueue,
                   const pj_http_req_param *param, const pj_http_req_callback *hcb, pj_http_req **http_req)
{
    pj_pool_t *own_pool;
    pj_http_req *hreq;
    char *at_pos;
    ...
    PJ_ASSERT_RETURN(pool && url && timer && ioqueue && hcb && http_req, PJ_EINVAL);

    *http_req = NULL;
    own_pool = pj_pool_create(pool->factory, NULL, INITIAL_POOL_SIZE, POOL_INCREMENT_SIZE, NULL);
    ...
    if (param) {
         ...
         */
        PJ_ASSERT_RETURN(hreq->param.addr_family == pj_AF_UNSPEC() || hreq->param.addr_family == pj_AF_INET() ||
                             hreq->param.addr_family == pj_AF_INET6(),
                         PJ_EAFNOTSUP);
        PJ_ASSERT_RETURN(!pj_strcmp2(&hreq->param.version, HTTP_1_0) || !pj_strcmp2(&hreq->param.version, HTTP_1_1),
                         PJ_ENOTSUP);
    ...
    if ((at_pos = get_url_at_pos(hreq->url.ptr, hreq->url.slen)) != NULL) {
        ...
        if (hreq->hurl.host.ptr > user_pos && hreq->hurl.host.ptr < user_pos + hreq->url.slen) {
            hreq->hurl.host.ptr -= removed_len;
        }
        /* path may come from a string constant, don't shift it if so */
        if (hreq->hurl.path.ptr > user_pos && hreq->hurl.path.ptr < user_pos + hreq->url.slen) {
...
static pj_status_t start_http_req(pj_http_req *http_req, pj_bool_t notify_on_fail)
{
    ...
    if (!http_req->resolved) {
        ...
        status =
            pj_sockaddr_init(http_req->param.addr_family, &http_req->addr, &http_req->hurl.host, http_req->hurl.port);
        if (status != PJ_SUCCESS || !pj_sockaddr_has_addr(&http_req->addr) ||
            (http_req->param.addr_family == pj_AF_INET() && http_req->addr.ipv4.sin_addr.s_addr == PJ_INADDR_NONE)) {
            goto on_return;
        }
        http_req->resolved = PJ_TRUE;
    }

    status = pj_sock_socket(http_req->param.addr_family, pj_SOCK_STREAM(), 0, &sock);
    if (status != PJ_SUCCESS)
    ...
    if (status != PJ_SUCCESS) {
        PJ_PERROR(1, (THIS_FILE, status, "Unable to bind to the requested port"));
        pj_sock_close(sock);
        goto on_return;
    }
    ...
    status = pj_activesock_create(http_req->pool, sock, pj_SOCK_STREAM(), NULL, http_req->ioqueue, &asock_cb, http_req,
                                  &http_req->asock);
    if (status != PJ_SUCCESS) {
        pj_sock_close(sock);
    ...
    status = pj_timer_heap_schedule(http_req->timer, &http_req->timer_entry, &http_req->param.timeout);
    if (status != PJ_SUCCESS) {
    ...
    status = pj_activesock_start_connect(http_req->asock, http_req->pool, (pj_sockaddr_t *)&(http_req->addr),
...
static pj_status_t auth_respond_basic(pj_http_req *hreq)
{
    ...
    pj_base64_encode((pj_uint8_t *)user_pass.ptr, (int)user_pass.slen, phdr->value.ptr + phdr->value.slen, &len);
    phdr->value.slen += len;
...
static void auth_create_digest_response(pj_str_t *result, const pj_http_auth_cred *cred, const pj_str_t *nonce,
...
static pj_status_t auth_respond_digest(pj_http_req *hreq)
{
    ...
    if (chal->algorithm.slen != 0 && pj_stricmp2(&chal->algorithm, "MD5")) {
        TRACE_((THIS_FILE, "Error: Unsupported digest algorithm \"%.*s\"", chal->algorithm.slen, chal->algorithm.ptr));
        return PJ_ENOTSUP;
    }
    ...
    if (chal->qop.slen == 0) {
        ...
        auth_create_digest_response(&digest_response, cred, &chal->nonce, NULL, NULL, NULL, &hreq->hurl.path,
        ...
        len = pj_ansi_snprintf(phdr->value.ptr, max_len,
                               ...
                               "algorithm=%.*s, "
                               "response=\"%.*s\"",
                               STR_PREC(cred->username), STR_PREC(chal->realm), STR_PREC(chal->nonce),
                               STR_PREC(hreq->hurl.path), STR_PREC(STR_MD5), STR_PREC(digest_response));
        ...
        auth_create_digest_response(&digest_response, cred, &chal->nonce, &nc, &cnonce, &qop, &hreq->hurl.path,
                                    &chal->realm, &hreq->param.method);
        max_len = len;
        len = pj_ansi_snprintf(phdr->value.ptr, max_len,
                               "Digest username=\"%.*s\", "
                               ...
                               "cnonce=\"%.*s\"",
                               STR_PREC(cred->username), STR_PREC(chal->realm), STR_PREC(chal->nonce),
                               STR_PREC(hreq->hurl.path), STR_PREC(STR_MD5), STR_PREC(digest_response), STR_PREC(qop),
                               STR_PREC(nc), STR_PREC(cnonce));
        if (len < 0 || len >= max_len)
            return PJ_ETOOSMALL;
        phdr->value.slen = len;
        ...
        TRACE_((THIS_FILE, "Error: Unsupported qop offer %.*s", chal->qop.slen, chal->qop.ptr));
        return PJ_ENOTSUP;
    }
 ...
 */
static void str_snprintf(pj_str_t *s, size_t size, pj_bool_t append, const char *format, ...)
{
    va_list arg;
    int retval;
...
static pj_status_t http_req_start_sending(pj_http_req *hreq)
{
    ...
    PJ_ASSERT_RETURN(hreq->state == SENDING_REQUEST || hreq->state == SENDING_REQUEST_BODY, PJ_EBUG);
    ...
    if (hreq->state == SENDING_REQUEST) {
        ...
        str_snprintf(&pkt, BUF_SIZE, PJ_TRUE, "%.*s %.*s %s/%.*s\r\n", STR_PREC(hreq->param.method),
                     STR_PREC(hreq->hurl.path), get_protocol(&hreq->hurl.protocol), STR_PREC(hreq->param.version));
        /* Header field "Host" */
        ...
        if (!pj_strcmp2(&hreq->param.method, http_method_names[HTTP_PUT])) {
            ...
            pj_utoa(hreq->param.reqdata.total_size ? (unsigned long)hreq->param.reqdata.total_size
...
static pj_status_t http_req_start_reading(pj_http_req *hreq)
{
    ...
    status = pj_activesock_start_read2(hreq->asock, hreq->pool, BUF_SIZE, (void **)&hreq->buffer.ptr, 0);
    if (status != PJ_SUCCESS) {
        /* Error reading */
        http_req_end_request(hreq);
        return status;
