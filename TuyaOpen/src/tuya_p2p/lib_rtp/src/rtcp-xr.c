...
/*
   ...
   [http://focus.ti.com/general/docs/bcg/bcgdoccenter.tsp?templateId=6116&navigationId=12078#42][David_Lide] 10
   Post-repair Loss RLE Report Block	            [RFC5725] 11	    Multicast Acquisition Report Block [RFC6332] 12
   IDMS Report Block	                            [RFC7272] 13	    ECN Summary Report [RFC6679] 14 Measurement
   Information Block	                [RFC6776] 15	    Packet Delay Variation Metrics Block	        [RFC6798] 16
   Delay Metrics Block	                            [RFC6843] 17	    Burst/Gap Loss Summary Statistics Block
   [RFC7004] 18	    Burst/Gap Discard Summary Statistics Block	    [RFC7004] 19	    Frame Impairment Statistics
   Summary	            [RFC7004] 20	    Burst/Gap Loss Metrics Block	                [RFC6958]
...
static int rtcp_xr_lrle_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                               const uint8_t *ptr, size_t bytes);
static int rtcp_xr_drle_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                               const uint8_t *ptr, size_t bytes);
static int rtcp_xr_prt_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                              const uint8_t *ptr, size_t bytes);
static int rtcp_xr_rrt_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                              const uint8_t *ptr, size_t bytes);
static int rtcp_xr_dlrr_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                               const uint8_t *ptr, size_t bytes);
static int rtcp_xr_ecn_unpack(struct rtp_context *ctx, const rtcp_header_t *header, struct rtcp_msg_t *msg,
                              const uint8_t *ptr, size_t bytes);
...
static void rtcp_on_xr_test(void *param, const struct rtcp_msg_t *msg)
{
    ...
    case RTCP_XR:
        ...
        case RTCP_XR_DLRR:
            ...
            assert(0x12345678 == msg->u.xr.u.dlrr.dlrr[0].ssrc && 0x23344556 == msg->u.xr.u.dlrr.dlrr[0].lrr &&
                   0x33343536 == msg->u.xr.u.dlrr.dlrr[0].dlrr);
...
{
    const uint8_t data[] = {0x05, 0x00, 0x00, 0x03, 0x12, 0x34, 0x56, 0x78,
